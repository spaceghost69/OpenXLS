...
public class XLSRecord implements BiffRec, BlockByteConsumer, Serializable, XLSConstants  {
        ...
        protected AbstractList<Continue> continues;    
        int originalsize = -1,
            originalIndex = 0,
            originalOffset = 0,
            ixfe = -1;
        ...
        public void setOpcode(short op){ this.opcode = op; }
	...
	*/
	...
	public int getOffset() {
	    if(this.data ==null)return this.originalOffset;
		return (int) offset;
	}
...
/* unused: 	public void setBlocks(Block[] myb) {
	...
	public String getRecDesc(){
		...
		// file offset
		ret += " file " + (databuf == null ? "no file"/*originalFileOffset*/ : 
			databuf.getFileOffsetString( offset, reclen ) );
		// cell address, if applicable
		if (this.isValueForCell()) ret += " cell " + this.getCellAddress();
     ...
     * 
     * @see com.valkyrlabs.formats.XLS.BiffRec#copyFormat(com.valkyrlabs.formats.XLS.BiffRec)
     */
    ...
    */
    public Object clone(){
        try{
            ...
            rec.init();
            return rec;
        }catch(Exception e){Logger.logInfo("cloning XLSRecord " + this.getCellAddress() + " failed: "+  e);}
        return null;
    }
    ...
    public boolean isNumber() {
        if(this.opcode == RK)return true;
        if(this.opcode == NUMBER) return true;
       return false;
    }
    ...
    /** methods from ArrayListHints
    
 	protected transient int recordIdx = -1;
    */
    /** provide a hint to the ArrayList
    ...
    */
    /** set the DEBUG level*/
    public void setDebugLevel(int b){DEBUGLEVEL=b;}
    ...
    */
    public void addContinue(Continue c){
        if(continues == null)continues = new ArrayList<Continue>();
        continues.add(c);
    }
    ...
    */
    public boolean hasContinues(){
        if(continues == null)return false;
        if(this.continues.size() > 0)return true;
        return false;
    }
    ...
    /** set whether this record contains the  value
        of the Cell.
    */
    public void setIsValueForCell(boolean b){isValueForCell = b;}
    
    /** associate this record with its Index record
    */
    public void setIndex(Index id){idx = id;}
    ...
    */
    public Boundsheet getSheet(){return (Boundsheet)worksheet;}

    public void setWorkBook(WorkBook wk){wkbook = (WorkBook)wk;}
    ...
    */
    ...
    public short getColNumber(){return col;}
    ...
    */
    public String getCellAddress(){
        int rownum = rw+1;
        if((rownum < 0) && (col >= 0)){ // > 32k and the rows go negative... !
        	rownum = MAXROWS+rownum;
        }else if((rownum == 0) && (col >= 0)){ // the very last row...MAXROWS_BIFF8
    		rownum = MAXROWS;
    	}
		if((rownum > MAXROWS) || (col < 0)){
			if(DEBUGLEVEL > -1)Logger.logWarn("XLSRecord.getCellAddress() Row/Col info incorrect for Cell:" + ExcelTools.getAlphaVal(col) + String.valueOf(rownum));
        	return "";
        }
	...
	*/
	public Object getDefaultVal(){
		if(this.isDoubleNumber) return new Double(0.0);
		if(this.isFPNumber)     return new Float(0.0f);   
		if(this.isBoolean)      return Boolean.valueOf(false);   
		if(this.isIntNumber)    return Integer.valueOf(0);
		if(this.isString)       return "";
		if(this.isFormula)      return this.getFormulaRec().getFormulaString();	
...
//					return getStringVal();
		...
		}catch(Exception e){return null;} // should never happen here...
	}
    ...
    */
    public String getStringVal(String encoding){return null;}
	...
	*/
    ...
    protected void mergeContinues() {
            ...
            if (cx != null) {
                ...
                while(it.hasNext()) {
                    ...
                    byte[] nb = c.getData();
                    if (c.getHasGrbit()) {	// remove it - happens in continued StringRec ... a rare case
	...
	public byte[] getBytesAt(int off, int len) {
	    if(this.data != null) {
	    		if(len+off>data.length)len = data.length-off; // deal with bad requests
   ...
   public void setLength(int len) {
       if(this.originalsize <= 0)this.originalsize = len;  // returns the original len always
