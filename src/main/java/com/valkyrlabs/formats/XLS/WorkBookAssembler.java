...
public class WorkBookAssembler implements XLSConstants {
    ...
    private static List assembleRows(Boundsheet thissheet, List addVec) {
        ...
        int insertRowidx = 0;
        if(dim !=null)insertRowidx = dim.getRecordIndex()+1;
    ...
    private static int getLastDBCellLocation(List addVec) {
        for (int i=addVec.size()-1;i>=0;i--) {
            ...
            if(b.getOpcode()==DBCELL)return i;
        }
        return 0;
    }
    ...
    private static List assembleChartRecs(Boundsheet thissheet, List addVec) {
        ...
        for (int i=0;i<chts.length;i++) {
            if (!insertedCharts.contains(chts[i].getTitle()) && chts[i].getSheet().equals(thissheet)) {
                ...
                if(chts[i].getObj()!=null)l.add(0, chts[i].getObj());
                if(chts[i].getMsodrawobj()!=null)l.add(0, chts[i].getMsodrawobj());
                int spid = 0;
                ...
                while (chartInsert<addVec.size()) {
                 ...
                 if (x.getOpcode() == WINDOW2 || x.getOpcode() == MSODRAWINGSELECTION 
                         || x.getOpcode()==NOTE) { 
                        insertValidx = chartInsert;
                        chartInsert = addVec.size();
                    }
