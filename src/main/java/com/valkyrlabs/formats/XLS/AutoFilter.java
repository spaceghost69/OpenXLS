 ...
 * *******************************************************************************************************************
 */
public final class AutoFilter extends com.valkyrlabs.formats.XLS.XLSRecord 
{
     ...
     * @param grbt
     */
    private void decodeGrbit(short grbit) {	// top 500: grbit= -1488
    ...
    public void evaluate() {
        ...
        if (fTop10) { 
        	if (fTop) { // ascending
        		...
        		evaluateBottomN();
        } else if ((doper1 instanceof AllBlanksDoper) ||
        		(doper1 instanceof NoBlanksDoper)) {
	    	...
	    	for (int i=1; i<rows.length; i++) {
	    		try {
		    		...
		    		passes= com.valkyrlabs.formats.XLS.formulas.Calculator.compareCellValue(c, val1, op1);	            	
	            	if (hasDoper2 && (wJoin || (!wJoin && passes))) 
	            		passes= com.valkyrlabs.formats.XLS.formulas.Calculator.compareCellValue(c, val2, op2);            		            	
		            if (!passes)
		            	rows[i].setHidden(true);
	    		} catch (Exception e) { ; } // just keep evaluation
	    	}
        }
    }
    ...
    private void evaluateTopN() {
    	...
    	for (int i=0; i < n; i++) maxVals[i]= Double.NEGATIVE_INFINITY;
    	...
    	for (int i= 1; i<rows.length; i++) {
    		try {
    			...
    			if (val >= curmin) { 
    				for (int j= 0; j < n; j++) {	// see where new value falls 
    ...
    private void evaluateBottomN() {
    	...
    	for (int i=0; i < n; i++) minVals[i]= Double.POSITIVE_INFINITY;
     ...
     * 
     * @see com.valkyrlabs.formats.XLS.XLSRecord#preStream()
     */
    ...
    public void setVal(Object val, String op) {
    	...
    	if (val instanceof String) {// doper1 should be a string
    		String s= (String) val;
    ...
    private class UnusedDoper extends Doper{
        ...
        public String toString() { return "Unused"; }
    ...
    private class RKDoper extends Doper{
        ...
        public String toString() { return new Double(val).toString(); }
    ...
    private class AllBlanksDoper extends Doper{
        ...
        public String toString() { return "All Blanks"; }
    }
    ...
    private class NoBlanksDoper extends Doper{
        ...
        public String toString() { return "No Blanks"; }
    }
