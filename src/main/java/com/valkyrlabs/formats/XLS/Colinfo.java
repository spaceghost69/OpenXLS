...
extends XLSRecord implements ColumnRange {
    /** 
	* serialVersionUID
	*/
    ...
    */
    ...
    public void init(){
        ...
        decodeGrbit();
        if(DEBUGLEVEL>5)Logger.logInfo("Col: " + ExcelTools.getAlphaVal(colFirst) + "-" + ExcelTools.getAlphaVal(colLast) + "  ixfe: " + String.valueOf((int)ixfe) + " width: " + colWidth);        
    }
    ...
    public boolean isSingleColColinfo() {
        if (getColFirst() == getColLast())return true;
        return false;
    }
