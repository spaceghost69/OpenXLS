...
import com.valkyrlabs.formats.LEO.StorageNotFoundException;
import com.valkyrlabs.formats.LEO.StorageTable;
...
// KSC: TESTING				
		try {
com.valkyrlabs.toolkit.Logger.log(rec.getClass().getName().substring(rec.getClass().getName().lastIndexOf(".")+1) + ": " + Arrays.toString(((PivotCacheRecord)rec).getRecord()));				
		} catch (ClassCastException e) {
com.valkyrlabs.toolkit.Logger.log(rec.getClass().getName().substring(rec.getClass().getName().lastIndexOf(".")+1) + ": " + Arrays.toString(ByteTools.shortToLEBytes(rec.getOpcode())) + Arrays.toString(ByteTools.shortToLEBytes((short)rec.getData().length)) + Arrays.toString(rec.getData()));				
		}
*/		
				if (wbh!=null)
					curRecs.add(rec);
	...
	public void createPivotCache(StorageTable directories, WorkBookHandle wbh, String ref, int sId) 
		...
		try {
		    ...
		    if (wbh.getDebugLevel() > 100)
		    	com.valkyrlabs.toolkit.Logger.log(String.format("creatpivotCache: ref: %s sid %d", ref, sId));
				/** 
...
//com.valkyrlabs.toolkit.Logger.log(br.getClass().getName().substring(br.getClass().getName().lastIndexOf(".")+1) + ": " + Arrays.toString(((PivotCacheRecord)br).getRecord()));				
			} catch (ClassCastException e) {
...
//com.valkyrlabs.toolkit.Logger.log(br.getClass().getName().substring(br.getClass().getName().lastIndexOf(".")+1) + ": " + Arrays.toString(ByteTools.shortToLEBytes(br.getOpcode())) + Arrays.toString(ByteTools.shortToLEBytes((short)br.getData().length)) + Arrays.toString(br.getData()));				
			}
		}
...
//com.valkyrlabs.toolkit.Logger.log("SXDB: " + Arrays.toString(sxdb.getRecord()));				
			SXDBEx sxdbex= (SXDBEx)SXDBEx.getPrototype();	//TODO: nFormulas						
			newbytes= ByteTools.append(sxdbex.getRecord(), newbytes);
//com.valkyrlabs.toolkit.Logger.log("SXDBEX: " + Arrays.toString(sxdbex.getRecord()));				
...
//com.valkyrlabs.toolkit.Logger.log("SXFDB: " + Arrays.toString(sxfdb.getRecord()));				
						SXFDBType sxfdbtype= (SXFDBType) SXFDBType.getPrototype();
						newbytes= ByteTools.append(sxfdbtype.getRecord(), newbytes);
//com.valkyrlabs.toolkit.Logger.log("SXDFBTYPE: " + Arrays.toString(sxfdbtype.getRecord()));				
						continue;
					} 
...
//com.valkyrlabs.toolkit.Logger.log("SXSTRING: " + Arrays.toString(sxstring.getRecord()));				
						break;
...
//com.valkyrlabs.toolkit.Logger.log("SXNUM: " + Arrays.toString(sxnum.getRecord()));				
						break;
...
//com.valkyrlabs.toolkit.Logger.log("SXBOOL: " + Arrays.toString(sxbool.getRecord()));				
...
//com.valkyrlabs.toolkit.Logger.log("EOF: " + Arrays.toString((b)));				
		return newbytes;
	}	
}
