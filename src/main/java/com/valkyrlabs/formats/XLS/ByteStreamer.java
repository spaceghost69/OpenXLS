...
implements Serializable, XLSConstants{
    ...
    public int getRecordIndex(int opcode){
    	...
    	for(int i=0;i<records.size();i++){
    		...
    		if(rec.getOpcode() == opcode)return i;
    	}
    	return -1;
   ...
   // public ByteStreamer(WorkBook b){this.book = b;} 
    ...
    public void setBytes(byte[] b){this.bytes = b;}
    public byte[] getBytes(){return this.bytes;}
    
    public int getRecVecSize(){return records.size();}
    
    public String getName(){return myname;}

    public String getSubstreamTypeName(){return "ByteStreamer";}

    public short getSubstreamType(){return WK_WORKSHEET;}
    
	/** get a record from the underlying array
	*/
	public BiffRec getRecordAt(int t, BiffRec rec){
	   if(rec instanceof Boundsheet ||
			   rec instanceof Dbcell)
...
/*        if(rec instanceof Mulled) {
        ...
        if(rec.getSheet()!=null) {
	        Sheet bs = rec.getSheet();
	        if(bs.getSheetRecs().contains(rec)) return bs.getSheetRecs().remove(rec);
	    }
    ...
    */
     ...
     */
     ...
     public void writeRecord (OutputStream out, BiffRec rec)
     throws IOException {
    	 ...
    	 if (dt.length > 0)
    		 out.write(dt);
    	 if (LEOFile.DEBUG) LEOFile.actualOutput+= (op.length + ln.length + dt.length); // debugging
    	 rec.postStream();
     }
     ...
     public StringBuffer writeOut(OutputStream out) {
        // create a byte level lockdown file in same directory as output
 		...
 		if(System.getProperties().get("com.valkyrlabs.OpenXLS.autocreatelockdown")!=null)
 			lockit = System.getProperties().get("com.valkyrlabs.OpenXLS.autocreatelockdown").equals("true");
...
//  				add sheet recs to output vector
          ...
          while(e.hasNext()){
              ...
              if(rec.getOpcode() == INDEX) { // need to get all it's component dbcell offsets set before we can process!
                  if (lastindex != null) {
...
//            offset dlen by number of new continue headers
          ...
          if (lastindex !=null)lastindex.updateDbcellPointers();
          e = rex.iterator();
					...
					// deal with CONTINUE record changes before streaming
	  				if (ContinueHandler.createContinues(rec, out, this )) {
	  				    ...
	  				    if (lockit) {
	  				    	...
	  				    	//lockdown.append("\r\n");
	  				    	lockdown.append("opc:0x"+ Integer.toHexString(rec.getOpcode())+ " ["+ ByteTools.getByteString(op, false)+ "]");
	  				    	lockdown.append("\r\n");
	  				    	lockdown.append("len:0x"+ Integer.toHexString(rec.getLength())+ " ["+ ByteTools.getByteString(ln, false)+ "]");
	  				    	lockdown.append("\r\n");
          ...
          for(int t=0;t<filler.length;t++)filler[t] = 0;
          ...
          try {
        	  ...
        	  while(its.hasNext()) {
        		  ...
        		  if(ob!=null)
        			  if(!ob.getName().equals("Root Entry") || true) {  // was newLeoProcessing, so should always run?
        				   if (ob.getBlockType() != Block.SMALL)	{// already written in miniFAT Container - see buildSAT 
        					   ob.writeBytes(out);
          ...
          if(lockit){ // write the lockdown file
        	  return lockdown;
          }
          return null;
      }
		...
		for (int i= 0; i < recArr.size(); i++) {
			try {
				...
				if (b instanceof com.valkyrlabs.formats.XLS.charts.SeriesText)
					writer.write("\t[" + ((com.valkyrlabs.formats.XLS.charts.SeriesText) b).toString()+ "]");
...
//					writer.write("\t[" + ByteTools.getByteDump(b.getData(), 0).substring(11)+ "]");
				...
				writer.newLine();
				if (b instanceof com.valkyrlabs.formats.XLS.charts.ChartObject) {
		        	writeRecs(((com.valkyrlabs.formats.XLS.charts.ChartObject)b).getChartRecords(), writer, ctr, level+1);
		        }
			} catch (Exception e) {
			}
		}
