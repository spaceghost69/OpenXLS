...
extends XLSCellRecord {
    ...
    */
    public int getIntVal(){
        if(this.getBooleanVal())return 1;
        return 0;
    }
    ...
    */
    ...
    public String getErrorCode(){
		...
		String retval = "";
		if ((errorval & 0x0) == 0x0) retval = "#NULL!";
		if ((errorval & 0x7) == 0x7) retval = "#DIV/0!";
		if ((errorval & 0xF) == 0xF) retval = "#VALUE!";
		if ((errorval & 0x17) == 0x17) retval = "#REF!";
		if ((errorval & 0x1D) == 0x1D) retval = "#NAME?";
		if ((errorval & 0x24) == 0x24) retval = "#NUM!";
		if ((errorval & 0x2A) == 0x2A) retval = "#N/A";
    	return retval;
    }
    ...
    */
    ...
    public void setBooleanVal(boolean newv){
        if(newv)this.getData()[6] = 1;
        else this.getData()[6] = 0;
        this.val = newv;
    }
