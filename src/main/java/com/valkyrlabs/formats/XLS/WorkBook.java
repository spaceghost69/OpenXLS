...
import com.valkyrlabs.formats.XLS.charts.Ai;
...
public class WorkBook implements Serializable, XLSConstants, Book {
	...
	void initImages() {
		...
		for (int i = 0; i < msodg.getMsodrawingrecs().size(); i++) { // 20070914
																		// KSC:
																		// store
																		// msodrawingrecs
																		// with
			...
			if (imageData != null) {
				...
				im.setShapeName(rec.getShapeName()); // set shape name as well
														// ...
				im.setImageType(msodg.getImageType(imgdx));// 20100519 KSC:
															// added!
	...
	public void mergeMSODrawingRecords() {
		...
		if (msodg != null) {
			...
			for (int i = 1; i < msodgMerge.size(); i++) {
				msodg.mergeRecords((MSODrawingGroup) msodgMerge.get(i)); // merges
																			// and
																			// removes
																			// secondary
																			// msodrawinggroups
				...
				this.getStreamer()
						.removeRecord((MSODrawingGroup) msodgMerge.get(i)); // remove
																			// existing
																			// continues
																			// from
																			// stream
	 ...
	 * <br>
	 * must sum up all other mso's on the particular sheet SPCONTAINERLEN and update
	 * the sheet mso header ...
	 * <br>
	 * this calculation is quite experimental, so far it's working in all known
	 * cases ...
	 * 
	 * @param bs
	 */
	...
	public String getStats(boolean usehtml) {
		...
		String ret = "-------------------------------------------" + rex;
		ret += "OpenXLS Version:     " + WorkBookHandle.getVersion() + rex;
	...
	/**
	 * returns the list of OOXML objects which are external or auxillary to the main
	 * workbook
	 ...
	 * key, path, local path + filename [, rid, [extra info], [embedded
	 * information]]
	 * e.g. theme, doc properties
	 * 
	 * @param o
	...
	/**
	 * return the External Supbook record associated with the desired
	 * externalWorkbook
	 * will create if bCreate
	 * 
	 * @param externalWorkbook String URL (name) of External Workbook
	 * @param bCreate          if true, will create an external SUPBOOK record for
	 *                         the externalWorkbook
	 * @return Supbook
	 */
	public Supbook getExternalSupbook(String externalWorkbook, boolean bCreate) {
		...
		for (int i = 0; i < this.supBooks.size(); i++) {
			if (((Supbook) this.supBooks.get(i)).isExternalRecord())
				if (externalWorkbook
						.equalsIgnoreCase(((Supbook) this.supBooks.get(i))
								.getExternalWorkBook()))
					sb = (Supbook) this.supBooks.get(i);
		}
		if (sb == null && bCreate) { // create
			sb = (Supbook) Supbook.getExternalPrototype(externalWorkbook);
			int loc = ((Supbook) supBooks.get(supBooks.size() - 1))
					.getRecordIndex(); // must have at least one global supbook
										// present
	...
	public int getSupbookIndex(Supbook sb) {
		for (int i = 0; i < this.supBooks.size(); i++) {
			if (((Supbook) this.supBooks.get(i)) == sb)
				return i;
		}
		return -1;
	}
	...
	protected void addPivotTable(Sxview sx) {
		this.ptViews.put(sx.getTableName(), sx); // Pivot Table View ==Top-level
	 ...
	 * <br>
	 * SxView is the top-level record of a Pivot Table
	...
	public WorkBook() {
		...
		if (cm != null) {
			try {
				...
				Logger.logWarn("Invalid Calc Mode Setting in System properties:"
						+ cm);
			}
		}
		if (System.getProperties()
				.get("com.valkyrlabs.OpenXLS.sharedupes") != null) {
			this.sharedupes = System.getProperties()
					.get("com.valkyrlabs.OpenXLS.sharedupes").equals("true");
			if (this.sharedupes) {
	...
	private void initBuiltinFormats() {
		...
		for (int i = 0; i < formats.length; i++)
			formatlookup.put(formats[i][0].toUpperCase(), Short
	...
	public int addName(Name n) {
		if (n.getItab() != 0) {
			...
			try {
				Boundsheet b = this.getWorkSheetByNumber(n.getItab() - 1);// one
																			// based
																			// pointer
				b.addLocalName(n);
				n.setSheet(b);
			...
			if (existo != null) { // handle duplicate named ranges
				String bnam = n.toString();
				if (bnam.indexOf("Built-in:") != 0) {
					try {
						if (((Name) existo).getLocation() != null) // use
																	// original
																	// - as good
																	// a guess
																	// as any
		...
		if ((myexternsheet != null) && (n.getExternsheet() == null)) {
			try {
				...
				Logger.logWarn("WorkBookHandle.addName() setting Externsheet failed for new Name: "
						+ e.toString());
			}
		}
		return names.size() - 1;
	 ...
	 * 
	 *         Why are we calling getExternalName one based, then removing for
	 *         ordinal, internal processes should always
	 *         be 0,1,2,3... -NR 1/06
	 */
	...
	public void addDefaultExternsheet() {
		...
		supBooks.add(sbb);
		Externsheet ex = (Externsheet) Externsheet
				.getPrototype(0x0000, 0x0000, this);
	 ...
	 * 
	 * This method will first attempt to look in the book names, then the sheet
	 * names,
	 * obviously different scoped names can have the same identifying name, so this
	 * could return
	 ...
	 * This is needed to support formula creation with non-existent names
	 * referenced.
	 *
	 ...
	 * 
	 * NOTE: this doesn't actually get a "matching" font, it has to be the exact
	 * font.
	 * 20070826 KSC: changed to match font characterstics, not just return exact
	 * matching font
	 */
	public int getFontIdx(Font f) {
		// 20070819 KSC: Try this to see if better! Matches 6 key
		// attributes (size, name, color, etc.)
		for (int i = fonts.size() - 1; i >= 0; i--) { // start from the back so
	 ...
	 * 
	 * @return
	 *         KSC: is this method necessary now with above getFontIdx changes?
	 */
	 ...
	 * For some reason, the addFont only puts it into an array that is never
	 * accessed
	...
	/**
	 * Get the font at the specified index. Note that the number 4 does not exist,
	 * so index correctly based of that.
	 * 
	 * So, if you call getFont(5), you are really doing getFont(4) from the internal
	 * array
	 * 
	 * @param t
	 * @return
		...
		if (this.fonts.size() >= t) {
			if (t >= fonts.size()) {
				Logger.logWarn("font " + t
						+ " not found. Workbook contains only: " + fonts.size()
						+ " defined fonts.");
	...
	public BiffRec addRecord(BiffRec rec, boolean addtorec) {
		...
		if (rec instanceof Bof) {
			...
			if (bofct == eofct) { // not a chart or other non Sheet Bof
		...
		if (opc == XLSRecord.DBCELL || opc == XLSRecord.ROW
				...
				/* || opc==MULBLANK */
				|| opc == CHART || opc == XLSRecord.FILEPASS
		...
		switch (opc) {
			...
			case CF:
				Condfmt cfmt = (Condfmt) bs.getConditionalFormats()
						.get(bs.getConditionalFormats().size() - 1);
				cfmt.addRule((Cf) rec);
				break;
			...
			case INDEX:
				...
				if (bs == null) {
					Logger.logWarn("ERROR: WorkBook.addRecord( Index ) error: BAD LBPLYPOS.  The wrong LB:"
							+ lbplypos);
					try {
			...
			case ARRAY:
				...
				arr.setParentRec(lastFormula); // [BugTracker 1869] link array
			...
			case SUPBOOK: // KSC: must store ordinal positions of SupBooks, for
							// adding Externsheets
				supBooks.add(rec);
				if (myADDINSUPBOOK == null) { // see if this is the ADD-IN SUPBOOK
												// rec
			...
			case BOUNDSHEET:
				...
				if (win1 != null) {// Windows1 record is optional 20101004
			...
			case SXVIEW:
				addPivotTable(((Sxview) rec)); // Pivot Table View ==Top-level
												// record for a Pivot Table
				break;
			...
			case QSISXTAG:
				try {
					Sxview sx = (Sxview) this.ptViews.values()
							.toArray()[this.ptViews.size() - 1];
					sx.addSubrecord(rec);
				} catch (Exception e) {
					;
				}
			...
			case MSODRAWING:
				...
				if (msodg != null)
					...
					;
				// do what???com.valkyrlabs.toolkit.Logger.log("PROBLEM with
				// MSODG!");
				break;
	 ...
	 */
	public BiffRec getCell(String cellname) throws CellNotFoundException, SheetNotFoundException {
	 ...
	 */
	public BiffRec getCell(String sheetname, String cellname) throws CellNotFoundException, SheetNotFoundException {
		...
		try {
			...
			throw new SheetNotFoundException(sheetname + " not found");
	...
	private void addWorkSheet(Long lbplypos, Boundsheet sheet) {
		if (sheet == null) {
			...
			if (DEBUGLEVEL > 10)
				Logger.logInfo("Workbook Adding Sheet: " + sheet.getSheetName()
	 ...
	 * records
	 * that are bound to this book, adding them here will not add them to the
	 * workbook;
	 * 
	 * @param bookNameRecs
	 */
	 ...
	 * Note this is not that primary repository for names, it just contains the name
	 * records
	 * that are bound to this book, removing them here will not remove them
	...
	public void removeWorkSheet(Boundsheet sheet) {
		...
		for (int i = 0; i < namesOnSheet.length; i++)
	 ...
	 */
	public Boundsheet getWorkSheetByName(String bstr) throws SheetNotFoundException {
		try {
			...
			Logger.logWarn("WorkBook.getWorkSheetByName failed: "
					+ ex.toString());
		}
		throw new SheetNotFoundException(
				"Worksheet " + bstr + " not found in " + this.toString());
	}
	 ...
	 */
	public Boundsheet getWorkSheetByHash(String s) throws SheetNotFoundException {
		Boundsheet[] bs = this.getWorkSheets();
		for (int i = 0; i < bs.length; i++) {
			if (bs[i].getSheetHash().equalsIgnoreCase(s))
				return bs[i];
		}
		return null;
	}
	 ...
	 */
	public Boundsheet getWorkSheetByNumber(int i) throws SheetNotFoundException {
		...
		;
		if (bs == null) { // External Sheet Ref NOT FOUND
			throw new SheetNotFoundException(
					String.valueOf(i) + " not found");
		}
		return bs;
	}
	...
	/**
	 * get a handle to the first BOF to perform offset functions which don't know
	 * where the
	...
	public void updateFormatCache(Xf xf) {
		if (xf.tableidx != -1) { // if this xf has been already added to the
									// workbook
			if (formatCache.containsValue(xf)) { // xf signature has
													// changed/it's been updated
				Iterator ii = formatCache.keySet().iterator(); // remove and
																// update below
				while (ii.hasNext()) {
	 ...
	 * 
	 * If a name exists that refers directly to this sheet then duplicate it,
	 * otherwise workbook scoped names are
	 * not copied
	 */
	...
	/**
	 * Inserts a newly created Name record into the correct location in the
	 * streamer.
	 * 
	 * 
	 * @param n
	...
	public Chart addChart(Chart destChart, String NewChartName, Boundsheet boundsht) {
		...
		for (int x = 0; x < recs.size(); x++) {
			XLSRecord rec = (XLSRecord) recs.get(x);
			rec.setWorkBook(this);
	...
	/**
	 * updates Mso (MSODrawingGroup + Msodrawing) records upon add/copy worksheet
	 * and add/copy charts
	...
	public void addChartUpdateMsodg(MSODrawing mso, Boundsheet sht) {
		...
		if (hdr != null && hdr != mso) { // already have a header rec
	 ...
	 * NOTE: STILL EXPERIMENTAL TESTS OK IN BASIC CIRCUMSTANCES BUT MUST BE TESTED
	 * FURTHER
	 */
	 ...
	 * @param origWorkBookName original WorkBook Name (nec. for resolving possible
	 *                         external references) *
	 * @throws ClassNotFoundException
	 * @throws IOException
	 * @boolean SSTPopulatedBoundsheet - a boundsheet that has all of it's sst data
	 *          saved off in LabelSST records.
	 *          one would use this when moving a sheet from a workbook that was not
	 *          an original or created from getNoSheetWorkBook.
	 *          Do not use this if the data already exists in the SST, you are just
	 *          causing bloat!
	 */
	public void addBoundsheet(byte[] inbytes, String origSheetName, String NewSheetName, String origWorkBookName,
	 ...
	 * @param origWorkBookName original WorkBook Name (nec. for resolving possible
	 *                         external references) *
	 * @boolean SSTPopulatedBoundsheet - the boundsheet has all of it's sst data
	 *          saved off in LabelSST records.
	 *          one would use this when moving a sheet from a workbook that was not
	 *          an original or created from getNoSheetWorkBook.
	 *          Do not use this if the data already exists in the SST, you are just
	 *          causing bloat!
	 */
	public void addBoundsheet(Boundsheet bound, String origSheetName, String newSheetName, String origWorkBookName,
		...
		XLSRecord x = null;
		if (lastbound != null) {
			try { // lastbound must be reset because other operations could
					// alter
				lastbound = this
						.getWorkSheetByNumber(this.getNumWorkSheets() - 1);
				x = (XLSRecord) lastbound.getSheetRecs()
						.get(lastbound.getSheetRecs().size() - 1);
			} catch (Exception e) {
				;
			}
		...
		try {
			...
			Logger.logWarn("Adding new sheetRef failed in addBoundsheet()"
					+ e.toString());
		}
		...
		for (int i = 0; i < chts.size(); i++) {
			Chart chart = (Chart) chts.get(i); // obviously algorithm has
		...
		/**********
		 * This loop handles Boundsheet records contained in the sheet level streamer,
		 * that is, not the valrecs
		 *****/
		...
		for (int z = 1; z < numrecs; z++) {
			...
			if (DEBUGLEVEL > 5)
				try {
					Logger.logInfo("Copying: " + xl.toString() + ":"
							+ String.valueOf(newoffset) + ":" + xl.getLength());
				} catch (Exception e) {
					;
				}
			if (xl instanceof Codename) {
			...
			} else if (xl instanceof Cf) { // must check Conditional Format
											// formula refs and handle any
											// external references
				try {
					updateFormulaPtgRefs(((Cf) xl)
							.getFormula1(), origSheetName, newSheetName, origWorkBookName);
					// NOTE: FORMULA2 can be null -- TODO: should check here
					updateFormulaPtgRefs(((Cf) xl)
							.getFormula2(), origSheetName, newSheetName, origWorkBookName);
				} catch (Exception e) {
				}
			} else if (xl.getOpcode() == OBJ) {
				((Obj) xl).setObjId(bound.lastObjId++);
			} else if (xl.getOpcode() == MSODRAWING
					|| (xl.getOpcode() == CONTINUE
							&& ((Continue) xl).maskedMso != null)) { // 20100510
																		// KSC:
																		// handle
																		// masked
																		// mso's
				...
				if (mso.getImageIndex() > 0) { // add image bytes as well, if
												// any
					ImageHandle im = bound
							.getImageByMsoIndex(mso.getImageIndex());
					int idx = msodg.addImage(im.getImageBytes(), im
							.getImageType(), false);
					bound.imageMap.put(im, Integer.valueOf(idx)); // 20100518
																	// KSC:
																	// makes
																	// more
																	// sense?
		...
		if (msodg != null) {// Moved from above so don't udpate at every mso
							// addition
		...
		if (SSTPopulatedBoundsheet) {
			...
			for (int i = 0; i < b.length; i++) {
				b[i].setWorkBook(this);
				if (b[i].getOpcode() == XLSConstants.LABELSST) {
					Labelsst s = (Labelsst) b[i];
		...
		if (DEBUGLEVEL > 5)
			Logger.logInfo("changesize for  new boundsheet: "
					+ bound.getSheetName() + ": " + tout);
		copying = false;
	}

	/**
	 * traverses all rows and their associated cells in the newly transfered sheet,
	 * ensuring formula/cell references and format references are correctly
	 * transfered
	...
	private void updateTransferedCellReferences(Boundsheet bound, String origSheetName, String origWorkBookName) {
		...
		List boundFonts = bound.getTransferFonts(); // ALL fonts in the source
													// workbook
		...
		for (int i = 0; i < rows.length; i++) {
			...
			if (rows[i].getIxfe() != this.getDefaultIxfe())
				transferFormatRecs(rows[i], localFonts, boundFonts, localXfs, boundXfs); // 20080709
																							...
																							// for
																							// row
			Iterator rowcells = rows[i].getCells().iterator();
			Mulblank aMul = null;
			short c = 0;
			while (rowcells.hasNext()) {
				...
				b.setWorkBook(this); // Moved to before updateFormulaPtgRefs
										// [BugTracker 1434]
				if (b instanceof Formula) { // Examine Ptg Refs to handle
											// external sheet references not
											// contained in this workbook
					updateFormulaPtgRefs((Formula) b, origSheetName, bound
							.getSheetName(), origWorkBookName);
		...
		for (int i = 0; i < c.size(); i++) {
			Chart cht = (Chart) c.get(i);
			ArrayList fontrefs = cht.getFontxRecs();
			for (int x = 0; x < fontrefs.size(); x++) {
				Fontx fontx = (Fontx) fontrefs.get(x);
	...
	/**
	 * examine all Ptg's referenced by this formula, looking for hanging or missing
	 * sheet references
	...
	private void updateFormulaPtgRefs(Formula f, String origSheetName, String newSheetName, String origWorkBookName) {
		try {
			...
			for (int k = 0; k < p.length; k++) {
				if (p[k] instanceof PtgRef) {
					PtgRef ptg = (PtgRef) p[k];
					try {
						if (!(ptg instanceof PtgArea3d) || ((PtgArea3d) ptg)
								.getFirstSheet()
								.equals(((PtgArea3d) ptg).getLastSheet())) {
							 ...
							 * ptg.setSheetName(newSheetName);
							 */
						} else { // uncommon case of two sheet range
							...
							ptg.setLocation(ptg.toString()); // reset ixti if
																// nec.
						}
					} catch (SheetNotFoundException e) {
						Logger.logWarn(
								"External Reference encountered upon updating formula references:  Worksheet Reference Found: "
										+ ptg.getSheetName());
						ptg.setExternalReference(origWorkBookName);
					}
				} else if (p[k] instanceof PtgExp) {
					PtgExp ptgexp = (PtgExp) p[k];
					try {
						Ptg[] pe = ptgexp.getConvertedExpression(); // will fail
																	// if
																	// ShrFmla
																	// hasn't
																	// been
						...
						for (int j = 0; j < pe.length; j++) {
							if (pe[j] instanceof PtgRef) {
								PtgRef ptg = (PtgRef) pe[j];
								try {
									if (ptg instanceof PtgArea3d) { // PtgRef3d,
																	// etc.
										this.getWorkSheetByName(ptg
												.getSheetName());
										ptg.setLocation(ptg.toString()); // reset
																			// ixti
																			// if
																			// nec.
									}
									...
									Logger.logWarn(
											"External References Not Supported:  UpdateFormulaReferences: External Worksheet Reference Found: "
													+ ptg.getSheetName());
									ptg.setExternalReference(origWorkBookName);
								}
							}
			...
			Logger.logErr("WorkBook.updateFormulaRefs: error parsing expression: "
					+ e);
		}
	}

	/**
	 * given a record in an previously external workbook, ensure that xf and font
	 * records
	 * are correctly input into the current workbook and that the pointers are
	 * correctly updated
	 * 
	 * @param b          BiffRec
	 * @param localFonts HashMap of string version of all fonts, font nums in
	 *                   current workbook
	 * @param boundFonts List of string version of all fonts, font nums in external
	 *                   workbook
	 * @param localXfs   HashMap of string version of all xfs, xf nums in current
	 *                   workbook
	 * @param boundXfs   List of string version of all xfs, xf nums in external
	 *                   workbook
	 */
	private void transferFormatRecs(BiffRec b, HashMap<String, Integer> localFonts, List boundFonts,
			HashMap<String, Integer> localXfs, List boundXfs) {
	 ...
	 *                   workbook
	 */
	private int transferFormatRecs(int oldXfNum, HashMap<String, Integer> localFonts, List boundFonts,
			HashMap<String, Integer> localXfs, List boundXfs) {
		int localNum = -1;
		if (boundXfs.size() > oldXfNum) {// if haven't populatedForTransfer i.e.
											// haven't opted to transfer formats
											// ...
			Xf origxf = (Xf) boundXfs.get(oldXfNum); // clone xf so modifcations
			...
			if (origxf != null) {
				...
				/** XF **/
				Xf localxf = FormatHandle
						.cloneXf(origxf, origxf.getFont(), this); // clone xf so
																	// modifcations
																	// don't
																	// affect
																	// original
				...
				/** FORMAT **/
				Format fmt = origxf.getFormat(); // number format - is null if
													// format is general ...
				if (fmt != null) // add if necessary
					localxf.setFormatPattern(fmt.getFormat()); // adds new
																// format
																// pattern if
																// not found
				...
				if (xfNum == null) { // insert it into the book
	...
	public int addPivotStream(String ref, String sheetName, int sid) {
		...
		for (int i = records.size() - 1; i > 0 && z == -1; i--) {
			...
			if (opcode == SXADDL) { // find last cache id and increment
				/*
		...
		TableStyles tx = (TableStyles) TableStyles.getPrototype(); // see if
																	// this is
																	// really
																	// necessary
																	// ...
	...
	public void addPivotCache(String ref, WorkBookHandle wbh, int sId) {
		if (ptcache == null) {
			...
			ptcache.createPivotCache(factory.myLEO
					.getDirectoryArray(), wbh, ref, sId);
		}
	}
	...
	public SxStreamID getPivotStream(int cacheid) {
		// int z= 0;
		for (int i = 0; i < ptstream.size(); i++) {
			int sid = ((SxStreamID) ptstream.get(i)).getStreamID();
			if (sid == cacheid)
				// if (z++==cacheid)
				return (SxStreamID) ptstream.get(i);
		}
		/*
		 * List records= this.getStreamer().records;
	...
	/**
	 * Initialize the indirect functions in this workbook by calculating the
	 * formulas
	 * 
	 */
	public void initializeIndirectFormulas() {
		Iterator i = indirectFormulas.iterator(); // contains all INDIRECT
													// funcvars + params
		while (i.hasNext()) {
	...
	public void addExternsheet() {
		if (myexternsheet == null) {
			...
			try {
				...
				Logger.logWarn("WorkBook.addExternSheet() locating Sheet for adding Externsheet failed: "
						+ e);
			}
	...
	/**
	 * Returns a map of xml strings representing the XF's in this workbook/Integer
	 * of lookup.
	 * These are used as a comparitor to determine if additional xf's need to be
	 * brought in or
	 * not and to give the new XF number if the xf exists.
	 * 
	 * Changed 20080226 KSC: to use XF toString as XML is limited in format,
	 * toString is more complete
	 * 
	 ...
	 * SST contains
	 * all standard string records in cells, but may not include such things as
	 * strings that are contained
	 * within formulas. This is useful for such things as full text indexing of
	 * workbooks
	 * 
	...
	public void closeSheets() {
		for (int i = boundsheets.size() - 1; i > 0; i--) {
			...
			if (b.streamer != null) { // do separately because may call
										// boundsheet close
				b.streamer.close();
				b.streamer = null;
			}
		...
		if (resetVars) { // just clearing out sheets instead of closing workbook
		...
		if (resetVars) { // just clearing out sheets instead of closing workbook
			if (((BiffRec) recs[0]).getOpcode() == BOF) {// should!!
				lastBOF = (Bof) recs[0];
				firstBOF = (Bof) recs[0];
			}
		}
	...
	public void close() {
		...
		if (this.isExcel2007) {
			try {
				String externalDir = OOXMLAdapter
						.getTempDir(this.getFactory().getFileName());
