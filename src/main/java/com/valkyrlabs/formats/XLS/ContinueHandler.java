...
public class ContinueHandler implements Serializable, XLSConstants{	
    ...
    */
    public void addRec(BiffRec rec,int datalen){
        ...
        if(nextOpcode == CONTINUE){
            if(DEBUGLEVEL > 11)Logger.logInfo("Next OPCODE IS CONTINUE: " + Integer.toHexString(nextOpcode));
        }
        if(nextOpcode == CONTINUE && opcode != CONTINUE){ // the continued rec
            if(continued != null){
                continued.init();                 
                continued = null;
            }
			...
			lastCont = null;
        }else if(opcode == CONTINUE){ // add to the continued rec
            splitContRec = rec;
            rec.init();
            if (continued == null && lastCont == null) {
                ...
                if (DEBUGLEVEL > 0)Logger.logWarn("Warning:  Out of spec split txo continue record found, reconstructing." + splitPrevRec.toString());
                
            }else {
                if(lastCont!=null)((Continue)rec).setPredecessor(lastCont);
                else ((Continue)rec).setPredecessor(continued);
                if((continued.getOpcode()==SST)||(continued.getOpcode()==STRINGREC)){
                    if(DEBUGLEVEL > 2)Logger.logInfo("Sst Continue.  grbit:" + ((Continue)rec).getGrbit());
                }else{
            ...
            if(handleTxo){
            	...
            	if (lastTxo.text==null) {	 
                	...
                	try {	 
                		...
                		if (DEBUGLEVEL > 0) Logger.logErr("ContinueHandler.txo parsing- encountered unknown Continue record");
                	}
            	...
            	try {	// When a Continue record follows an Obj record, it is either masking a Msodrawing record - or it's a "big rec" continues 
            		...
            		if (DEBUGLEVEL > 0) Logger.logErr("ContinueHandler.Obj parsing- encountered unknown Continue record");
            	}
            }else {
	...
	*/
	public static int createSstContinues(Sst rec, int insertLoc){ 
	   ...
	   // int sstoffset =  continuesizes[0].intValue() - rec.getOrigSstLen();
	    ...
	    for(int i=1;i<=numconts;i++){ // start after the 1st continue length which is the Sst data body
	        if(continuesizes[i].intValue()==0)break;
	        ...
	        if(thisgr != null)hasGrbit = ((thisgr.byteValue() < 0x2)&&(thisgr.byteValue() >= 0x0)); // Sst grbit is either 0h or 1h, otherwise it's String data
	        if(continuesizes[i].intValue()==MAXRECLEN)hasGrbit = false; // this is a non-standard Sst Continue
			...
			if(hasGrbit)thiscont.setHasGrbit(true);
			insertLoc++;
		}
    ...
    */
    ...
    public static int createBigContinues(BiffRec rec, int insertLoc){            
        ...
        for(int i=0;i<numconts;i++){
            // if this is the last Continue rec it is probably shorter than CONTINUESIZE
            if(datalen - boundaries[i] < MAXRECLEN)sizer = (datalen - boundaries[i]);
            byte[] continuedata = new byte[sizer];
            System.arraycopy(dta, boundaries[i], continuedata, 0, continuedata.length);
