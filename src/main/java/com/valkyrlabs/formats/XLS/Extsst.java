...
public final class Extsst extends com.valkyrlabs.formats.XLS.XLSRecord 
{
    /** 
	* serialVersionUID
	*/
    ...
    */
    public boolean getUpdatesAllBOFPositions(){return true;}
   ...
   public  void updateIsstinfs(){
        ...
        if(totstrs > 1024){
            newdsst = totstrs / 128;
            if((totstrs % newdsst)>0)newdsst++;            
        }
        int totissts = totstrs / newdsst;
        if((totstrs % newdsst)>0)totissts++;
        int ctr = 0;
       ...
       int sstOffset = mysst.getOffset();
        for(int t=0;t<totissts;t++){
           ...
           sstpos = str.getSSTPos() +4; // always 4 off
            ...
            if(continueStrPos > RecLen && (whichContinue +1) < mysst.continues.size()) { // new bucket
                updateRecLen = true;
        ...
        if(DEBUGLEVEL > 3)Logger.logInfo("Done creating Isstinfs");    
    }
