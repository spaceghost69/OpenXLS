...
public final class Name extends XLSRecord {
    ...
    /*
     * this byte array differs from the prototype in that it has no description
     * field. It is
     * used by formulas inserting names that don't exist within the workbook. The
     * name will
     * show up in the formula, but nowhere else on the excel file
     **/
    private final byte[] FILLER_NAME_BYTES = { 0x0, 0x0, 0x0, 0x1, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
            0x0, 0x0 };
    // 20090811 KSC: this prototype contains a range; clear out private byte[]
    // PROTOTYPE_NAME_BYTES =
    // {0x0,0x0,0x0,0x6,0xb,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x61,0x64,0x66,0x61,0x64,0x66,0x3b,0x1,0x0,0x21,0x0,0x21,0x0,0x1,0x0,0x3,0x0};
    private final byte[] PROTOTYPE_NAME_BYTES = { 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
            0x0 };
    ...
    public Name(WorkBook bk, String namestr) {
        ...
        } catch (SheetNotFoundException x) {
    ...
    /**
     * Used for default name entry in formulas. The name will not exist in the
     * workbook outside
     ...
     * init Name record
     *
     * @param initExpression true if should parse formula/ref expression (will be
     *                       false on wb load)
     */
    ...
    /**
     * Return the name which identifies this Name record. If this is a built in
     * record
     * it will return a generic version of what the built-in is doing.
     *
     * @see com.valkyrlabs.formats.XLS.XLSRecord#toString()
     */
    ...
    /**
     * Return the location of the Name record. This seems to be slightly wrong as it
     * only
     * returns the location of PTGA. It's possible to have more complex records than
     * this.
     *
     * @return
     * @throws Exception
    ...
    /**
     * Essentially a wrapped setLocation call that is handled for initialization of
     * names
     * from OOXML files. Our parsing is not up to snuff for some more complex name
     * records,
    ...
    public void setLocation(String newloc, boolean clearAffectedCells)
        ...
        try {// can be a named value constant
            ...
            if (ptgs.size() == 1) { // usual case of 1 ref or 1 PtgMemFunc (complex expression)
                ptga = (Ptg) ptgs.pop();
                // KSC: memory usage changes: now parent rec nec. for reference tracking; if
                // change must update
        ...
        if (ptga instanceof PtgRef) { // ensure that references are absolute
            ...
            // clear affected cells so that formulas which reference the named range get
            // recalced with the new value(s)
            if (clearAffectedCells) { // will only be false when initializing an OOXML workbook, which does not need
                                      // to clear affected cells since it's initializing
                try {
    ...
    public void updateSheetRefs(String origWorkBookName) {
        ...
        if (ptga instanceof PtgArea3d) { // PtgRef3d,etc
            ...
            try {
                ...
                Logger.logWarn(
                        "External References Not Supported:  UpdateSheetReferences: External Worksheet Reference Found: "
                                + p.getSheetName());
        ...
        for (int i = 0; i < expression.size(); i++) {
            ...
            try {
                System.arraycopy(b, 0, updated, offset, b.length/*
                                                                 * 20071206 KSC: Not necessarily the same
                                                                 * ptg.getLength()
                                                                 */);
            } catch (Exception e) {
     ...
     * @throws SheetNotFoundException{
     */
    public void setNewScope(int newitab) throws SheetNotFoundException {
        if (this.itab == 0) {
