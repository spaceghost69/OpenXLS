 ...
 *
 */
public class Setup extends com.valkyrlabs.formats.XLS.XLSRecord{
	...
	public void init(){
    	...
    	// IEEE fp numbers
    	byte[] bx1 = {this.getByteAt(pos++),this.getByteAt(pos++),this.getByteAt(pos++),this.getByteAt(pos++),this.getByteAt(pos++),this.getByteAt(pos++),this.getByteAt(pos++),this.getByteAt(pos++)};
    	headerMargin		= ByteTools.eightBytetoLEDouble(bx1); //Header margin (IEEE number)
    	byte[] bx2 = {this.getByteAt(pos++),this.getByteAt(pos++),this.getByteAt(pos++),this.getByteAt(pos++),this.getByteAt(pos++),this.getByteAt(pos++),this.getByteAt(pos++),this.getByteAt(pos++)};    	
    	...
    	if ((grbit & 0x4)==0x4) {//	 Paper size, scaling factor, paper orientation (portrait/landscape), print resolution and number of copies are not initialised
    		scale= 100;
    		grbit |= 0x2;	// set orientation to portrait
    		// defaults for resolution?
    		// default for paper size
    		if ("AR AS BR BS BM BO CA CL CO CR CU DM DO EC GD GP GU GT GY HN HT JM KN KY LC MX NI PA PE PR PY SR SV TC US UM UY VC VE VI ".indexOf(Locale.getDefault().getCountry() + " ") == -1)
