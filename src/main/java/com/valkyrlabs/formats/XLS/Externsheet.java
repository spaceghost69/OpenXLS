...
public final class Externsheet extends com.valkyrlabs.formats.XLS.XLSRecord {
    /**
     * serialVersionUID
     */
    ...
    public void addPtgListener(IxtiListener p)
     ...
     * 
     * Also updates the ixti listeners, which appear to be records that contain an
     * internal
     ...
     * 
     */
    void addSheet(int firstSheet, int lastSheet)
            throws SheetNotFoundException {
        if (DEBUGLEVEL > 10)
    ...
    public short addExternalSheetRef(String externalWorkbook, String externalSheetName) {
        // get the external supbook record for this external workbook, creates if not
        // present
    ...
    /**
     * Insert location checks if a specific boundsheet range already has a
     * reference.
     * 
     * If the range already exists within the externsheet the index to the
     * range is returned. Else, it adds the range to the externsheet and returns the
     * index.
     * 
     * @param firstBound
     * @param lastBound
     * @return
     */
    public int insertLocation(int firstBound, int lastBound)
            throws SheetNotFoundException {
        Iterator it = rgs.iterator();
        int i = 0;
        while (it.hasNext()) {
    ...
    public int getXtiReference(String firstSheet, String secondSheet) {
        for (int i = 0; i < rgs.size(); i++) {
            rgxti thisXti = (rgxti) rgs.get(i);
            try {
                if (thisXti.getSheetName(thisXti.sheet1num).equalsIgnoreCase(firstSheet)
                        && thisXti.getSheetName(thisXti.sheet2num).equalsIgnoreCase(secondSheet)) {
                    return i;
                }
            } catch (SheetNotFoundException we) {
                if (DEBUGLEVEL > 10)
                    Logger.logWarn(
                            "Externsheet.getXtiReference:  Attempt to find Externsheet reference for sheet failed: "
                                    + we);
            }
        }
        return -1;
    ...
    /**
     * Certain records require a virtual reference, this is not a real reference to
     * a sheet,
     * rather an entry that is used by add in formulas, values are FE FF FE FF
     * 
     * This method either finds the existing reference, or creates a new one and
     * returns
     * the pointer
     * 
     * 
    ...
    public int getVirtualReference() {
        for (int i = 0; i < rgs.size(); i++) {
            rgxti thisXti = (rgxti) rgs.get(i);
            if (thisXti.sheet1num == (short) 0xFFFE &&
                    thisXti.sheet2num == (short) 0xFFFE)
                return i;
        }
        byte[] bts = new byte[6];
        if (this.wkbook != null) {
    ...
    /**
     * In some cases, we need to have an Xti reference to a non-existing sheet. For
     * instance, a chart
     * or formula with a ptgRef3d that referrs to a missing sheet. Externsheet
     * handles this by having
     * an internal record populated with -1's This method searches for a
     * non-existing record, and if
    ...
    public int getBrokenXtiReference() {
        for (int i = 0; i < rgs.size(); i++) {
            rgxti thisXti = (rgxti) rgs.get(i);
            if (thisXti.sheet1num/* getSheet1Num() */ == 0xFFFF &&
                    thisXti.sheet2num/* getSheet2Num() */ == 0xFFFF)
                return i;
        }
        byte[] bts = new byte[6];
        if (this.wkbook != null) {
    ...
    class rgxti implements java.io.Serializable {
         ...
         */
        public String getSheetName(int sheetNum) throws SheetNotFoundException {
