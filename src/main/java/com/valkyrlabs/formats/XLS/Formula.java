...
extends XLSCellRecord {
	...
	*/
	...
	public void init() {
		...
		super.init();
		if ((data=getData()) == null) throw new IllegalStateException(
   	...
   	private void clearExpression() {
		...
		if (this.hasAttachedString()) {	// remove that too
   		...
   		while (iter.hasNext()) {
   			...
   			if (ptg instanceof PtgRef) ( (PtgRef)ptg ).removeFromRefTracker();
   		}
    ...
    public void addInternalRecord (BiffRec b) {
    	if (internalRecords == null) internalRecords = new ArrayList( 3 );
        
        if (b instanceof Shrfmla) internalRecords.add( 0, b );
        else if (b instanceof StringRec) {
        	if (!haveStringRec || string==null){// should ONLY have 1 StringRec
    ...
    public List getInternalRecords() {
    	if (internalRecords == null) return Collections.emptyList();
    ...
    */
    ...
    public void setCalcAlways(boolean fAlwaysCalc) {
    	if (fAlwaysCalc) grbit |= FALWAYSCALC;
    	else grbit &= ~FALWAYSCALC;
    }
...
/**
    ...
    * */
    ...
    void populateExpression() {
        ...
        try {
        	short length = ByteTools.readShort(
        			this.getByteAt(20), this.getByteAt(21) );
        	
        	if (length + 22 > data.length) throw new Exception (
        			"cce longer than record");
        	
            expression = ExpressionParser.parseExpression(
            		this.getBytesAt (22, reclen - 22), this, length );
    ...
    void initSharedFormula (Shrfmla target) throws FormulaNotFoundException 
    {
    	...
    	if (expression.size() != 1
    			|| !(expression.get(0) instanceof PtgExp)) {
    	...
    	if (target != null) {
			...
			shared = ((Formula) getSheet().getCell( 
					pointer.getRwFirst(), pointer.getColFirst() ) ).shared;	// find shared cell linked to host/first formula cell in shared formula series
			if (shared == null) 
				throw new Exception();
		} catch (Exception e) {
			// If this is the host cell, fail silently. This method will be
			// re-called by the ShrFmla record's init method.
			if (this.getCellAddress().equals(
					pointer.getReferent() ))
				return;
			
			// Otherwise, complain and clear fShrFmla
			throw new FormulaNotFoundException (
            		"FORMULA at " + this.getCellAddress()
...
//					this.addInternalRecord( new StringRec( (String)cachedValue ) ); // will be added in workbook.addRecord
    ...
    public void preStream() {
    	...
    	if (cachedValue == null) {
    		grbit |= FCALCONLOAD;
    		if (writeValue == null) writeValue = new Double( Double.NaN );
    	}
    	...
    	for (int idx = 0; idx < expr.size(); idx++) {
    		...
    		if (ptg instanceof PtgArray) {
    			...
    			rgblen += extra.length;
    		} else if (ptg instanceof PtgMemArea) {	// has PtgExtraMem structure appended 
    	...
    	if (writeValue instanceof Number) {
    		...
    		if (writeValue instanceof String) {
    			if (!isErrorValue((String)writeValue)) {
	    			...
	    			if (sval.equals("") || string==null) {	// the latter can occur when input from XLSX; a cachedvalue is set without an associated StringRec
    			...
    			value[0] = (byte)0x01;
    			value[2] = ( ((Boolean)writeValue).booleanValue()
    					? (byte)0x01 : (byte)0x00);
    	...
    	for (int idx = 0; idx < ptgdata.length; idx++) {
    		System.arraycopy(
    				ptgdata[ idx ], 0, newdata, offset, ptgdata[ idx ].length);
    		offset += ptgdata[ idx ].length;
    	}
    	
    	// Expression Extra Data (rgb)
    	if (rgblen > 0) System.arraycopy( rgb, 0, newdata, offset, rgblen );
    	setData( newdata );
    	dirty = false;
    }
     ...
     * 
     * @see com.valkyrlabs.formats.XLS.XLSRecord#getIntVal()
     */
     ...
     * 
     * Note this is for the standards that were determined with excel and com.valkyrlabs.OpenXLS
	 ...
	 */
	public Object calculateFormula() 
	...
	public Object calculate()
	{
		...
		try{
			...
			if (depth>WorkBookHandle.RECURSION_LEVELS_ALLOWED){
	        	...
	        	cachedValue = new CalculationException( 
	...
	private void setSharedFormula (boolean isSharedFormula) {
		if (isSharedFormula) grbit |= FSHRFMLA;
		else grbit &= ~FSHRFMLA;
	}
...
/*		if (expression.get(0) instanceof PtgExp) {
     ...
     * 
     * @see com.valkyrlabs.formats.XLS.XLSRecord#setCachedValue(java.lang.Object)
     */
...
//         this.updateRecord(); no need; will be updated after recalc, which will automatically happen on write
				...
				if (p.getIsReference()) {
					if (!((((PtgRef)p).wholeRow && colInc!=0) || (((PtgRef)p).wholeCol && rowInc!=0))) { 
						if (!(p instanceof PtgArea)) {
							...
							int[] rc= ExcelTools.getRangeRowCol(s);
							boolean[] bRelRefs= { ((PtgArea) p).getFirstPtg().isRowRel(), ((PtgArea) p).getLastPtg().isRowRel(), ((PtgArea) p).getFirstPtg().isColRel(), ((PtgArea) p).getLastPtg().isColRel() }; 
							if (bRelRefs[0]) 
								rc[0]+=rowInc;					
							if (bRelRefs[1]) 
								rc[2]+=rowInc;							
    ...
    public static boolean isErrorValue(String s) {
        ...
        return (Collections.binarySearch(Arrays.asList(new String[] {"#DIV/0!", "#N/A", "#NAME?", "#NULL!", "#NUM!", "#REF!", "#VALUE!"}), s.trim())>-1);     	
    }
