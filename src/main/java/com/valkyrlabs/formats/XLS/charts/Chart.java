    ...
    public boolean addInitialChartRecord(BiffRec br) {
        ...
        if (hierarchyDepth != 0) {
            if (br.getOpcode() == AI) {
                ...
                chartseries.add(new Object[]{br, Integer.valueOf(nCharts)});    // store series records - initially map all to default chart (normal case)
            } else if (br.getOpcode() == AXISPARENT) {
                chartaxes = new ChartAxes((AxisParent) br);
            } else if (br.getOpcode() == AXIS) {
                chartaxes.add((Axis) br);
            } else if (br.getOpcode() == CHARTFORMAT) {    // usually only 1 (default chart id= 0) but can have up to 9 overlay charts
                nCharts++;
            } else if (br.getOpcode() == SERIESLIST) { // maps chart # to series #
                chartseries.addSeriesMapping(nCharts - 1, ((SeriesList) br).getSeriesMappings());// 1-based series #'s to "assign" to the overlay chart
            }
            try {
                if (((GenericChartObject) br).chartType != -1) { // store the chart object which defines this chart (NOTE: may be up to 4 charts in one = overlay charts)
                    ChartFormat cf = null;
...
// sadly, can't do it here - have to wait until process hierarchy chartobj.add(ChartType.getChartObject((GenericChartObject)br, cf, this.getWorkBook()));
            ...
            if (br.getOpcode() == DIMENSIONS) dimensions = (Dimensions) br;
            br.getData();
            postRecs.add(br);
        }
    ...
    public void initChartRecords() {
        try {    // Added to find/set obj + msodrawing records - replaces setObj
    ...
    private ChartObject initChartObject(ChartObject cobj, BiffRec[] cRecs) {
        ...
        for (int i = 0; i < cRecs.length; i++) {
            ...
            if (!(b.getOpcode() == BEGIN) && !(b.getOpcode() == END)) {
                if ((cRecs.length > i + 1) && ((cRecs[i + 1]).getOpcode() == BEGIN)) { // this is an object with sub-data
                    try {
...
//                            	Logger.logInfo("Display Units");
    ...
    public List assembleChartRecords() {
        ...
        if (dirtyflag) {
            for (int i = 0; i < nCharts; i++) {
                chartseries.updateSeriesMappings(chartgroup.get(i).getSeriesList(), i);    // if has multiple or overlay charts, update series mappings
            }
    ...
    public Series addSeries(String seriesRange, String categoryRange, String bubbleRange, String legendRange, String legendText, int nChart) {
        Series s = chartseries.addSeries(seriesRange, categoryRange, bubbleRange, legendRange, legendText, chartgroup.get(nChart), nChart);
        setMetricsDirty();
        return s;
    }
    ...
    public void removeSeries(Series seriestodelete) {
        ...
        for (int i = 0; i < chartArr.size(); i++) {
            ...
            if (b.getOpcode() == SERIES) {
                nSeries++;
                if (seriestodelete.equals(b)) {    // this is the one to delete
                    chartArr.remove(i);
    ...
    public void setTitle(String str) {
        ...
        if ((str == null || str.equals("")) && charttitle == null)    // if no title and setting title to "", just leave it
            return;
        if (charttitle == null) { // 20070709 KSC: Adding a new chart, add Title recs ...
            try {
    ...
    public Font getDefaultFont() {
        for (int i = 0; i < chartArr.size(); i++) {
            if (((BiffRec) chartArr.get(i)).getOpcode() == DEFAULTTEXT) {
                if (((DefaultText) chartArr.get(i)).getType() == 2) {// correct code???? 2 or 3 ??
    ...
    public Font getTitleFont() {
        if (charttitle == null) return null;
    ...
    public void updateSheetRefs(String newSheetName, String origWorkBook) {
        ...
        while (i.hasNext()) {
            ...
            try {
                for (int j = 0; j < s.chartArr.size(); j++) {
                    if (((BiffRec) s.chartArr.get(j)).getOpcode() == AI) {
                        ((Ai) s.chartArr.get(j)).updateSheetRef(newSheetName, origWorkBook);    // 20080630 KSC: add sheet name for lookup
                    }
                }
            } catch (Exception e) {
...
*/
    ...
    public void setChartFont(String type, String val) {
        if (type.equalsIgnoreCase("Title"))
            for (int i = 0; i < chartArr.size(); i++) {
                BiffRec b = chartArr.get(i);
                if (b.getOpcode() == TEXTDISP) {    // should be the title
                    TextDisp td = (TextDisp) b;
                    if (td.isChartTitle()) {        // should!!
            ...
            for (int i = 0; i < chartArr.size(); i++) {
                ...
                if (b.getOpcode() == DEFAULTTEXT) {
                    if (((DefaultText) b).getType() == Integer.parseInt(type)) {
                        i++;
                        b = chartArr.get(i);
                        if (b.getOpcode() == TEXTDISP) {    // should be!!
                            TextDisp td = (TextDisp) b;
                            td.setFontId(Integer.parseInt(val));
                            break;
                        }
    ...
    public short[] getCoords() {
        ...
        if (!metricsDirty) {    // use adjusted values for w and h
    ...
    public HashMap getMetrics(WorkBookHandle wbh) {
        if (metricsDirty) {
            try {
                ...
                // Chart title offset
                com.valkyrlabs.formats.XLS.Font titlefont = this.getTitleFont();
                if (titlefont != null && !this.getTitle().equals("")) { // apparently can still have td even when no title is present ...
...
//KSC: TESTING!		
//com.valkyrlabs.toolkit.Logger.log("Original lcoords:  " + Arrays.toString(lcoords));					
                    } else {
                        lcoords = new int[6];
...
//com.valkyrlabs.toolkit.Logger.log("Before Adjustments:  x:" + chartMetrics.get("x") + " w:" + chartMetrics.get("w") + " cw:" + chartMetrics.get("canvasw") + " y:" + chartMetrics.get("y") + " h:" + chartMetrics.get("h") + " ch:" + chartMetrics.get("canvash"));				
                // now adjust plot area coordinates based on canvas w, h, title and label offsets, and legend box, if any
                chartMetrics.put("x", chartMetrics.get("x") + (Double) this.getAxes().axisMetrics.get("YAXISLABELOFFSET") + (Double) this.getAxes().axisMetrics.get("YAXISTITLEOFFSET"));
                ...
                chartMetrics.put("h", chartMetrics.get("canvash") - chartMetrics.get("y") - (Double) this.getAxes().axisMetrics.get("XAXISLABELOFFSET") - (Double) this.getAxes().axisMetrics.get("XAXISTITLEOFFSET") - 10);
//com.valkyrlabs.toolkit.Logger.log("After Adjustments:   x:" + chartMetrics.get("x") + " w:" + chartMetrics.get("w") + " cw:" + chartMetrics.get("canvasw") + " y:" + chartMetrics.get("y") + " h:" + chartMetrics.get("h") + " ch:" + chartMetrics.get("canvash"));				
                ...
                if (lcoords[5] == Legend.RIGHT) {
                    ...
                    if (legendBeg < 0 || legendEnd < 0) { // try to adjust
                        if (legendEnd < 0) {
...
//com.valkyrlabs.toolkit.Logger.log("Adjusted LCoords:  " + Arrays.toString(lcoords)); 				
