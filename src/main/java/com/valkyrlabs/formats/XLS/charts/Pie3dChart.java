...
public class Pie3dChart extends PieChart {
    ...
    public String getSVG(HashMap<String, Double> chartMetrics, HashMap<String, Object> axisMetrics, ChartSeries s) {
        ...
        if (n > 0) {
            ...
            double total = 0.0;
            for (int j = 0; j < oneseries.length; j++) {    // get total in order to calculate percentages
                total += oneseries[j];
            }
            if (dls.length == 1) {    // no series-specific data labels; expand to entire series for loop below
                int dl = dls[0];
            ...
            for (int j = 0; j < oneseries.length; j++) {
                ...
                if (y1 >= centery) {    // the lower portion of a 3d pie chart that makes it look 3d
                    ...
                    svg.append("<path " + getScript(curranges[j]) + "  fill='" + seriescolors[j] + "' fill-opacity='" + getFillOpacity() + "' style='filter:url(#multiply)' " + getStrokeSVG(2, getMediumColor()) + " path='' d='" + path + "' fill-rule='evenodd'/>\r\n");
                }
