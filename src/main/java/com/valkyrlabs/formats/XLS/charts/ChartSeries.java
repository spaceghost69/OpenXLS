...
import com.valkyrlabs.formats.OOXML.DLbls;
import com.valkyrlabs.formats.OOXML.DPt;
import com.valkyrlabs.formats.OOXML.Marker;
...
public class ChartSeries implements ChartConstants, Serializable {
    ...
    private final ArrayList<Object[]> series = new ArrayList(); // ALL series in chart MAPPED to chart that "owns" it
     ...
     * @return
     */
    public static Series parseOOXML(XmlPullParser xpp, WorkBookHandle wbh, ChartType parentChart,
            boolean hasPivotTableSource, Stack<String> lastTag) {
        try {
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                if (eventType == XmlPullParser.START_TAG) {
                    ...
                    if (tnm.equals("ser")) { // Represents a single field in the PivotTable. This complex type contains
                    ...
                    } else if (tnm.equals("cat") || tnm.equals("xVal")) { // children: CHOICE OF: multiLvlStrRef,
                                                                          // numLiteral, numRef, strLit, strRef
                        idx = 1;
                    ...
                    } else if (tnm.equals("spPr")) { // series spPr
                        lastTag.push(tnm); // keep track of element hierarchy
                        ...
                        idx = 3;
                    } else if (tnm.equals("shape")) { // bar only
                        parentChart.convertShape(xpp.getAttributeValue(0));
                    } else if (tnm.equals("smooth")) { // line chart
                        ...
                        // NOTE: two types of values; 1- reference denoted by f element parents can be
                        // numRef, strRef or multiLvlStrRef
                        // 2- text value denoted by v element parent is strRef
                    } else if (tnm.equals("formatCode")) { // part of numCache element
                        Xf.addFormatPattern(wbh.getWorkBook(), OOXMLAdapter.getNextText(xpp));
                        // *******************************************
                        // TODO: add to y pattern ***
                        // *******************************************
                    } else if (tnm.equals("f")) { // range element -- legend cell, Cat range, Value range, Bubble data
                                                  // reference
                        ranges[idx] = OOXMLAdapter.getNextText(xpp);
                    } else if (tnm.equals("v")) { // value or text of series or category (parent=tx)
                        if (idx == 0) // legend text
                            legendText = OOXMLAdapter.getNextText(xpp); // legend text; possible to have legend text
                                                                        // without a legend cell range (ranges[1])
                        else if (idx == -1 || ranges[idx].equals("")) { // shoudln't!! can't have a textual refernce in
                                                                        // place of a series or cat value (can you?)
                            Logger.logWarn("ChartSeries.parseOOXML: unexpected text value");
                        }
                    } else if (tnm.equals("numCache") || tnm.equals("strCache") || tnm.equals("multiLvlStrRef")) { // parent=
                                                                                                                   // cat
                                                                                                                   // or
                                                                                                                   // vals
                                                                                                                   // (series
                                                                                                                   // values)
                        cache = tnm;
                    } else if (tnm.equals("ptCount")) { // parent= numCache or strCache (governs either f element)
                        if (hasPivotTableSource) { // OK, if have a pivot table source then the range referenced in f is
                                                   // only a SUBSET
                            // unclear if at any other time the range referenced is a subset ... [NOTE: in
                            // testing, only pivot charts hit]
                            ...
                            if (!ranges[idx].equals("") && ranges[idx].indexOf(",") == -1) {
                                try {
                                    ...
                                    if (cells.getCells().length != npoints) { // must adjust
                                        ...
                                        while (eventType != XmlPullParser.END_DOCUMENT) {
                                            if (eventType == XmlPullParser.START_TAG) {
                                                ...
                                                if (tnm.equals("pt")) {
                                                     ...
                                                     * Logger.
                                                     * logWarn("ChartSeries.parseOOXML: unexpected pivot value order- skipping"
                                                     * );
                                                     * z++;
                                                     */
                                                }
                                        ...
                                        // pivot charts: apparently always truncate/skip last cell in range (which
                                        ...
                                        if (npoints < clist.length) {// truncate!
                                            ...
                                            // com.valkyrlabs.toolkit.Logger.log("Truncate list: old range: " +
                                            // ranges[idx] + " new range: " + cells.getSheet().getQualifiedSheetName() +
                                            // "!" + ExcelTools.formatLocation(rc));
                                            ranges[idx] = cells.getSheet().getQualifiedSheetName() + "!"
                                                    + ExcelTools.formatLocation(rc);
                                        }
                                    ...
                                    Logger.logErr("ChartSeries.parseOOXML: Error adjusting pivot range for "
                                            + parentChart + ":" + e.toString());
                    ...
                    if (xpp.getName().equals("ser")) {
                        ...
                        Series s = parentChart.getParentChart().getChartSeries().addSeries(ranges[2], ranges[1],
                                ranges[3], ranges[0], legendText, parentChart,
                                parentChart.getParentChart().getChartOrder(parentChart));
                        if (sp != null)
                            s.setSpPr(sp);
                        // TODO: " When you create a chart, by default - the first six series are the
                        // six accent colors in order - but not the exact color or any variation that
                        // appears in the palette. They're typically (unless the primary accent color
                        // being modified is extremely dark) a bit darker than the primary accent color.
                        // Chart series 7 - 12 use the actual primary accent colors 1 through 6 ... and
                        // then chart series 13 starts a set of lighter variations of the six accent
                        // colors that are also slightly different from any position in the palette."
                        else if (seriesidx < 7) // TODO: figure out where to get colors past 6
                            if (seriesidx > 0 && parentChart instanceof PieChart)
                                Logger.logWarn(
                                        "ChartSeries.parseOOXML:  more than 1 series encountered for a Pie-style chart");
                            else
                                s.setColor(wbh.getWorkBook().getTheme().genericThemeClrs[seriesidx + 4]); // series
                                                                                                          // colors
                                                                                                          // start at 4
                        if (d != null)
                            s.setDLbls(d);
    ...
    /**
     * returns an array containing the and maximum values of Y (Value) axis, along
     * with the
     * maximum number of series values of each series (bar, line, etc.) ...
     * <br>
     * sets all series cache values: seriesvalues, seriesranges, seriescolors,
     * legends and minmaxcache
     *
     * <br>
     * Note: this will only reset cached values if isDirty
     *
     * @return double[]
     */
    public double[] getMetrics(boolean isDirty) {
        ...
        com.valkyrlabs.formats.XLS.Boundsheet sht = parentChart.getSheet();
        java.util.Vector s = this.getAllSeries(-1);
        // Category values
        // *******************************************************************************
        if (s.size() > 0) {
            try {
                CellRange cr = new CellRange(((Series) s.get(0)).getCategoryValueAi().toString(), parentChart.wbh,
                        true);
        ...
        /*
         * A Scatter chart has two value axes, showing one set of numerical data along
         * the x-axis and another along the y-axis.
         * It combines these values into single data points and displays them in uneven
        ...
        if (charttype != PIECHART && charttype != DOUGHNUTCHART) {
            ...
            for (int i = 0; i < s.size(); i++) {
                ...
                legends[i] = com.valkyrlabs.formats.XLS.OOXMLAdapter.stripNonAscii(myseries.getLegendText()).toString();
                ...
                if (myseries.hasBubbleSizes()) { // append bubble sizes to series values ... see BubbleChart.getSVG for
                                                 // parsing
                    int z = ch.length;
            ...
            legends = new String[categories.length]; // for PIE/DONUT charts, legends are actually category labels, not
                                                     // series labels
            for (int i = 0; i < categories.length; i++)
                legends[i] = com.valkyrlabs.formats.XLS.OOXMLAdapter.stripNonAscii(categories[i].toString()).toString();
            for (int i = 0; i < s.size(); i++) {
                Series myseries = ((Series) s.get(i));
                // legends[i]=
                // com.valkyrlabs.formats.XLS.OOXMLAdapter.stripNonAscii(myseries.getLegend());
                CellRange cr = new CellRange(myseries.getSeriesValueAi().toString(), parentChart.wbh, true);
                CellHandle[] ch = cr.getCells();
                ...
                for (int j = 0; j < ch.length; j++) {
                    try {
                        ...
                        if (i == 0) { // only do for 1st series; will be the
                             ...
                             * || seriescolors[j] == 0x4E)
                             * seriescolors[j] = com.valkyrlabs.formats.XLS.FormatConstants.COLOR_WHITE;
                             */
                        }
            ...
            if (s.size() > 0) {
                // PIE: 1 series data
                CellHandle[] cats = new CellRange(((Series) s.get(0)).getCategoryValueAi().toString(), parentChart.wbh,
                        true).getCells();
                if (cats != null) {
                    nseries = cats.length;
                    legends = new String[cats.length]; // for PIE charts, legends are actually category labels, not
                                                       // series labels
                ...
                try {
                    ...
                    for (int i = 0; i < nseries; i++) {
                         ...
                         * seriescolors[i] = com.valkyrlabs.formats.XLS.FormatConstants.COLOR_WHITE;
                         */
                        // legends[i]=
                        // com.valkyrlabs.formats.XLS.OOXMLAdapter.stripNonAscii(myseries.getLegend());
     ...
     *                      s * @return
     */
    public Series addSeries(String seriesRange, String categoryRange, String bubbleRange, String legendRange,
            String legendText, ChartType chartObject, int nChart) {
    ...
    /**
     * attempts to replace all category elements containing originalval text to
     * newval
     *
     * @param originalval
     * @param newval
     ...
     *
     * @return JSONObject in form of: c:catgeory range, {v:series range,
     *         l:legendrange, b: bubble sizes} for each series
     */
     ...
     * @param s JSONArray
     */
    public void setSeriesJSON(JSONArray s)
    ...
    public Ptg[] getCellRangePtgs() {
        ArrayList locptgs = new ArrayList();
        for (int i = 0; i < series.size(); i++) {
            Series s = (Series) ((Object[]) series.get(i))[0];
    ...
    /**
     * @return an HashMap of Series Range Ptgs mapped by Series representing all the
     *         series in the chart
     */
     ...
     * bubble: invertIfNegative, dPt, dLbls, trendline, errBars, xVal, yVal,
     * bubbleSize, bubble3D (bubbleChart)
     * line marker, dPt, dLbls, trendline, errBars, cat, val, smooth (line3DChart,
     * lineChart, stockChart)
     * pie: explosion, dPt,dLbls, cat, val (doughnutChart, ofPieChart, pie3DChart,
     * pieChart)
     * surface: cat, val (surfaceChart, surface3DChart)
     * scatter: marker, dPt, dLbls, trendline, errBars, xVal, yVal, smooth
     * (scatterChart)
     * radar: marker, dPt, dLbls, cat, val (radarChart)
     * area: pictureOptions, dPt, dLbls, trendline, errBars, cat, val (area3DChart,
     * areaChart)
     * bar: invertIfNegative, pictureOptions, dPt, dLbls, trendline, errBars, cat,
     * val, shape (bar3DChart, barChart)
     */
    // TODO: finish options
    // TODO: refactor !!!
        ...
        for (int i = 0; i < v.size(); i++) {
            ...
            if (s.getDLbls() != null)
                ...
                if (dl > 0) { // todo: showLegendKey catpercent ? sppr + txpr
                    // TODO: spPr, txPr
                    DLbls dlbl = new DLbls(((dl | 0x1) == 0x1), ((dl | 0x08) == 0x08), false, ((dl | 0x10) == 0x10),
                            ((dl | 0x40) == 0x40),
            ...
            if (ct == BUBBLECHART) { // also include bubble sizes
    ...
    protected void updateSeriesMappings(SeriesList sl, int thischartnumber) {
        ...
        try {
            ...
            throw new WorkBookException("ChartSeries.updateSeriesMappings failed:" + e.toString(),
                    WorkBookException.RUNTIME_ERROR);
        }
    }

    /**
     * return Data Labels Per Series or default Data Labels, if no overrides
     * specified
     *
    ...
    public int[] getDataLabelsPerSeries(int defaultDL, int charttype) {
        if (charttype == PIECHART || charttype == DOUGHNUTCHART) { // handled differently
            if (series.size() > 0) {
