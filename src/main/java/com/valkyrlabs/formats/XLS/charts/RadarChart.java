...
//		radar = (Radar) charttype;
     ...
     * @return String svg
     */
    public String getSVG(HashMap<String, Double> chartMetrics,
                         HashMap<String, Object> axisMetrics, ChartSeries s) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = 0; j < curseries.length; j++) {
                ...
                points += x1 + "," + y1 + " ";
                String l = getSVGDataLabels(dls, axisMetrics, curseries[j],
                        percentage, j, legends, categories[j].toString());
                if (l != null) {
                    double labelx1 = centerx + (r + 5)
                            * (Math.cos(Math.toRadians(angle))); //
                    double labely1 = centery - (r + 5)
            ...
            points += x0 + "," + y0;
            // 1st line is black
            svg.append("<polyline " + getScript("") + " fill-opacity='0' "
                    + getStrokeSVG(4, "black") + " points='" + points + "'"
                    + "/>\r\n");
            // 2nd line is the series color
            svg.append("<polyline " + getScript("") + "   id='series_"
                    + (i + 1) + "' fill='none' fill-opacity='0' "
                    + getStrokeSVG(3, seriescolors[i]) + " points='" + points
                    + "'" + "/>\r\n");
