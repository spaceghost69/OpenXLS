 ...
 and data table of the chart. This record MUST be ignored on a secondary axis group. */
    ...
    public void setTitle(int axisType, String Title) {
        ...
        if (a != null) {
            ...
            a.setTitle(Title);
            if (defaultIsVert) a.getTd().setRotation(90);
        }
    }
    ...
    public JSONObject getJSON(int axisType, com.valkyrlabs.OpenXLS.WorkBookHandle wbh, int chartType, double yMax, double yMin, int nSeries) {
    ...
    public HashMap<String, Object> getMetrics(int charttype, HashMap<String, Double> chartMetrics, float[] plotcoords, Object[] categories) {
        double[] minmax = this.getMinMax(chartMetrics.get("min"), chartMetrics.get("max"));    // sets min/max on Value axis, based upon axis settings and actual minimun and maximum values
        ...
        if (!this.getTitle(XAXIS).equals("")) {
            com.valkyrlabs.formats.XLS.Font ef = this.getTitleFont(XAXIS);
        ...
        if (this.hasAxis(YAXIS)) {
            ...
            if (!title.equals("")) {
                com.valkyrlabs.formats.XLS.Font ef = this.getTitleFont(YAXIS);
                java.awt.Font f = new java.awt.Font(ef.getFontName(), ef.getFontWeight(), (int) ef.getFontHeightInPoints());
                AtomicInteger h = new AtomicInteger(0);
...
/*    			    			
            ...
            if (major > 0) {
                ...
                if (Math.floor(major) != major) {    // contains a fractional part ... avoid java floating point issues
                    ...
                    for (double i = min; i <= max; i += major) {
                        ...
                        series[z++] = new StringBuffer(
                                CellFormatFactory.fromPatternString(
                                        (String) getMetric("yPattern")).format(bd.toString()));
                    }
                } else { // usual case of an int scale
                    int z = 0;
                    for (double i = min; i <= max; i += major)
                        series[z++] = new StringBuffer(CellFormatFactory.fromPatternString((String) getMetric("yPattern")).format(i));
                }
            }
        } catch (Exception e) {
        ...
        // Label Offsets ...
        if (this.hasAxis(XAXIS) && charttype != RADARCHART && charttype != RADARAREACHART) { //(Pie, donut, etc. don't have axes labels so disregard
            Object[] s;            // Determine X Axis Label offsets
            double width;
            AtomicInteger rot;
            com.valkyrlabs.formats.XLS.Font lf;
            String pattern;
            if (charttype != BARCHART) {
                s = categories;
        ...
        if (this.hasAxis(YAXIS) && charttype != RADARCHART && charttype != RADARAREACHART) {    //(Pie, Donut, etc. don't have axes labels so disregard
     ...
     * @return
     */
    private double getLabelOffsets(com.valkyrlabs.formats.XLS.Font lf, double width, Object[] strings, AtomicInteger rot, String pattern, boolean horiz) {
        double retwidth = 0;
        ...
        try {
            ...
            for (int i = 0; i < strings.length; i++) {
                ...
                } catch (IllegalArgumentException e) { // trap error formatting
    ...
    private double getRotatedWidth(java.awt.Font f, AtomicInteger height, String s, int rot) {
        ...
        for (int i = 0; i < slines.length; i++) {
            ...
            if (rot == 0) {
                ...
                height.set((int) Math.max(height.intValue(), StringTool.getApproximateStringWidth(f, CellFormatFactory.fromPatternString(null).format(slines[i]))));
            } else { // 45
                width = StringTool.getApproximateStringWidth(f, CellFormatFactory.fromPatternString(null).format(slines[i]));
    ...
    private double getRotatedWidthVert(java.awt.Font f, AtomicInteger height, String s, int rot) {
        ...
        for (int i = 0; i < slines.length; i++) {
            ...
            if (Math.abs(rot) == 90) {    // means VERTICAL orientation - default
                ...
                width = Math.max(height.intValue(), StringTool.getApproximateStringWidth(f, CellFormatFactory.fromPatternString(null).format(slines[i])));
            } else { // 45
                width = StringTool.getApproximateStringWidth(f, CellFormatFactory.fromPatternString(null).format(slines[i]));
                width = Math.ceil(width * (Math.cos(Math.toRadians(rot))));
                height.set((int) Math.ceil(width * (Math.sin(Math.toRadians(rot)))));
        ...
        while (len > width) {
            ...
            if (lastSpace == -1) {    // no spaces to break apart via \n's - rotate!
                if (str.indexOf(' ') == -1) {
                    ...
                    break;
                } else lastSpace = s.toString().indexOf(' ');
            }
