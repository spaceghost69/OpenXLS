...
public final class Ai extends GenericChartObject implements ChartObject {
    ...
    // expression bytes ..
    protected static byte[] AI_TYPE_LEGEND = new byte[]{0, 2, 0, 0, 0, 0,
            0, 0};                                                                // ,
    // 7,
    // 0,
    // 58,
    ...
    // 0,
    // 0};
    protected static byte[] AI_TYPE_SERIES = new byte[]{1, 2, 0, 0, 0, 0,
            0, 0};                                                                // ,
    // 11,
    // 0,
    // 59,
    ...
    // 3,
    // 0};
    protected static byte[] AI_TYPE_CATEGORY = new byte[]{2, 2, 0, 0, 0, 0,
            0, 0};                                                                // 11,
    // 0,
    // 59,
    // 0,
    ...
    // 3,
    // 0};
    protected static byte[] AI_TYPE_BUBBLE = new byte[]{3, 1, 0, 0, 0, 0,
            0, 0};
    protected static byte[] AI_TYPE_NULL_LEGEND = new byte[]{0, 1, 0, 0, 0, 0,
            0, 0};
    protected int id = -1, ifmt = -1, cce = -1;
    private Stack<Ptg> expression;
    private short grbit = -1, rt = -1;
    private boolean fCustomIfmt = false;
    // private ArrayList xlsrecs = new
    // ArrayList();
    ...
    public void populateForTransfer(String origSheetName) {
        if ("".equals(boundName)) {
            ...
            for (int t = 0; t < expression.size(); t++) {
                Ptg p = expression.get(t);
                if (p instanceof PtgArea3d) {
                    PtgArea3d pt = (PtgArea3d) p;
                    try {
                        boundName = pt.getSheetName(); // original boundname +
                        // xti sheet ref
                        boundXti = pt.getIxti();
                    } catch (Exception e) {
                ...
                } else if (p instanceof PtgMemFunc) { // 20091015 KSC: for
                    // non-contiguous ranges
                    Ptg ptg = ((PtgMemFunc) p).getFirstloc();
                    if (ptg instanceof PtgRef3d) {
                        PtgRef3d pr = (PtgRef3d) ptg;
                        try {
                            boundName = pr.getSheetName(); // original boundname
                            // + xti sheet ref
                            boundXti = pr.getIxti();
                        } catch (Exception e) {
                            Logger.logErr("Ai.populateForTransfer: Chart contains links to other data sources");
     ...
     */
    public void setExternsheetRef(int x) throws SheetNotFoundException {
     ...
     */
    public void setExternsheetRef(int oldRef, int newRef) throws SheetNotFoundException {
        for (int t = 0; t < expression.size(); t++) {
            Ptg p = expression.get(t);
            if (p instanceof PtgArea3d) {
                PtgArea3d pt = (PtgArea3d) p;
                ...
                if (oRef == oldRef) { // got the one to update
                    ...
                    pt.setSheetName(this.getSheet().getSheetName()); // 20100415
                    // KSC:
                    // added
                ...
                if (oRef == oldRef) {
                    ...
                    pr.setSheetName(this.getSheet().getSheetName()); // 20100415
                    // KSC:
                    // added
                ...
                if (pr instanceof PtgRef3d) {
                    ...
                    if (oRef == oldRef) {
                        ((PtgRef3d) pr).removeFromRefTracker(); // 20100506 KSC:
                        // added
                        ((PtgArea3d) pr)
                                .setSheetName(this.getSheet().getSheetName()); // 20100415
                        // KSC:
                        // added
    ...
    public void updateSheetRef(String newSheetName, String origWorkBookName) {
        try {
            ...
            if (boundXti > -1) { // has populate for transfer been called -
                ...
                int newSheetNum = -1;
                try {
                    if (!boundName.equalsIgnoreCase(origSheetName)) { // Ai
                        // reference
                        // is on
                        // a
                        // dfferent
                        ...
                        // new
                        // workbook
                        newSheetNum = this.getWorkBook()
                                .getWorkSheetByName(boundName).getSheetNum();
                    } else // Ai reference is on same sheet, point now to new
                        // sheet
                        newSheetNum = this.getWorkBook()
                                .getWorkSheetByName(newSheetName).getSheetNum();
                } catch (Exception e) { // 20080123 KSC: if links arent there,
                    // fix == try to make an External ref
                    for (int t = 0; t < expression.size(); t++) {
                        if (expression.get(t) instanceof PtgArea3d) {
                            PtgArea3d p = (PtgArea3d) expression.get(t);
                            Logger.logWarn("External References are unsupported: External reference found in Chart: "
                                    + p.getSheetName());
                            p.setSheetName(boundName); // set external reference
                            // to original
                            // boundsheet name
                ...
                if (newSheetNum != -1) {
                    this.setSheet(this.getWorkBook()
                            .getWorkSheetByName(newSheetName)); // 20100415 KSC:
                    // set Ai sheet
                    // ref to new
                    // sheet
                    Externsheet xsht = this.getWorkBook().getExternSheet(true); // create
                    // if
                    // necessary
    ...
    public String getDefinition() {
        ...
        ep = expression.toArray(ep);
        for (int t = 0; t < ep.length; t++) {
            if (!(ep[t] instanceof PtgParen)) // 20091019 KSC: if complex
     ...
     * "C4:D9"
     */
    public List<Ptg> getPtgsByLocation(String loc) {
        try {
    ...
    public boolean setLocationPolicy(String loc, int l) {
        List<Ptg> dx = this.getPtgsByLocation(loc);
        Iterator<Ptg> lx = dx.iterator();
        while (lx.hasNext()) {
            Ptg d = lx.next();
            if (d != null) {
            ...
            try { // NOTE: Ai has only 1 expression OR 2: 1= PtgMemFunc
                ...
                if (expression.get(0) instanceof PtgMemFunc) {
                    try { // must find particular ptg in PtgMemFunc's
                        ...
                        Logger.logErr("Ai.changeAiLocation: Error updating Location in non-contiguous range "
                                + ex.toString());
                    }
                } else {
                    p.setLocation(aiLocs[i]); // updates ref. tracker
                    expression.set(0, p); // update expression with new Ptg
                    if (this.getType() == Ai.TYPE_TEXT) { // must reset text for
                        // SeriesText as
                        // well
                        try {
                            Object o = p.getValue();
    ...
    public boolean changeAiLocation(String loc, String newLoc) {
        ...
        try {
            if (expression.size() > 0) {
                ...
                ptg = expression.get(z); // 20090917 KSC: since creating
        ...
        if (z == -1 && newLoc.equals("")) {// no reference -- happens on
            // legends, category ai's ...
            this.getData()[1] = 1; // text reference rather than worksheet
            // reference
            return false;
        }
