...
public abstract class ChartType implements ChartConstants, Serializable {
    ...
    public static ChartType createChartTypeObject(GenericChartObject ch, ChartFormat cf, WorkBook wb) {
        ...
        switch (ch.chartType) {
            case ChartConstants.COLCHART:
                if (barshape == SHAPEDEFAULT) { // regular column
                    if (isStacked || is100Percent)
                        return new StackedColumn(ch, cf, wb);
                    if (threeD)
                        return new Col3DChart(ch, cf, wb);
                    return new ColChart(ch, cf, wb);
                } else if (barshape == SHAPECONE) { // Cone chart	 always 3d
                    return new ConeChart(ch, cf, wb);
                } else if (barshape == SHAPECYLINDER) { // Cylinder chart always 3d
                    return new CylinderChart(ch, cf, wb);
                } else if (barshape == SHAPEPYRAMID) {    // Pyramid chart	alwasy 3d
                    return new PyramidChart(ch, cf, wb);
                }
            case ChartConstants.BARCHART:
                if (barshape == SHAPEDEFAULT) { // regular Bar
                    if (threeD)
                        return new Bar3DChart(ch, cf, wb);
                    return new BarChart(ch, cf, wb);
                } else if (barshape == SHAPECONE) { // ConeBarchart	always 3d
                    return new ConeBarChart(ch, cf, wb);
                } else if (barshape == SHAPECYLINDER) { // CylinderBar chart	always 3d
                    return new CylinderBarChart(ch, cf, wb);
                } else if (barshape == SHAPEPYRAMID) {    // PyramidBar chart	always 3d
     ...
     * @return
     */
    public static GenericChartObject createUnderlyingChartObject(int chartType, Chart parentchart, ChartFormat cf) {//, ChartType chartobj) {
    ...
    public static ChartType parseOOXML(XmlPullParser xpp, WorkBookHandle wbh, Chart parentChart, int nChart) {
        try {
            ...
            if ((tnm.equals("bubble3D"))) {    // bubble3D tag appears for each series in 3D bubble chart
...
// sizeRepresents
// custSplit	-- OfPie
                    if (tnm.equals("grouping")) {    //This element specifies the type of grouping for a column, line, or area chart + 3d versions
                        if (v.equals("stacked"))
                        ...
                        else if (v.equals("standard")) {    // for Line, Line3d, Area, Area3d and Bar3d, Col3d - does not appear valid for Bar 2d and Col 2d
                    ...
                    } else if (tnm.equals("shape")) {    // bar3d
                    ...
                    } else if (tnm.equals("wireframe")) {    // surface
                        ...
                        cl.parseOOXML(xpp, lastTag, cf, wbh);
                    } else if (tnm.equals("overlap")) {    // bar
                    ...
                    } else if (tnm.equals("marker")) {        // line only?
...
// HANDLE bar shapes ***
        ...
        if (threeD == null) {
            if (options.contains(ChartOptions.THREED) || chartType == SURFACECHART) {    // surface charts ALWAYS have a 3 record as does pyramid, cone and cylinder charts
                ...
                else if (chartType == BUBBLECHART) { // scatter charts have no 3d option
     ...
     *
     * @param bShow public void showDataTable(boolean bShow) {
        ...
        if (showValue || showCategories || showValueLabel || showValuePercent || showBubbleLabel) {
            ...
            if (showCategories)
                l += CellFormatFactory.fromPatternString(
                        (String) axisMetrics.get("xPattern")).format(cat) + " ";    // categories
            if (showValue || showBubbleLabel) {
                l += CellFormatFactory.fromPatternString(
                        (String) axisMetrics.get("yPattern")).format(String.valueOf(val));
				/*try {
...
//		com.valkyrlabs.formats.XLS.Font f = this.getFont();	// this is not correct!
    ...
    public void WriteMainChartRecs(String fName) {
        class util {
            public void writeRecs(BiffRec b, BufferedWriter writer, int level) throws IOException {
                String tabs = "\t\t\t\t\t\t\t\t\t\t";
                if (b == null) return;
                writer.write(tabs.substring(0, level) + b.getClass().toString().substring(b.getClass().toString().lastIndexOf('.') + 1));
                if (b instanceof com.valkyrlabs.formats.XLS.charts.SeriesText)
                ...
                try {
                    if (((GenericChartObject) b).chartArr.size() > 0) {
                        ArrayList<com.valkyrlabs.formats.XLS.XLSRecord> chartArr = ((GenericChartObject) b).chartArr;
                        for (int i = 0; i < chartArr.size(); i++) {
                            writeRecs(chartArr.get(i), writer, level + 1);
        ...
        try {
            ...
            writer.newLine();
            ArrayList<com.valkyrlabs.formats.XLS.XLSRecord> chartArr = this.cf.chartArr;
            for (int i = 0; i < chartArr.size(); i++) {
                u.writeRecs(chartArr.get(i), writer, 0);
            }
