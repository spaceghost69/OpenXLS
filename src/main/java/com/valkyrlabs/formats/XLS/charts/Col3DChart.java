...
public class Col3DChart extends ColChart {
    public Col3DChart(GenericChartObject charttype, ChartFormat cf, WorkBook wb) {
        super(charttype, cf, wb);
        col = (Bar) charttype;
    }

    /**
     * gets the chart-type specific ooxml representation: <col3DChart>
     *
     * @return
     */
    public StringBuffer getOOXML(String catAxisId, String valAxisId, String serAxisId) {
        ...
        if (this.getParentChart().getAxes().hasAxis(ZAXIS)) {
            ...
            cooxml.append("\r\n");
        } else {// KSC: // KSC: appears to be necessary but very unclear as to why
