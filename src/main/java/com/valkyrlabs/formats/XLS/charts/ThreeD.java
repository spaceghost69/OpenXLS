...
import com.valkyrlabs.formats.OOXML.OOXMLElement;
...
public class ThreeD extends GenericChartObject implements ChartObject {
    ...
    private final byte[] PROTOTYPE_BYTES = new byte[]{0, 0, 0, 0, 30, 0, 100, 0,
            100, 0, -106, 0, 0, 0};
...
//						if (v!=null)
    ...
    public boolean setChartOption(String op, String val) {
        ...
        if (op.equalsIgnoreCase("AnRot")) { // specifies the clockwise rotation, in degrees, of the 3-D plot area around a vertical line through the center of the 3-D plot area.
            anRot = Short.parseShort(val); // usually 20
            bHandled = true;
        }
        if (op.equalsIgnoreCase("AnElev")) { // signed integer that specifies the rotation, in degrees, of the 3-D plot area around a horizontal line through the center of the 3-D plot area
            anElev = Short.parseShort(val); // usually 15
            bHandled = true;
        }
        if (op.equalsIgnoreCase("PcDist")) { // view angle for the 3-D plot area.
            pcDist = Short.parseShort(val); // usually 30
            bHandled = true;
        }
        if (op.equalsIgnoreCase("PcHeight")) { // specifies the height of the 3-D plot area as a percentage of its width
            pcHeight = Short.parseShort(val);
            bHandled = true;
        }
        if (op.equalsIgnoreCase("PcDepth")) { // specifies the depth of the 3-D plot area as a percentage of its width.
            pcDepth = Short.parseShort(val); // usually 100
            bHandled = true;
        }
        if (op.equalsIgnoreCase("PcGap")) { // specifies the width of the gap between the series and the front and back edges of the 3-D plot area as a percentage of the data point depth divided by 2. If
         ...
         */
        if (op.equalsIgnoreCase("Perspective")) { // specifies whether the 3-D plot area is rendered with a vanishing point.
            fPerspective = Boolean.valueOf(val).booleanValue();
            grbit = ByteTools.updateGrBit(grbit, fPerspective, 0);
            bHandled = true;
        }
        if (op.equalsIgnoreCase("Cluster")) { // specifies whether data points are clustered together in a bar chart group
            fCluster = Boolean.valueOf(val).booleanValue();
            grbit = ByteTools.updateGrBit(grbit, fCluster, 1);
            bHandled = true;
        }
        if (op.equalsIgnoreCase("ThreeDScaling")) { // specifies whether the height of the 3-D plot area is automatically determined
            f3dScaling = Boolean.valueOf(val).booleanValue();
            grbit = ByteTools.updateGrBit(grbit, f3dScaling, 2);
            bHandled = true;
        }
        if (op.equalsIgnoreCase("TwoDWalls")) { // A bit that specifies whether the chart walls are rendered in 2-D
