...
public class DoughnutChart extends ChartType {
     ...
     * @return
     */
    public JSONObject getJSON(ChartSeriesHandle[] series, WorkBookHandle wbh, Double[] minMax)
    ...
    public String getSVG(HashMap<String, Double> chartMetrics, HashMap<String, Object> axisMetrics, ChartSeries s) {
        ...
        for (int i = n - 1; i >= 0; i--) {    // for each series
            double[] curseries = (double[]) series.get(i);        // series data points
            double total = 0.0;
            for (int j = 0; j < curseries.length; j++) {    // get total in order to calculate percentages
                total += curseries[j];
            }
            if (dls.length == 1) {    // no seriess-specific data labels; expand to entire series for loop below
                int dl = dls[0];
                dls = new int[curseries.length];
                for (int z = 0; z < dls.length; z++)
                    dls[z] = dl;
            }
            svg.append("<circle " + getScript("") + " cx='" + centerx + "' cy='" + centery + "' r='" + r + "' " + getStrokeSVG(2, getDarkColor()) + " fill='none'/>\r\n");
            double x = centerx + r;
            double y = centery;
            String path = "";
            double percentage = 0;
        ...
        svg.append("<circle " + getScript("") + " cx='" + centerx + "' cy='" + centery + "' r='" + r + "' " + getStrokeSVG(2, getDarkColor()) + " fill='white'/>\r\n");
        svg.append("</g>\r\n");
