...
public class Calculator {
     ...
     * @return true if comparison of operator and value with cell value passes,
     *         false otherwise
     */
    public static boolean compareCellValue(BiffRec c, Object o, String op) {
        ...
        try {
            if (o instanceof Boolean) // TODO: 1.5 use Boolean.compareTo
                ...
                // compare= ((String)
                // o).toUpperCase().compareTo(c.getStringVal().toUpperCase());
            } else // it's a Double
        ...
        try {
            if (val instanceof Boolean) // TODO: 1.5 use Boolean.compareTo
                ...
                if ((compareval.indexOf('?') == -1) &&
                        (compareval.indexOf('*') == -1)) // if no wildcards
            ...
            } catch (Exception ex) { // just try string compare
     ...
     * @return
     */
    public static PtgArea getRange(Ptg p)
            throws IllegalArgumentException {
        if (p instanceof PtgArea)
            return (PtgArea) p;
        if (p instanceof PtgName) { // get source range
            Ptg[] pr = null;
            try {
