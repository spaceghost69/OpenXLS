...
public class EngineeringCalculator {
    ...
    */
    ...
    private static final double[] ERF_A = {3.16112374387056560E00,
            1.13864154151050156E02, 3.77485237685302021E02,
    ...
    private static final double[] ERF_B = {2.36012909523441209E01,
            2.44024637934444173E02, 1.28261652607737228E03,
    ...
    private static final double[] ERF_C = {5.64188496988670089E-1,
            8.88314979438837594E0, 6.61191906371416295E01,
    ...
    private static final double[] ERF_Q = {2.56852019228982242,
    ...
    protected static Ptg calcBin2Dec(Ptg[] operands) {
        ...
        // 10 bits at most
        if (bString.length() > 10) return new PtgErr(PtgErr.ERROR_NUM);
        ...
        if (DEBUG) Logger.logInfo("Result from BIN2DEC= " + pnum.getVal());
        return pnum;
    }
    ...
    protected static Ptg calcComplex(Ptg[] operands) {
        ...
        if (complexString.equals("")) complexString = "0";

        PtgStr pstr = new PtgStr(complexString);
        if (DEBUG) Logger.logInfo("Result from COMPLEX= " + pstr.getString());
        return pstr;
    }
    ...
    protected static Ptg calcConvert(Ptg[] operands) {
        ...
        String[] allUnits = {"g", "sg", "lbm", "u", "ozm", "m", "mi", "Nmi", "in", "ft", "yd", "ang", "Pica",
                "yr", "day", "hr", "mn", "sec", "Pa", "atm", "mmHg", "N", "dyn", "lbf",
                "J", "e", "c", "cal", "eV", "HPh", "Wh", "flb", "BTU", "HP", "W", "T", "ga",
                "C", "F", "K", "tsp", "tbs", "oz", "cup", "pt", "uk_pt", "qt", "gal", "l"};
        ...
        if (findUnits(fromUnits, allUnits) < 0) { // doesn't match; strip prefix and try again
            if (fromUnits.length() > 1) {
                ...
                if (findUnits(fromPrefix, metricPrefixes) < 0)
                    return new PtgErr(PtgErr.ERROR_NA);
            } else return new PtgErr(PtgErr.ERROR_NA);
        }
        ...
        if (DEBUG) Logger.logInfo("Result from CONVERT= " + pnum.getString());
        return pnum;
    }
    ...
    protected static Ptg calcDec2Hex(Ptg[] operands) {
        ...
        if (operands.length > 1)
            places = operands[1].getIntVal();
        if (dec < -549755813888L || dec > 549755813887L || places < 0) return new PtgErr(PtgErr.ERROR_NUM);
        ...
        PtgStr pstr = new PtgStr(hString);
        if (DEBUG) Logger.logInfo("Result from DEC2HEX= " + pstr.getString());
        return pstr;
    }
    ...
    protected static Ptg calcDec2Oct(Ptg[] operands) {
        ...
        if (operands.length > 1)
            places = operands[1].getIntVal();
        if (dec < -536870912L || dec > 536870911L || places < 0) return new PtgErr(PtgErr.ERROR_NUM);
        ...
        PtgStr pstr = new PtgStr(oString);
        if (DEBUG) Logger.logInfo("Result from DEC2OCT= " + pstr.getString());
        return pstr;
    }
    ...
    protected static Ptg calcDelta(Ptg[] operands) {
        ...
        PtgNumber pnum = new PtgNumber(result);
        if (DEBUG) Logger.logInfo("Result from DELTA= " + pnum.getString());
        return pnum;
    }
    // ===========================================================================
...
//			if (lower_limit < 0 /*|| upper_limitupper_limit < 0*/) return new PtgErr(PtgErr.ERROR_NUM);
            ...
            if (!Double.isNaN(upper_limit)) {    // Erf(upper)-Erf(lower)
     ...
     */
    public static boolean hasConverged(final double xn, final double xo,
     ...
     * @return flag indicating if accuracy is reached.
     */
    public static boolean hasReachedAccuracy(final double xn, final double xo,
                                             final double eps) {
    ...
    private static double erfRational(final double x) {
        ...
        final double[] a = {.254829592, -.284496736, 1.421413741,
...
*/
    ...
    protected static Ptg calcGEStep(Ptg[] operands) {
        ...
        PtgNumber pnum = new PtgNumber(result);
        if (DEBUG) Logger.logInfo("Result from GESTEP= " + pnum.getString());
        return pnum;
    }
    ...
    protected static Ptg calcHex2Bin(Ptg[] operands) {
        ...
        if (hString.length() > 10) return new PtgErr(PtgErr.ERROR_NUM);
        int places = 0;
        if (operands.length > 1)
            places = operands[1].getIntVal();
        ...
        if (dec < -512 /*0xFFFFFFFE00*/ || dec > 0x1FF || places < 0) return new PtgErr(PtgErr.ERROR_NUM);
        if (dec < 0) {    // truncate to 10 digits automatically (should already be two's complement)
            bString = bString.substring(Math.max(bString.length() - 10, 0));
        } else if (places > 0) {
            if (bString.length() > places) return new PtgErr(PtgErr.ERROR_NUM);
        ...
        PtgStr pstr = new PtgStr(bString);
        if (DEBUG) Logger.logInfo("Result from HEX2BIN= " + pstr.getString());
        return pstr;
    }
    ...
    protected static Ptg calcHex2Dec(Ptg[] operands) {
        ...
        PtgNumber pnum = new PtgNumber(dec);
        if (DEBUG) Logger.logInfo("Result from HEX2DEC= " + pnum.getVal());
        return pnum;
    }
    ...
    protected static Ptg calcHex2Oct(Ptg[] operands) {
        ...
        if (dec < -536870912L /*0xFFE0000000*/ || dec > 0x1FFFFFFF || places < 0) return new PtgErr(PtgErr.ERROR_NUM);
        if (dec < 0) {    // truncate to 10 digits automatically (should already be two's complement)
            oString = oString.substring(Math.max(oString.length() - 10, 0));
        } else if (places > 0) {
            if (oString.length() > places) return new PtgErr(PtgErr.ERROR_NUM);
        ...
        PtgStr pstr = new PtgStr(oString);
        if (DEBUG) Logger.logInfo("Result from HEX2OCT= " + pstr.getString());
        return pstr;
    }
     ...
     * *
     */
    private static Complex imParseComplexNumber(String complexNumber)
            throws NumberFormatException {
        Complex c = new Complex();
        if (complexNumber.length() > 0) {
            try {
                int i = complexNumber.length();
                if (complexNumber.charAt(i - 1) == 'i' ||
                        complexNumber.charAt(i - 1) == 'j') {
    ...
    protected static Ptg calcImAbs(Ptg[] operands) {
        ...
        PtgNumber pnum = new PtgNumber(result);
        if (DEBUG) Logger.logInfo("Result from IMABS= " + pnum.getString());
        return pnum;
    }
    ...
    protected static Ptg calcImaginary(Ptg[] operands) {
        ...
        if (DEBUG) Logger.logInfo("Result from IMAGINARY= " + pnum.getString());
        return pnum;
    }
    ...
    protected static Ptg calcImArgument(Ptg[] operands) {
        ...
        PtgNumber pnum = new PtgNumber(result);
        if (DEBUG) Logger.logInfo("Result from IMARGUMENT= " + pnum.getString());
        return pnum;
    }
    ...
    protected static Ptg calcImConjugate(Ptg[] operands) {
        ...
        if (DEBUG) Logger.logInfo("Result from IMCONGUGATE= " + pstr.getString());
        return pstr;
    }
    ...
    protected static Ptg calcImCos(Ptg[] operands) {
        ...
        if (DEBUG) Logger.logInfo("Result from IMCOS= " + pstr.getString());
        return pstr;
    }
    ...
    protected static Ptg calcImDiv(Ptg[] operands) {
        ...
        if (DEBUG) Logger.logInfo("Result from IMDIV= " + pstr.getString());
        return pstr;
    }
    ...
    protected static Ptg calcImExp(Ptg[] operands) {
        ...
        if (DEBUG) Logger.logInfo("Result from IMEXP= " + pstr.getString());
        return pstr;
    }
    ...
    protected static Ptg calcImLn(Ptg[] operands) {
        ...
        if (DEBUG) Logger.logInfo("Result from IMLN= " + pstr.getString());
        return pstr;
    }
    ...
    protected static Ptg calcImLog10(Ptg[] operands) {
        ...
        if (DEBUG) Logger.logInfo("Result from IMLOG10= " + pstr.getString());
        return pstr;
    }
    ...
    protected static Ptg calcImLog2(Ptg[] operands) {
        ...
        if (DEBUG) Logger.logInfo("Result from IMLOG2= " + pstr.getString());
        return pstr;
    }
    ...
    protected static Ptg calcImPower(Ptg[] operands) {
        ...
        if (DEBUG) Logger.logInfo("Result from IMPOWER= " + pstr.getString());
        return pstr;
    }
    ...
    protected static Ptg calcImProduct(Ptg[] operands) {
        ...
        if (DEBUG) Logger.logInfo("Result from IMSPRODUCT= " + pstr.getString());
        return pstr;
    }
    ...
    protected static Ptg calcImReal(Ptg[] operands) {
        ...
        if (DEBUG) Logger.logInfo("Result from IMREAL= " + pnum.getString());
        return pnum;
    }
    ...
    protected static Ptg calcImSin(Ptg[] operands) {
        ...
        if (DEBUG) Logger.logInfo("Result from IMSIN= " + pstr.getString());
        return pstr;
    }
    ...
    protected static Ptg calcImSqrt(Ptg[] operands) {
        ...
        if (DEBUG) Logger.logInfo("Result from IMSQRT= " + pstr.getString());
        return pstr;
    }
    ...
    protected static Ptg calcImSub(Ptg[] operands) {
        ...
        if (DEBUG) Logger.logInfo("Result from IMSUB= " + pstr.getString());
        return pstr;
    }
    ...
    protected static Ptg calcImSum(Ptg[] operands) {
        ...
        PtgStr pstr = new PtgStr(imSum);
        if (DEBUG) Logger.logInfo("Result from IMSUM= " + pstr.getString());
        return pstr;
    }
    ...
    protected static Ptg calcOct2Bin(Ptg[] operands) {
        ...
        if (oString.length() > 10) return new PtgErr(PtgErr.ERROR_NUM);
        int places = 0;
        if (operands.length > 1)
            places = operands[1].getIntVal();
        ...
        if (dec < -512 /*7777777000*/ || dec > 0777 || places < 0) return new PtgErr(PtgErr.ERROR_NUM);
        if (dec < 0) {    // truncate to 10 digits automatically (should already be two's complement)
            bString = bString.substring(Math.max(bString.length() - 10, 0));
        } else if (places > 0) {
            if (bString.length() > places) return new PtgErr(PtgErr.ERROR_NUM);
        ...
        PtgStr pstr = new PtgStr(bString);
        if (DEBUG) Logger.logInfo("Result from OCT2BIN= " + pstr.getString());
        return pstr;
    }
    ...
    protected static Ptg calcOct2Dec(Ptg[] operands) {
        ...
        PtgNumber pnum = new PtgNumber(dec);
        if (DEBUG) Logger.logInfo("Result from OCT2DEC= " + pnum.getVal());
        return pnum;
    }
    ...
    protected static Ptg calcOct2Hex(Ptg[] operands) {
        ...
        PtgStr pstr = new PtgStr(hString);
        if (DEBUG) Logger.logInfo("Result from OCT2HEX= " + pstr.getString());
        return pstr;
    }
