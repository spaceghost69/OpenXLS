...
public class PtgArea extends PtgRef implements Ptg {
    ...
    public Ptg[] getComponents() {
        ...
        try {
            ...
            // usually don't need to set sheet on setlocation becuase uses parent_rec's
            // sheet
            // cases of named range or if location sheet does not = parent_rec sheet, set
            // sheet explicitly
    ...
    public boolean contains(int[] rc) {
        ...
        return rc[1] <= thisRange[3]; // col after the last ref col?
    }
    ...
    public String toString() {
        ...
        if (getParentRec() != null)
            if (ret.indexOf("!") < 0)
                try { // Catch SheetNotFoundExceptionhandle Unresolved External refs
                    getSheetName();
                    if (sheetname != null)
                        ret = sheetname + "!" + ret;
                } catch (SheetNotFoundException we) {
    ...
    public void setLocation(int[] rowcol) {
        locax = null; // cache reset
        if (firstPtg == null) {
            // this.record = new byte[] {0x25, 0, 0, 0, 0, 0, 0, 0, 0}; -- don't as can be
            // called from PtgArea3d
            if (this.getParentRec() != null)
                this.populateVals();
        } else if (this.useReferenceTracker)
            this.removeFromRefTracker();
    ...
    private String getLocationHelper() {
        ...
        if (!(this instanceof PtgArea3d)) {
            // if (addr1.equals(addr2)) // this is proper but makes so many assertions fail,
            // revert for now
            // return addr2;
            return addr1 + ":" + addr2;
        }
        ...
        // have sheetname
        if (sh1.equals(sh2)) { // range is in one sheet
            if (!sh1.equals("")) {
                if (!addr1.equals(addr2))
                    return sh1 + "!" + addr1 + ":" + addr2;
                else
                    return sh1 + "!" + addr1;
            } else if (sheetname != null) { // both sheets in sub-ptgs are null
                sh1 = sheetname;
        ...
        } else if (sh2 == null) { // only 1 sheetnaame specified
            if (!addr1.equals(addr2)) // 20081215 KSC:
                return sh1 + "!" + addr1 + ":" + addr2;
            return sh1 + "!" + addr1;
        }
     ...
     * Value class tokens will be changed dependent on further conditions. In array
     * type functions and name type
     * functions, or if the forced array class state is set, it is changed to array
     * class. In all other cases (cell type formula
    ...
    public Object getValue() {
        ...
        if (isArray && array != null && array.length() > 1) { // 20090817 KSC: [BugTracker 2683]
    ...
    public Ptg[] getColComponents(int colNum) {
        ...
        if (p != null) {
            ...
            Ptg[] ret = (Ptg[]) par.getParentRec().getWorkBook()
                    .getRefTracker().getLookupColCache().get(lu + ":" + Integer.valueOf(colNum));
            if (ret != null)
                return ret;
        }
    ...
    public BiffRec[] getRefCells() {
        ...
        try {
            ...
            if (numcols < 0)
                numcols = startcol - endcol; // 20090521 KSC: may have range switched so that firstPtg>lastPtg (example
                                             // in named ranges in tcr_formatted_2007.xlsm)
            numcols++;
            int numrows = endrow - startrow;
            if (numrows < 0)
                numrows = startrow - endrow; // 20090521 KSC: may have range switched so that firstPtg>lastPtg (example
                                             // in named ranges in tcr_formatted_2007.xlsm)
            numrows++;
            ...
            if (totcell == 0)
                totcell++;
            if (totcell < 0) {
                Logger.logErr("PtgArea.getRefCells.  Error in Ptg locations: " + firstPtg.toString() + ":"
                        + lastPtg.toString());
                totcell = 0;
            }
            refCell = new BiffRec[totcell];
            int rowctr = 0;
            // 20090521 KSC: try to handle both cases i.e. ranges such that first<last or
            // first>last
            if (startcol < endcol)
                endcol++;
            else
