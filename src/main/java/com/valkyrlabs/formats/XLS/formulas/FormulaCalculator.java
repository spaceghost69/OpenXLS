...
public class FormulaCalculator {
    ...
    private static final boolean DEBUG = false; // just use this to see ptg calcing
     ...
     */
    public static Object calculateFormula(Stack expression)
            throws FunctionNotSupportedException {
        int sz = expression.size();
        Ptg[] stck = new Ptg[sz];
        stck = (Ptg[]) expression.toArray(stck);
        Stack calcStack = new Stack();
        for (int t = 0; t < sz; t++) { // flip the stack TODO: investigate why needed
            calcStack.add(0, stck[t]);
        }
        Stack tempstack = new Stack();
        Stack newstck = calcStack;
        while (!newstck.isEmpty()) {// loop while there are Ptgs
     ...
     * ptg should return a valid value PTG.
     */
    public static Ptg calculateFormulaPtg(Stack expression)
            throws FunctionNotSupportedException {
        int sz = expression.size();
        Ptg[] stck = new Ptg[sz];
        stck = (Ptg[]) expression.toArray(stck);
    ...
    static void handlePtg(Stack newstck, Stack vals)
        ...
        if (p.getIsOperator() || p.getIsControl() || p.getIsFunction()) {
            ...
            if (p.getIsControl() && !vals.isEmpty()) {
                ...
                if (DEBUG) Logger.logInfo("opr: " + p.toString());
            }
            ...
            Ptg[] vx = new Ptg[t];
            for (; x < t; x++) vx[(t - 1) - x] = (Ptg) vals.pop();// get'em
