...
public class PtgArray extends GenericPtg implements Ptg {
    ...
    boolean isIntermediary = false;                    // 20090824
    // KSC:
    // true
    // if
    // this
    ...
    private void populateVals() {
        if (record.length > 8) { // means that array data has already been
            // appeneded to end of record array; store
            // in rgvals
            rgval = new byte[record.length - 8];
            System.arraycopy(record, 8, rgval, 0, rgval.length); // save post
            // array=
            // nc, nr +
            // array
            // data
    ...
    public void parseArrayComponents() {
        ...
        if (!isIntermediary) // 20090824 KSC: sometimes an intermediary ptgarry
        ...
        try {
            ...
            for (int i = 3; i < rgval.length; ) { // 20090820 KSC: post array
                // contains nc & nr so i
                // should be initially 3
                // instead of 0 [BugTracker
                // 2683]
                if (rgval[i] == 0) { // empty value
                    i++;
                    i += 8;
                    arrVals.add(""); // TODO: Empty Constant should be null?
                } else if (rgval[i] == 0x1) { // its a number
                    i++;
                    ...
                    arrVals.add(d);
                    i = i + 8;
                } else if (rgval[i] == 0x2) { // its a string
                    int strLen = ByteTools
                            .readShort(rgval[i + 1], rgval[i + 2]);
                    i += 3;
                    int grbt = rgval[i++];
                    ...
                    try {
                        ...
                        Logger.logInfo("decoding formula string in array failed: "
                                + e);
                    }
                    arrVals.add(strVal);
                    i += strLen;
                } else if (rgval[i] == 0x4) { // its a boolean
                    if (rgval[++i] == 0)
                        arrVals.add(Boolean.valueOf(false));
                    else
                        arrVals.add(Boolean.valueOf(true));
                    i = i + 8;
                } else if (rgval[i] == 0x10) { // it's an error value
                    int errCode = rgval[++i];
                    switch (errCode) {
                        case 0:
                            arrVals.add("#NULL!");
                ...
                nitems++;
                if (nitems == ((nc + 1) * (nr + 1))) { // Finished with this
                    // array!
                    int length = i;
                    i = rgval.length;
                    ...
                    if (rgval.length != length) {// then truncate both record +
                        // rgval
    ...
    public void setVal(String arrStr) {
        ...
        if (cols[0].length >= 255) { // 20090824 KSC: apparently sometimes an
            // intermediary calculations step can
            // include > 256 array elements ...
            isIntermediary = true;
            nc = cols[0].length - 1;
        }
        databytes[8] = (byte) ((cols[0].length - 1) & 0xFF); // nc-1 // 20090819
        // KSC: placed
        // in wrong pt
        // of record:
        // was [1]
        ...
        System.arraycopy(ByteTools
                .shortToLEBytes((short) (rows.length - 1)), 0, databytes, 9, 2); // nr-1
        // //
        // 20090819
        // KSC:
        // placed
    ...
    public String getString() {
        ...
        String retstr = "";
        if (nc == 0 && nr == 0) { // if it's a single value, just return val
    ...
    private byte[] valuesIntoByteArray(String constVal) {
        ...
        try { // number?
            ...
            try {
                if (constVal.equalsIgnoreCase("true")
                        || constVal.equalsIgnoreCase("false")) {
                    Boolean bb = Boolean.valueOf(constVal);
                    thisElement[0] = 0x4; // id for boolean value
                    thisElement[1] = (byte) (bb.booleanValue() ? 1 : 0);
                } else if (constVal == null || constVal.equals("")) { // emtpy
                    // or
                    // null
                    // value
                    thisElement[0] = 0x0; // id for empty value
                } else if (constVal.charAt(0) == '#') { // it's an error value
                    ...
                    if (constVal.equals("#NULL!"))
                        ...
                        errCode = 0x24;
                    else if (constVal.equals("#N/A!") || constVal.equals("#N/A")
                            || constVal.equals("N/A"))
                        errCode = 0x2A;
                    ...
                    try {
                        ...
                        System.arraycopy(ByteTools
                                .shortToLEBytes((short) b.length), 0, thisElement, 1, 2);
                ...
                Logger.logWarn("PtgArray.valuesIntoByteArray:  error parsing array element:"
                        + ex);
            }
        }
        return databytes;
    ...
    public Ptg[] getComponents() {
        ...
        for (int i = 0; i < arrVals.size(); i++) {
            ...
            if (o instanceof Double) {
                ...
                if (FormulaParser.isRef((String) o)
                        || FormulaParser.isRange((String) o)) { // it's a range
                    ...
                    System.arraycopy(pacomps, 0, temp, retVals.length
                            - 1, pacomps.length);
                    retVals = temp;
                } else {
     ...
     * array.  If you need a multidimensional array please use the other get(int,int)method
     *
     * @param loc
     * @return
     */
    public Ptg get(int loc) {
        Ptg[] p = this.getComponents();
        return p[loc];
    }

    public Ptg get(int col, int row) {
        Ptg[] p = this.getComponents();
        try {
            int loc = 0;
            for (int i = 0; i < row; i++) {
            ...
            Logger.logErr("PtgArray.get: error retrieving value at ["
                    + row + "," + col + "]: " + e);
        }
        return null;
    }
