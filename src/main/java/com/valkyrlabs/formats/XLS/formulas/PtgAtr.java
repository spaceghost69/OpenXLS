...
public class PtgAtr extends GenericPtg implements Ptg {
    ...
    public String getString() {
        ...
        this.init();
        if (bitAttrIf > 0) return "";  //"IF("; this is already taken care of by another ptg.
...
// 	if(bitAttrSemi      > 0)   return "SEMI(";
    ...
    public String getString2() {
        if (bitAttrIf > 0) return "";  //")"; this is already taken care of by another ptg.
...
//        if(bitAttrSemi      > 0)   return ")";
    ...
    public boolean getIsControl() {
        this.init();
        if (getIsPrimitiveOperator()) return false;
...
//        if(bitAttrSemi   > 0)return false;
        ...
        if (bitAttrChoose > 0) return false;
        return false;
    }
...
// Old version?		if(bitAttrSemi   > 0)return true; // it just shows that this is a volatile function
        return bitAttrVolatile > 0; // it just shows that this is a volatile function
    }
...
//        if ((grbit & 0x1)== 0x1){bitAttrSemi = 1;}
    ...
    */
    public Ptg calculatePtg(Ptg[] pthing)
