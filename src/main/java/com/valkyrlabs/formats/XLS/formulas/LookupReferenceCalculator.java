...
public class LookupReferenceCalculator {
    ...
    protected static Ptg calcAddress(Ptg[] operands) {
        if (operands.length < 2) return new PtgErr(PtgErr.ERROR_VALUE);
        ...
        if (operands.length > 2) {
            if (operands[2].getValue() != null) { //checking for a ptgmissarg
                abs_num = ((Integer) operands[2].getValue()).intValue();
            }
        }
        if (operands.length > 3) {
            if (operands[3].getValue() != null) { //checking for a ptgmissarg
                Boolean b = Boolean.valueOf(String.valueOf(operands[3].getValue()));
                ref_style = b.booleanValue();
            }
        }
        if (operands.length > 4) {
            if (operands[4].getValue() != null) { //checking for a ptgmissarg
    ...
    protected static Ptg calcChoose(Ptg[] operands) {
        ...
        try {
            ...
            int i = (int) e;
            if (i > operands.length + 1 || i < 1) return new PtgErr(PtgErr.ERROR_REF);
        ...
        if (operands[0] instanceof PtgFuncVar) {
            ...
            try {
                if (operands[0] instanceof PtgArea) {
                    ...
                    loc += 1;
                    return new PtgInt(loc);
                } else if (operands[0] instanceof PtgName) {    // table???
     ...
     * indicated cell.
     */
    protected static Ptg calcHlookup(Ptg[] operands)
            throws FunctionNotSupportedException {

        if (operands.length < 3) return new PtgErr(PtgErr.ERROR_VALUE);
        boolean sorted = true;
        ...
        // error check
        if (lookupComponents == null || lookupComponents.length == 0) return new PtgErr(PtgErr.ERROR_REF);
        ...
        if (isNumber) {
            double match_num;
            try {
                match_num = Double.parseDouble(
                        lookup_value.getValue().toString());
            } catch (NumberFormatException e) {
            ...
            for (int i = 0; i < lookupComponents.length; i++) {
                ...
                try {
                    val = Double.parseDouble(
                            lookupComponents[i].getValue().toString());
                } catch (NumberFormatException e) {
                ...
                if (val == match_num) {
                    ...
                    if (i == 0) return new PtgErr(PtgErr.ERROR_NA);
                    else return valueComponents[i - 1].getPtgVal();
                }
            }

            if (sorted) return valueComponents[lookupComponents.length - 1].getPtgVal();
...
//				 TODO: check rc sanity here
                ...
                try {
                    ...
                    if (o instanceof PtgArea3d) refp = new PtgRef3d();
                    refp.setParentRec(o.getParentRec());
                    refp.setUseReferenceTracker(false);
                    if (sht != null) refp.setSheetName(sht);
    ...
    protected static Ptg calcIndirect(Ptg[] operands) {
        try {
            if (operands[0] instanceof PtgStr) {
                ...
                // detect range
                if (!(FormulaParser.isRef(locx) ||
                        FormulaParser.isRange(locx))) {    // see if it's a named range
    ...
    public static Ptg calcLookup(Ptg[] operands) {
        ...
        if (operands.length > 2) { //normal version of lookup
        ...
        try {
            ...
            for (int i = 1; i <= values.length; i++) {
                ...
                if (v0 instanceof Integer) {
                    if (v1 != null) mType = (((Integer) v0).compareTo((Integer) v1));
                    match = (((Integer) v0).compareTo((Integer) lookupValue));
                } else if (v0 instanceof Double) {
                    if (v1 != null) mType = (((Double) v0).compareTo((Double) v1));
                    ...
                    if (v1 != null) mType = (((String) v0).compareTo((String) v1));
                    match = (((String) v0).compareTo((String) lookupValue));
                }
                if (i < values.length) { // only check order
...
*/
    ...
    protected static Ptg calcOffset(Ptg[] operands) {
        ...
        if (height == 1 && width == 1) { // it's a single reference
            if (rc.length > 3) {    // truncate
                int[] temp = new int[2];
                System.arraycopy(rc, 0, temp, 0, 2);
                rc = temp;
            }
        } else if (!(height == -1 && width == -1)) {
            if (rc.length < 3) { // make a range
                int[] temp = new int[4];
                System.arraycopy(rc, 0, temp, 0, 2);
                rc = temp;
            }
     ...
     */
    protected static Ptg calcRow(Ptg[] operands)
            throws FunctionNotSupportedException {
        if (operands == null || operands.length != 1) return new PtgErr(PtgErr.ERROR_VALUE);
        if (operands[0] instanceof PtgFuncVar) {
            // we need to return the col where the formula is.
            PtgFuncVar pfunk = (PtgFuncVar) operands[0];
            try {
     ...
     */
    protected static Ptg calcRows(Ptg[] operands)
            throws FunctionNotSupportedException {
        try {
            int rsz = 0;
            if (operands[0] instanceof PtgStr) {
     ...
     */
    protected static Ptg calcVlookup(Ptg[] operands)
            throws FunctionNotSupportedException {
        ...
        try {
            ...
            if (operands.length > 3) {
                ...
                if (vx != null) {
                    try {
                        ...
                        if (bool.intValue() == 0) rangeLookup = false;
                    }
                }
            }
            ...
            PtgRef[] valueComponents = null;
            // first, get the lookup Column Vals
            if (table_array instanceof PtgName) {    // 20090211 KSC:
                PtgArea3d pa = new PtgArea3d(false);
            ...
            } else if (table_array instanceof PtgMemFunc) { //  || table_array instanceof PtgMemArea){
                try {
            ...
            if (isNumber) {
                ...
                for (int i = 0; i < lookupComponents.length; i++) {
                    ...
                    try {
                        val = Double.parseDouble(
                                lookupComponents[i].getValue().toString());
                        if (val == 0) {    // VLOOKUP does NOT treat blanks as 0's
                ...
                if (rangeLookup) return valueComponents[lookupComponents.length - 1].getPtgVal();
                else return new PtgErr(PtgErr.ERROR_NA);
            }
                ...
                if (rangeLookup) {    // approximate match
                    ...
                    for (int i = 0; i < lookupComponents.length; i++) {
                        try {
                            ...
                            if (val.equalsIgnoreCase(match_str)) {// we found it
                                return valueComponents[i].getPtgVal();
                            } else if (val.length() >= match_len && val.substring(0, match_len).equalsIgnoreCase(match_str)) { // matches up to length, but not all, return previous
                                ...
                                return valueComponents[i - 1].getPtgVal();
                            } else if (i == lookupComponents.length - 1) {// we reached the last one so use this
                                return valueComponents[i].getPtgVal();
                            }
                        } catch (Exception e) {
                    ...
                    for (int i = 0; i < lookupComponents.length; i++) {
                        try {
                            ...
                            try {
                                if (val.equalsIgnoreCase(match_str)) {// we found it
                                    return valueComponents[i].getPtgVal();
                                } else if (i == lookupComponents.length - 1) {// we reached the last one so error out
                                    return new PtgErr(PtgErr.ERROR_NA);
                                }
                            } catch (Exception e) {
        ...
        } catch (Exception e) {    // appears that an error with operands results in a #NA error
            return new PtgErr(PtgErr.ERROR_NA);
