...
public class PtgArea3d extends PtgArea implements Ptg, IxtiListener {
    ...
    public String getString() {
        try {
            if (this.getIsWholeCol() || this.getIsWholeRow()) { // handle non-standard ranges i.e. $B:$C or $1:$3
    ...
    public String getSheetName() {
        if (sheetname == null) {
            if (parent_rec != null) {
                ...
                if (sheetname == null && parent_rec != null && parent_rec.getSheet() != null) { // try this:
    ...
    public void populateVals() {
        ...
        if (parent_rec != null && parent_rec instanceof Shrfmla) {
            lastPtg.formulaRow = parent_rec.getLastRow();
            lastPtg.formulaCol = parent_rec.getLastCol();
        }
    ...
    public void setLocation(String[] s) {
        ...
        this.setWholeRowCol();
        this.updateRecord();
        // TODO: must deal with non-symmetrical absolute i.e. if first and last ptgs
        // don't match
    ...
    public Ptg[] getComponents() {
        ...
        try {
            ...
            if (!this.wholeCol && !this.wholeRow) { // normal case
                ...
                endcol = endloc[1];
                endrow = endloc[0];
            } else if (this.wholeRow) { // like $1:$1
                startcol = 0;
                try {
                    endcol = this.getSheet().getMaxCol();
                } catch (NullPointerException ne) { // can happens when Name record is being init'd and sheet records
                                                    // are not set yet
                    return null;
                }
                startrow = endrow = firstPtg.rw;
            } else if (this.wholeCol) { // like $J:$J
    ...
    /**
     * sets the column to be relative (relative is true) or absolute (relative is
     * false)
     * <br>
     * absolute references do not shift upon column inserts or deletes
     * <br>
     * NOTE: DOES NOT handle asymmetrical ranges i.e. 1st ref is absolute, 2nd is
     * relative
     *
    ...
    /**
     * sets the row to be relative (relative is true) or absolute (relative is
     * false)
     * <br>
