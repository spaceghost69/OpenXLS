...
// DOESN'T APPEAR TO MATCH EXCEL				   
    ...
    protected static Ptg calcErrorType(Ptg[] operands) {
        ...
        String s = o.toString();
        if (s.equalsIgnoreCase("#NULL!")) return new PtgInt(1);
        if (s.equalsIgnoreCase("#DIV/0!")) return new PtgInt(2);
        if (s.equalsIgnoreCase("#VALUE!")) return new PtgInt(3);
        if (s.equalsIgnoreCase("#REF!")) return new PtgInt(4);
        if (s.equalsIgnoreCase("#NAME?")) return new PtgInt(5);
        if (s.equalsIgnoreCase("#NUM!")) return new PtgInt(6);
        if (s.equalsIgnoreCase("#N/A")) return new PtgInt(7);
...
// TODO: what is correct definition of "Active Worksheets"  - hidden state doesn't seem to affect"		   
        ...
        else if (type_text.equals("origin")) {		/* Returns the absolute cell reference of the top and leftmost
...
// TODO: FINISH R1C1 reference style
        ...
        } else if (type_text.equals("osversion")) {    //Current operating system version, as text.
        ...
        } else if (type_text.equals("recalc")) {    //Current recalculation mode; returns "Automatic" or "Manual".
            if (operands[0].getParentRec().getWorkBook().getRecalculationMode() == 0) // manual
                return new PtgStr("Manual");
            return new PtgStr("Automatic");
        } else if (type_text.equals("release")) {    //Version of Microsoft Excel, as text.
    ...
    protected static Ptg calcIsna(Ptg[] operands) {
        if (operands.length != 1) return PtgCalculator.getError();
        if (operands[0] instanceof PtgErr) {
