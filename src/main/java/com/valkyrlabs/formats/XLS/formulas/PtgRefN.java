...
public class PtgRefN extends PtgRef {
    ...
    public String getLocation() {
        ...
        if (fColRel) {  // the column is a relative location
            ...
            if (realCol >= MAXCOLS) realCol -= MAXCOLS;
        }
    ...
    */
    public void setLocation(int[] rowcol) {
        ...
        if (record != null) {    // 20090217 KSC: had some errors here, redid
            if (fRwRel)
                formulaRow = rowcol[0];
            else
                rw = rowcol[0];
    ...
    */
    ...
    public PtgRef convertToPtgRef(XLSRecord r/*PtgExp pxp*/) {
        ...
        if (i[1] >= MAXCOLS_BIFF8 &&
                !r.getWorkBook().getIsExcel2007())    // TODO: determine if this is an OK maxcol (Excel 2007)
            i[1] -= MAXCOLS_BIFF8;
...
//	  	PtgRef prf = new PtgRef(s, r /*pxp.getParentRec()*/, false); //false);	 	
     ...
     * @see com.valkyrlabs.formats.XLS.formulas.PtgRef#getIntLocation()
     */
    public int[] getIntLocation() {
        ...
        if (colNew >= MAXCOLS) colNew -= MAXCOLS;    // 20070205 KSC: Added	20080102 KSC: added =
    ...
    public PtgArea getArea() {
        ...
        if (i[1] >= MAXCOLS_BIFF8 &&
                !this.parent_rec.getWorkBook().getIsExcel2007())    // TODO: determine if this is an OK maxcol (Excel 2007)
            i[1] -= MAXCOLS_BIFF8;
        if (i[3] >= MAXCOLS_BIFF8 &&
                !this.parent_rec.getWorkBook().getIsExcel2007())    // TODO: determine if this is an OK maxcol (Excel 2007)
            i[3] -= MAXCOLS_BIFF8;
