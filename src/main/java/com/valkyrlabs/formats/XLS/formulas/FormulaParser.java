...
public final class FormulaParser {
    ...
    protected static Stack getPtgsFromFormulaString(XLSRecord form, String fmla, boolean bMergeWithLast) {
        ...
        if (fmla.equals("")) {    // 20081120 KSC: Handle Missing Argument
            Stack s = new Stack();
...
// TODO: complex ranges??  	  
                ...
                if (!funcName.equals("")) {
                    ...
                    if (funcPtg != null) {    // yes, then handle function paramters i.e. evertyhing between the parentheses
                        ...
                        if (endparen < fmla.length() - 1) {
                            if (fmla.charAt(endparen + 1) == ':') {    // it's a VERY complex range :)
                                inRange = true;
                    ...
                    int endparen = getMatchOperator(fmla, i, '(', ')');
                    if (endparen == -1 || (endparen < fmla.length() - 1 && fmla.charAt(endparen + 1) == ':')) {    // it's a VERY complex range :)
                        inRange = true;
                        prefix = "(";
                        continue;
                    }
        ...
        if (!prefix.trim().equals("")) {    // get any remaining elements
            if (operands[0] == null)
                operands[0] = prefix.trim();
            else
                operands[1] = prefix.trim();
...
*/
    ...
    private static Stack parseFunctionPtg(XLSRecord form, String func, String fmla, Ptg funcPtg) {
        ...
        if (enclosing) {
            ...
            ArrayList cv = splitFunctionOperands(fmla);
            funcLen = cv.size();
            // loop through the operands to the function and recurse
            for (int y = 0; y < cv.size(); y++) {
                String s = (String) cv.get(y);        // flag as a complete expression
...
//  			return 0;
    ...
    public static int getMatchOperator(String input, int startParenLoc, char matchOpenChar, char matchCloseChar) {
        ...
        for (int i = startParenLoc; i < input.length(); i++) {
            if (input.charAt(i) == '"' || input.charAt(i) == '\'') {// handle quoted strings within input (quoted strings may of course contain match chars ...
    ...
    private static Stack parseFinalLevel(XLSRecord form, String fmla, boolean bIsComplete) {
        Stack returnStack = new Stack();
        ArrayList parseThings = new ArrayList();
        ...
        for (int x = 0; x < elements.size(); x++) {
            String val = (String) elements.get(x);
            String name = convertString(val, bk);
    ...
    */
     ...
     * @return
     */
    private static Stack convertToStack(ArrayList parseThings) {
        Stack returnStack = new Stack();
        Stack pOperators = new Stack();
        for (int x = 0; x < parseThings.size(); x++) {
            Ptg pthing = (Ptg) parseThings.get(x);
            returnStack.push(pthing);
        }
        return returnStack;
    }
     ...
     */
    private static ArrayList splitFunctionOperands(String formStr) {
        ...
        boolean loop = true;
        int pos = 0;
        ArrayList badLocs = new ArrayList();
        while (loop) {
            char c = '"';
        ...
        if (formStr.indexOf(",") == -1) {
            ...
            while (holder != -1) {
                ...
                if (i != -1) {
                    ...
                    String s = formStr.substring(placeholder);
                    locs.add(s);
                    return locs;
                }
            }
     ...
     * @return
     */
    private static ArrayList parsePtgOperators(String s, boolean bUnary) {
...
*/
     ...
     */
    private static ArrayList splitString(String formStr, boolean bIsComplete) {
        // Use a vector, and the collections methods to sort in natural order
        ArrayList locs = new ArrayList();
        ArrayList retVect = new ArrayList();
...
//					char prevc= s.charAt(s.length()-1);
    ...
    public static Formula setFormula(Formula form, String fmla, int[] rc) {
        if (fmla.charAt(0) != '{') {
            ...
            } catch (FunctionNotSupportedException e) {  // 200902 KSC: still add record if function is not found (using N/A in place of said function)
    ...
    public static String getExpressionString(Stack expression) {
        ...
        for (int t = 0; t < sz; t++) { // flip the stack
    ...
    public static void adjustParameterIds(Stack expression) {
        ...
        while (!newstck.isEmpty()) {
            ...
            if (p.getIsControl()) {
                ...
                if (p.getOpcode() == 0x15) { // its a parens... and there is a val
                    if (t > 0) {
     ...
     * @throws Exception
     */
    public static Formula getFormulaFromString(String formStr, Boundsheet st, int[] rc)
            throws Exception {
     ...
     * @throws Exception
     */
    public static Formula setFormulaString(String formStr, int[] rc)
            throws /* 20070212 KSC: FunctionNotSupported*/Exception {
        Formula f = new Formula();
        f = FormulaParser.setFormula(f, formStr, rc);
        return f;
    }
