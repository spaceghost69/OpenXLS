...
public class DateTimeCalculator {
    ...
    protected static Ptg calcDate(Ptg[] operands) {
        ...
        if (alloperands.length != 3) return PtgCalculator.getError();
                ...
                if (d != Calendar.SATURDAY && d != Calendar.SUNDAY) {
                    ...
                    if (holidays.size() > 0) {
                        for (int i = 0; i < holidays.size(); i++) {
                            if (startDate.equals(holidays.get(i))) {
                                OKtoIncrement = false;
                                break;
                            }
                        }
    ...
    protected static Ptg calcNow(Ptg[] operands) {
        ...
        // com.valkyrlabs.toolkit.Logger.log(dt.toGMTString());
    ...
    protected static Ptg calcWorkday(Ptg[] operands) {
        ...
        try {
            ...
            if (operands.length > 2 && operands[2] != null) {    // holidays
                if (operands[2] instanceof PtgRef) {
                    Ptg[] dts = operands[2].getComponents();
                    for (int i = 0; i < dts.length; i++) {
                        holidays.add(getDateFromPtg(dts[i]));
    ...
    protected static Ptg calcYearFrac(Ptg[] operands) {
        ...
        if (yf < 0) yf *= -1;    // =# days between dates, no negatives
