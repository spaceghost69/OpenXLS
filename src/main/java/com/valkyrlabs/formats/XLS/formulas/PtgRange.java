    ...
    public Ptg calculatePtg(Ptg[] form) {
        ...
        try {
            ...
            for (int k = 0; k < first.size(); k++) {
                ...
                if (sheet == null) sheet = pr.getSheetName();    // TODO: 3d ranges??????
                int[] rc = pr.getIntLocation();
                if (rc.length > 2) { // it's a range
            ...
            return perr;
        } catch (Exception e) {    // handle error ala Excel
            PtgErr perr = new PtgErr(PtgErr.ERROR_VALUE);
            return perr;
        }
    }
