...
public class MathFunctionCalculator {
    ...
    protected static Ptg calcSum(Ptg[] operands) throws CalculationException {
        if (operands.length < 1) return PtgCalculator.getNAError();
        double result = 0;
        double[] dub = PtgCalculator.getDoubleValueArray(operands);
        if (dub == null) return PtgCalculator.getNAError();
...
//	PtgNumber ptnum = new PtgNumber(dd);
    ...
    protected static Ptg calcAtan2(Ptg[] operands) throws CalculationException {
        if (operands.length != 2) return PtgCalculator.getNAError();
        double[] dd = PtgCalculator.getDoubleValueArray(operands);
        if (dd == null) return new PtgErr(PtgErr.ERROR_NA);
    ...
    protected static Ptg calcAtanh(Ptg[] operands) {
        ...
        if (dd > 1 || dd < -1) return PtgCalculator.getError();
        if (new Double(dd).isNaN()) // Not a Num -- possibly PtgErr
            return PtgCalculator.getError();
    ...
    protected static Ptg calcCeiling(Ptg[] operands) throws CalculationException {
        double[] dd = PtgCalculator.getDoubleValueArray(operands);
        if (dd == null) return new PtgErr(PtgErr.ERROR_NA);//20090130 KSC: propagate error
        if (dd.length != 2) return PtgCalculator.getNAError();
        double num = dd[0];
    ...
    protected static Ptg calcCombin(Ptg[] operands) throws CalculationException {
        ...
        if (num1 < num2) return PtgCalculator.getError();
    ...
    protected static Ptg calcExp(Ptg[] operands) {
        if (operands.length > 1 || operands[0].getComponents() != null) { // not supported by function
...
// KSC: not quite right!!!	replaced with above 
    ...
    protected static Ptg calcLCM(Ptg[] operands) {
        if (operands.length < 1) return new PtgErr(PtgErr.ERROR_NA);
        Ptg[] numbers = PtgCalculator.getAllComponents(operands);
        // algorithm:
        // LCM(a, b)= (a*b)/GCD(a, b)
        long lcm = 0;
    ...
    protected static Ptg calcLn(Ptg[] operands) {
        ...
        if (new Double(dd).isNaN()) return PtgCalculator.getError(); // Not a Num -- possibly PtgErr
...
//		Logger.logErr("MMULT: error in operands " + e.toString());
    ...
    protected static Ptg calcMRound(Ptg[] operands) {
        if (operands.length != 2) return new PtgErr(PtgErr.ERROR_NA);
        double m = 0.0, n = 0.0;
        try {
        ...
        if ((n < 0 && m > 0) || (n > 0 && m < 0)) return new PtgErr(PtgErr.ERROR_NUM);
    ...
    protected static Ptg calcMultinomial(Ptg[] operands) {
        ...
        for (int i = 0; i < numbers.length; i++) {
            ...
            if (n < 1) return new PtgErr(PtgErr.ERROR_NUM);
            sum += n;
            facts *= factorial(n);
        }
...
//	double num2 = dd[1];
    ...
    protected static Ptg calcProduct(Ptg[] operands) throws CalculationException {
        if (operands.length < 1) return PtgCalculator.getNAError();
        double[] dd = PtgCalculator.getDoubleValueArray(operands);
        if (dd == null) return new PtgErr(PtgErr.ERROR_NA);//propagate error
        double result = dd[0];
    ...
    protected static Ptg calcRoman(Ptg[] operands) {
        ...
        if (operands.length == 1) {
            ...
            int i = (int) dd;
            if (i < 0 || i > 3999) return PtgCalculator.getError(); // can't write nums that high!
...
*/
    ...
    protected static Ptg calcSumif(Ptg[] operands) {
        try {
            ...
            try {
                ...
                if (operands.length > 2) {    // see if has a sum_range; if not, source range is used for values as well as test
                ...
                for (int i = 0; i < cells.length; i++) {
                    ...
                    } catch (Exception e) {    // don't report error
    ...
    protected static Ptg calcSumproduct(Ptg[] operands) {
        ...
        for (int i = 0; i < operands.length; i++) {
            if (operands[i] instanceof PtgErr) return new PtgErr(PtgErr.ERROR_NA);    // it's what excel does
