 ...
 */
	...
	public boolean getProtected (int option) {
		...
		case FeatHeadr.ALLOWOBJECTS:
			...
			if (protect != null) return objprotect == null;
			return true; // defaults to allowed
		
		case FeatHeadr.ALLOWSCENARIOS:
			if (scenprotect != null) return scenprotect.getIsLocked();
			return true; // defaults to allowed
	...
	public void setProtected (int option, boolean value) {
		...
		case FeatHeadr.ALLOWSCENARIOS:
			if (protect != null) setScenProtect( value );
			break;
		}
	...
	private void insertObjProtect() {
		if (objprotect == null) {
			...
			sheet.insertSheetRecordAt( objprotect, (scenprotect != null ?
					(BiffRec)scenprotect : (BiffRec)protect)
						.getRecordIndex() + 1 );
		}
	}
