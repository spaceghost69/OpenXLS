...
public final class Unicodestring implements  XLSConstants, Serializable{
    ...
    */
    byte[] getEasternBytes(){
        ...
        if(data[0]==0x0)        Logger.logInfo("Unicodestring has zero length.");
        return data;
    }
    
    /** return the grbit for this UString
    */
    byte getGrbit(){return this.grbit;}
...
com.valkyrlabs.toolkit.Logger.log("Unicodestring.initEasternEncoding: Format runs are not correct");
...
// report error???
    ...
    */
    ...
    boolean isBreakable(int pos){
        ...
        if(pos>(mypos + cchExtRst+stringarray.length+offer))        return false; // in ExtRst data...
    ...
    */
    ...
    public byte[] read(){
      if (westernencoding)return this.getWesternBytes();
...
*/            		
    ...
    */
    public void updateUnicodeString(String s){
        if(s.equals(this.toString()))return;
        // 0       cch         2       Count of characters in string (NOT the number of bytes)
        // 2       grbit       1       Option flags
        ...
        }catch(UnsupportedEncodingException e){Logger.logInfo("Problem encoding string: " + e);}
        int strdatalen = strbytes.length;
        int strlen = s.length();
        byte[] blen = ByteTools.cLongToLEBytes(strlen);
        if((grbit & 0x4) == 0x4){ // it was an eastern string.  Blow that extrst out...
            grbit = (byte) (grbit & 0xFB);
            ExtRst = null;
            offer -= 4;
            westernencoding = true;
        } 
        if ((grbit & 0x8)==0x8) { // had formatting runs - remove (since cannot keep with new string)
        	formatlen= 0;
        	fRichSt= false;
        	grbit ^= 0x8;
        	offer-= 2;
    ...
    public boolean hasFormattingRuns() {
        if (numformattingruns>0)return true;
        return false;
    }
