 ...
 * @see Note
 */
public final class Note extends com.valkyrlabs.formats.XLS.XLSRecord {
    ...
    private final byte[] PROTOTYPE_BYTES = new byte[]{0, 0,    /* row */
            0, 0,    /* col */
            0,        /* hidden state */
            0,        /* unknown */
            1, 0,    /* ordinal id */
    ...
    private Txo getAssociatedTxo() {
        ...
        int idx = -1;
        if (bs != null) {// shouldn't!
            idx = bs.getIndexOf(OBJ);
            if (idx == -1) return null;    // should't!
            while (idx < bs.getSheetRecs().size()) {
                if (((BiffRec) bs.getSheetRecs().get(idx)).getOpcode() == OBJ) {
                    ...
                    if (o.getObjType() == 0x19 && o.getObjId() == this.id) { // got it!
                        // now find the next TXO
                        idx++;
                        while (idx < bs.getSheetRecs().size() &&
                                (((BiffRec) bs.getSheetRecs().get(idx))).getOpcode() != TXO)
                            idx++;
                        break;
                    }
                }
            ...
            while (idx < bs.getSheetRecs().size()) {
                if (((BiffRec) bs.getSheetRecs().get(idx)).getOpcode() == OBJ) {
                    ...
                    if (o.getObjType() == 0x19 && o.getObjId() == this.id) { // got it!
                        ...
                        else if (opcodenext == CONTINUE) {// continue masking mso - AFTER object record
                            if (idx + 5 < bs.getSheetRecs().size())
                                return (((Continue) bs.getSheetRecs().get(idx + 5))).maskedMso;    // the 1st mso is actually at position 5: Obj (note)/Continue/Txo/Continue/Continue/Continue
                        }
                    }
                }
                idx++;
