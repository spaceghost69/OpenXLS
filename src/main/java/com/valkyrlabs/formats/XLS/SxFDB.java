...
public class SxFDB extends XLSRecord implements XLSConstants, PivotCacheRecord {
	...
	*/
	...
	public void init(){
        ...
        // grouping-related
        ifdbParent= ByteTools.readShort(this.getByteAt(2),this.getByteAt(3));	// specifies the cache field index, as specified by Cache Fields, of the grouping cache field for this cache field
        ifdbBase= ByteTools.readShort(this.getByteAt(4),this.getByteAt(5));		// specifies the cache field index, as specified by Cache Fields, of the base cache field
        //ignored:  citmUnq= ByteTools.readShort(this.getByteAt(6),this.getByteAt(7)); // "Undefined and MUST be ignored."
        csxoper= ByteTools.readShort(this.getByteAt(8),this.getByteAt(9));		// specifies the number of cache items in this cache field when this cache field is a grouping cache field,
        cisxoper= ByteTools.readShort(this.getByteAt(10),this.getByteAt(11));	// specifies the number of cache items in the base cache field that are grouped by this cache field
        ...
        if (cch > 0) {        // 0xFFFF if none
            ...
            }catch(UnsupportedEncodingException e){Logger.logInfo("SXString.init: " + e);}
        }
...
//	Logger.logWarn("SXFDB: not all cache items are used");        
	...
	public String toString() { 
		return String.format("SXFDB -p: %d d: %d csxoper: %d cisxoper: %d catm: %d stFieldName: %s fAllAtoms? %b text? %b num? %b calculated? %b", ifdbParent, ifdbBase, csxoper, cisxoper, catm, stFieldName, fAllAtoms, fTextEtcField, fNumField, fCalculatedField);
	}
	   /**
...
//				1, 20,    /* grbit 5121 -- fAllAtoms no dates or nums...*/  
	    ...
	    * @return
	    */
	   public int getNCacheItems() { return catm; }
	   ...
	   public void setCacheField(String s) {
		   ...
		   byte[] strbytes= new byte[0];
			if (stFieldName!=null) {
			    ...
			    }catch(UnsupportedEncodingException e){Logger.logInfo("SxFDB: " + e);}
			}
	    ...
	    */  
	   public String getCachefield() { return stFieldName; }
