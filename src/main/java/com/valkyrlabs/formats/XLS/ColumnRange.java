...
public interface ColumnRange {
	...
	implements ColumnRange, Serializable {
	...
	implements java.util.Comparator<ColumnRange>, Serializable {
		...
		public int compare (ColumnRange r1, ColumnRange r2) {
		    ...
		    if ((single1 || single2) && single1 != single2) { // XOR
		    	...
		    	if (range.getColFirst() <= single.getColFirst()
		    			&& range.getColLast() >= single.getColLast() ) {
		    		
		    		// if it's a range boundary, it chooses what it is
		    		if (single instanceof CellAddressible.RangeBoundary) {
		    			int value = ((CellAddressible.RangeBoundary)
		    							single).comareToRange();
