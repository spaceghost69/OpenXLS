...
public final class MSODrawing extends com.valkyrlabs.formats.XLS.XLSRecord 
{	private static final long serialVersionUID = 8275831369787287975L;
    ...
    public byte[] createRecord(int spid, String imageName, String shapeName, int imageIndex) {
    	...
    	int pos =0;
    	System.arraycopy(msofbtSpContainer1Bytes,0,retData,pos,msofbtSpContainer1Bytes.length);pos+=msofbtSpContainer1Bytes.length;
    	System.arraycopy(msofbtSp1Bytes,0,retData,pos,msofbtSp1Bytes.length);pos+=msofbtSp1Bytes.length;
    	System.arraycopy(msofbtOPT1Bytes,0,retData,pos,msofbtOPT1Bytes.length);pos+=msofbtOPT1Bytes.length;
    	System.arraycopy(msofbtClientAnchor1Bytes,0,retData,pos,msofbtClientAnchor1Bytes.length);pos+=msofbtClientAnchor1Bytes.length;
    	System.arraycopy(msofbtClientData1Bytes,0,retData,pos,msofbtClientData1Bytes.length);pos+=msofbtClientData1Bytes.length;	// 20100420 KSC: empty client data record- necessary???
    	
    	if (bIsHeader){	//This is only present in the first msodrawing per sheet    		    		
            if (lastSPID < SPID) 
            	lastSPID= SPID;		// TODO: Shouldn't assume to be SPID
    		int totalSPRECORDS = 0;    
    	
    		// Header also contains Shape Id Seed SP record
    		MsofbtSp msofbtSp = new MsofbtSp(MSODrawingConstants.MSOFBTSP,MSODrawingConstants.msosptMin,2);  //1st shape rec is of type MSOSPTMIN 
    		...
    		pos =0;
        	System.arraycopy(msofbtDgContainerBytes,0,headerRec,pos,msofbtDgContainerBytes.length);pos+=msofbtDgContainerBytes.length;
        	System.arraycopy(msofbtDgBytes,0,headerRec,pos,msofbtDgBytes.length);pos+=msofbtDgBytes.length;
        	System.arraycopy(msofbtSpgrContainerBytes,0,headerRec,pos,msofbtSpgrContainerBytes.length);pos+=msofbtSpgrContainerBytes.length;
        	System.arraycopy(msofbtSpContainerBytes,0,headerRec,pos,msofbtSpContainerBytes.length);pos+=msofbtSpContainerBytes.length;
        	System.arraycopy(msofbtSpgrBytes,0,headerRec,pos,msofbtSpgrBytes.length);pos+=msofbtSpgrBytes.length;
        	System.arraycopy(msofbtSpBytes,0,headerRec,pos,msofbtSpBytes.length);pos+=msofbtSpBytes.length;
		...
		if (isHeader()) {	  	
			...
			if (d0+d1!=0) {
				if (DGCONTAINERLENGTH!=(DGCONTAINERATOMS+SPGRCONTAINERLENGTH+SOLVERCONTAINERLENGTH+8)) {  // this may not be 100% since must account for OTHER record's SOLVERCONTAINER LENGTHS
					com.valkyrlabs.toolkit.Logger.log("DGCONTAINERLENGTH DIFF: " + (DGCONTAINERLENGTH-(DGCONTAINERATOMS+SPGRCONTAINERLENGTH+SOLVERCONTAINERLENGTH+8)));
					diff= true;
				}			  	
				// ******* sum of SPCONTAINERS ***************
				if (SPGRCONTAINERLENGTH+8!=SPCONTAINERLENGTH+otherSPCONTAINERLENGTH) {
					com.valkyrlabs.toolkit.Logger.log("SPGRCONTAINERLENGTH DIFF: " + (SPGRCONTAINERLENGTH+8-(SPCONTAINERLENGTH+otherSPCONTAINERLENGTH)));
					diff= true;
				}
			}
		}
...
/*// debug: check algorithm:
	com.valkyrlabs.toolkit.Logger.log(this.toString());    	
	com.valkyrlabs.toolkit.Logger.log(this.debugOutput());
...
/**/
	    ...
	    super.getData();
	  	...
	  	for(;bis.available()>0;){
			...
			if ((0xF&header[0])==15) {	// 15 for containers, version for atoms
				...
				switch(fbt){		
					...
					case MSODrawingConstants.MSOFBTSP:  			// A shape atom rec (inst= shape type) rec= shape ID + group of flags
						...
						if (flag!=5) { 	// if it's not the SPIDseed, update SPID
							System.arraycopy(ByteTools.cLongToLEBytes(SPID), 0, data, 0, 4);
						}
						if (fHaveSpt) {	// shape type is contained within inst var
...
*/		
    	...
    	if (bIsHeader) {
    		...
    		int pos =0;
        	System.arraycopy(dgcontainer,0,header,pos,dgcontainer.length);pos+=dgcontainer.length;
        	System.arraycopy(dgcontaineratoms,0,header,pos,dgcontaineratoms.length);pos+=dgcontaineratoms.length;
        	System.arraycopy(spgrcontainer,0,header,pos,spgrcontainer.length);pos+=spgrcontainer.length;
        	System.arraycopy(container2,0,header,pos,container2.length);pos+=container2.length;
        	System.arraycopy(spcontainer2atoms,0,header,pos,spcontainer2atoms.length);pos+=spcontainer2atoms.length;
    ...
    public void addHeader() {
		...
		int pos =0;
    	System.arraycopy(msofbtDgContainerBytes,0,headerRec,pos,msofbtDgContainerBytes.length);   pos+=msofbtDgContainerBytes.length;// 8
    	System.arraycopy(msofbtDgBytes,0,headerRec,pos,msofbtDgBytes.length);                     pos+=msofbtDgBytes.length;
    	System.arraycopy(msofbtSpgrContainerBytes,0,headerRec,pos,msofbtSpgrContainerBytes.length);pos+=msofbtSpgrContainerBytes.length;// 8
    	System.arraycopy(msofbtSpContainerBytes,0,headerRec,pos,msofbtSpContainerBytes.length);   pos+=msofbtSpContainerBytes.length;// 8
    	System.arraycopy(msofbtSpgrBytes,0,headerRec,pos,msofbtSpgrBytes.length);                 pos+=msofbtSpgrBytes.length;
    	System.arraycopy(msofbtSpBytes,0,headerRec,pos,msofbtSpBytes.length);                     pos+=msofbtSpBytes.length;
    	...
    	*/
	    ...
	    super.getData();
	  	...
	  	for(;bis.available()>0;){
		    ...
		    fbt = ((0xFF&header[3])<<8)|(0xFF&header[2]);
		    len = ByteTools.readInt(header[4], header[5], header[6], header[7]);
			if ((0xF&header[0])!=15) {	// skip containers
			    // parse atoms or sub-records (distinct from container records above)
				byte[] data= new byte[len];
				bis.read(data,0,len);
				switch(fbt){		
    ...
    public void updateHeader(int otherSPContainers, int otherContainers, int numShapes, int lastSPID) {
	  	...
	  	for(;bis.available()>0;){
		    ...
		    if (fbt==MSODrawingConstants.MSOFBTDGCONTAINER) {
		    	...
		    	;	// skip containers
	  			...
	  			if (hasSPGRCONTAINER) {	// already has the 1 required SPGRCONTAINER, if more than 1, multiple groups
		...
		/*if (origSPGRL!=spgrcontainerlength || origDGL!=dgcontainerlength) {			
			com.valkyrlabs.toolkit.Logger.log(this.toString());
			com.valkyrlabs.toolkit.Logger.log(this.debugOutput());
			com.valkyrlabs.toolkit.Logger.log("ORIGDG=" + origDGL + " ORIGSPL=" + origSPGRL + " DIFF: " + (origDGL-dgcontainerlength) + "-" + (origSPGRL-spgrcontainerlength));
			diff= true;
		}
		/**/
	    ...
	    super.getData();
	  	...
	  	for(;bis.available()>0;){
		    ...
		    } else if (fbt==MSODrawingConstants.MSOFBTDG)  {  // Drawing Record: count + MSOSPID seed 
				buf= new byte[len];
				bis.read(buf,0,len);
	  	...
	  	for(;bis.available()>0;){
		    ...
		    if (fbt==MSODrawingConstants.MSOFBTOPT) {
				...
				for (int i= 0; i < n; i++) {
					...
					if (propertyId==MSODrawingConstants.msooptpib)	{// blip to display = image index						
     ...
     */
    public boolean isHeader() { return bIsHeader; }
     ...
     * @return
     */
    public int getDrawingId() { return drawingId; }
    /**
...
*/		
     ...
     * @return
     */
    public short getOriginalWidth() { return origWidth;    }
    ...
    * @return
    */    
   public short getHeight() { return origHeight; }
   ...
   public void setWidth(int w) {
	   ...
	   if (z >= w) {	// 20100322 KSC: was > w, so the == case never was handled, see TestImages.insertImageOffsetDisappearance bug
		   col1= col;
    ...
    */
    public void setHeight(int h) {
		...
		for (int i= row+1; y < h; i++) {
			...
			if (y+rh < h)
				y+= rh;
			else {		// height is met; see what offset into row i is necessary
				row1= i;
	...
	private double getRowHeight(int row) {
		...
		} catch (Exception e) {	// exception if no row defined // no row defined - use default row height 20100504 KSC		
				return this.getSheet().getDefaultRowHeight(); // default
		}
...
//        mso.updateRecord();
    ...
    protected static XLSRecord getTextBoxPrototype() {
        MSODrawing mso = new MSODrawing();
        mso.setOpcode(MSODRAWING);
        mso.setData(new byte[] {0, 0, 13, -16, 0 ,0, 0, 0});	// only contains 1 sub-record:  0xF00D or 61453= msofbtTextBox==shape has attached text
        mso.init();
        return mso;
    }
    ...
    private void updateDGRecord() {
	  	...
	  	for(;bis.available()>0;){
		    ...
		    buf = new byte[len];
			bis.read(buf,0,len);	// position at next sub-rec 
		    if (fbt==MSODrawingConstants.MSOFBTDG)  {  // Drawing Record: count + MSOSPID seed 
    ...
    private void updateClientAnchorRecord(short[] bounds) {
	  	...
	  	for(;bis.available()>0;){
		    ...
		    buf = new byte[len];
			bis.read(buf,0,len);	// position at next sub-rec 
			if (fbt==MSODrawingConstants.MSOFBTCLIENTANCHOR) {		// Anchor or location fo a shape
				// udpate bounds
				...
				if (buf.length==newrec.length) {// should!!!
					// update Msodrawing data ...
					System.arraycopy(newrec, 0, data, data.length-bis.available()-newrec.length, newrec.length);				
				}					
				else
    ...
    public void setOPTSubRecord(int propertyId, boolean isBid, boolean isComplex, int dtx, byte[] complexBytes) {
	    ...
	    updateRecord();	
	    if (origlen!=optrec.toByteArray().length) {	// must update header
...
//					}
    ...
    public String toString(){
    	...
    	sb.append("Msodrawing: image=" + this.imageName + ".\t" + this.shapeName + "\timageIndex=" + imageIndex + " sheet=" + ((this.getSheet()!=null)?this.getSheet().getSheetName():"none"));
    ...
    public void createDropDownListStyle(int col) {
	    ...
	    this.updateRecord(++this.wkbook.lastSPID, null, null, -1, new short[] {(short)col, 0, 0, 0, (short)(col+1), 0, 1, 0});  // generate msoDrawing using correct values moved from above
    ...
    public void createCommentBox(int row, int col) {	
	    ...
	    this.updateRecord(++this.wkbook.lastSPID, null, null, -1, new short[] {(short)(col+1), 240, (short) row, 30, (short)(col+3), 496, (short) (row+4), 196});  // generate msoDrawing using correct values moved from above
    }
...
//		    if (fbt <= 0xF005) { // ignore containers
	  		if (version==15) { // it's a container - no parsing
		    		// MSOFBTSPGRCONTAINER:		// Shape Group Container, contains a variable number of shapes (=msofbtSpContainer) + other groups 0xF003 
		    		// MSOFBTSPCONTAINER:		// Shape Container 0xF004
		    	if (fbt==MSODrawingConstants.MSOFBTDGCONTAINER) {
