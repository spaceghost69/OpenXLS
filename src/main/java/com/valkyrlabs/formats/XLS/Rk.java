...
public final class Rk
        extends XLSCellRecord implements Mulled {
...
// NOTE isFPNumber, isIntNumber was set in translateRk anyways so take out of here
        ...
        switch (RKType) {
            ...
            default:
                Logger.logInfo("incorrect RK type for RK record: " + RKType);
        }
        return 0.0;
    }
...
*/
    ...
    void translateRK(byte[] rkval) {
        ...
        if (DEBUG) Logger.logInfo(String.valueOf(rkType));
        if (DEBUG) Logger.logInfo(String.valueOf(Rkdouble));
        ...
        switch (rkType) {
            ...
            case Rk.RK_INT_100:
                ...
                try {
                    if (Long.parseLong(newnum) > 0) { // there's FP digits
                        this.isFPNumber = true;
                        this.isIntNumber = false;
                    } else {
                        this.isFPNumber = false;
...
//					they should be ints
    ...
    public int getRecordIndex() {
        if (super.getRecordIndex() < 0) { // this is a MulRk
            if (mymul == null) return -1; // throw new InvalidRecordException("Rk without a recidx nor a Mulrk.");
    ...
    /**
     * @see com.valkyrlabs.formats.XLS.XLSRecord#setIntVal(int)
     */
    ...
    public double getDblVal() {
        if (isIntNumber) return RKint;
        return Rkdouble;
    }

    public float getFloatVal() {
        if (isIntNumber) return (float) RKint;
        return (float) Rkdouble;
    }

    /**
     * @see com.valkyrlabs.formats.XLS.XLSRecord#setFloatVal(float)
     */
    ...
    /**
     * @see com.valkyrlabs.formats.XLS.XLSRecord#setDoubleVal(double)
     */
