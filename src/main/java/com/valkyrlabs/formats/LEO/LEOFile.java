...
public class LEOFile implements Serializable {
    ...
    public final static int MAXDIFATLEN = 109;                    // maximum
    // DIFATLEN
    // =
    // 109;
    // if
    ...
    // into
    // extraDIFAT
    public final static int IDXBLOCKSIZE = 128;                    // number
    // of
    // indexes
    // that
    // can
    ...
    final static void initSmallBlockIndex(int[] newidx, Block b) {
        while (b.hasNext() && !(b == b.next())) {
            ...
            if (origps < 0) {
                Logger.logWarn("WARNING: LEOFile Block Not In MINIFAT vector: "
                        + b.getOriginalIdx());
            } else {
    ...
    public final static FileBuffer readFile(File fpath) {
        ...
        String tmpfu = (String) System.getProperties()
                .get("com.valkyrlabs.formats.LEO.usetempfile");
     ...
     *
     * <br> By default, OpenXLS will lock open WorkBook files, to close the file after parsing and work with a
     * temporary file instead, use the following setting:
     * <br><br>
     * System.getProperties().put("com.valkyrlabs.formats.LEO.usetempfile", "true");
     * <br><br>
    ...
    public final static void getBytes(List outblocks, OutputStream out) {
        ...
        while (it.hasNext()) {
            ...
            try {
                ...
                Logger.logWarn("ERROR: gettting bytes from blocks failed: "
                        + a);
            }
    ...
    public synchronized List writeBytes(OutputStream out, int workbook_byte_size) {
        ...
        int miniFATStart = -2; // start block position of short sector container
        // blocks (-2 for none)
        int sbidxpos = -2; // start position of short sector idx chain (-2 for
        // none)
        int sbsz = 0; // size of short sector container
        int[] DIFAT; // holds the sector id chains for the 1st 109 sector index
        // blocks (DIFAT)
        int[] extraDIFAT; // holds the sector id chains for any sector index
        // blocks (DIFAT) > 109
        ...
        Iterator<Storage> e = (Iterator<Storage>) directories.getAllDirectories().iterator();
        while (e.hasNext()) {
            Storage thisStore = (Storage) e.next();
            if (thisStore != book && thisStore != rootStore) { // &&
                // thisStore!=encryptionInfo?
                storages.add(thisStore);
                if (thisStore.getBlockType() == Block.SMALL) { // count number
                    // of miniStream
                    // blocks
        ...
        if (numMiniFATSectors > 0) {
            Storage[] sbs = buildMiniFAT(storages, numMiniFATSectors); // returns
            // two
            // non-directory
            // storages:
            // miniStream
            ...
            // index
            storages.add(sbs[0]);
            numMiniFATSectors = sbs[1].getBlockVect().size(); // trap number of
            // miniStream
            // block indexes
            storages.add(sbs[1]);
        }
        ...
        numblocks += Math.ceil(directories.directoryVector.size() / 4); // each
        // 4
        // directories==1
        // block
        // (512
        ...
        for (int t = 0; t < storages.size(); t++) {
            Storage nstr = (Storage) storages.get(t); // saved stores containt
        ...
        int workbook_idx_block_size = LEOFile
                .getSizeInBlocks(workbook_byte_size, BIGBLOCK.SIZE);
        workbook_idx_block_size = Math
                .max(workbook_idx_block_size, getMinBlocks() - 1);// ensure
        // minimum #
        // blocks
        ...
        // or FAT/miniFAT
        DIFAT = new int[Math.min(numFATSectors, MAXDIFATLEN)]; // 1 up to the
        // 1st 109
        // sector index
        // blocks used
        // to describe
        // this file
        extraDIFAT = new int[Math.max(numFATSectors - MAXDIFATLEN, 0)]; // if
        // necessary,
        // sector
        // index
        // blocks
        ...
        if (!isEncrypted) {
            ...
            book.setActualFileSize((workbook_idx_block_size + 1)
                    * BIGBLOCK.SIZE);
            if (DEBUG)
                Logger.logInfo("Workbook actual bytes: "
                        + book.getActualFileSize());
        }
        ...
        for (int t = 0; t < storages.size(); t++) {
            ...
            if (!nstr.miniStreamStorage) { // miniStream blocks are handled
                // separately
                if (nstr.getName().equals("miniStream")) { // miniStream (short
                    // sector container)
                    miniFATStart = blockpos; // this goes in rootstore
                    // startblock to denote start of
                    // short sector container
                    sbsz = nstr.getActualFileSize(); // actual size of short
                    // sector (miniFAT)
                    // container - stored in
                    // root storage
                } else if (nstr.getName().equals("miniFAT")) { // miniFAT (short
                    // sector) index
                    sbidxpos = blockpos;
                }
                ...
                if (blks != null) {
                    ...
                    for (int i = 0; i < blks.length - 1; i++) { // -1 to trap
                        // last block
                        // with special
                        // end-of-block
                        // flag (-2)
        ...
        rootStore.setStartBlock(miniFATStart); // root store start points to
        ...
        // storages
        rootStore.setBytes(directories.rebuildRootStore()); // now rebuild
        // rootstore from
        // all other
        // storages
        rootstart = blockpos;
        ...
        if (numFATSectors > MAXDIFATLEN) { // handle extra blocks - need extra
            ...
            for (int i = 0; i < (int) Math.ceil((numFATSectors - MAXDIFATLEN)
                    / (IDXBLOCKSIZE * 1.0)); i++)
        ...
        Block[] FATSectors = getIDXBlocks(FAT); // convert each int to 4-bytes
        // in sector/block format -->
        // FAT Sectors
        idxstore.setBlocks(FATSectors); // input the FAT Sectors to a
        ...
        if (numFATSectors > MAXDIFATLEN) {
            ...
            // the FAT
            extraDIFATStart = blockpos - (int) Math
                    .ceil((numFATSectors - MAXDIFATLEN) / (IDXBLOCKSIZE * 1.0));
            Storage xbbstore = buildExtraDIFAT(extraDIFAT, extraDIFATStart); // create
            // block(s)
            // from
            // the
            // extraDIFAT
        ...
        if (!header.init())
            throw new RuntimeException("LEO File Header Not Initialized"); // invalid
        // WorkBook
        // File
    ...
    private Storage buildExtraDIFAT(int[] difatidx, int xbbpos) {
        ...
        if (difatidx != null) {
            ...
            for (int i = 0; i < xbytes.length; ) {
                ByteBuffer bl = BlockFactory.getPrototypeBlock(Block.BIG)
                        .getByteBuffer();
    ...
    private Storage[] buildMiniFAT(List storages, int numsbs) {
        ...
        int[] sbidx = new int[(int) Math.ceil(numsbs / (IDXBLOCKSIZE * 1.0))
                * IDXBLOCKSIZE];
        byte[] smallblocks = new byte[0]; // sum up all short sectors, will be
        ...
        for (int i = 0; i < storages.size(); i++) {
            ...
            if (thisStore.getBlockType() == Block.SMALL) {
                ...
                thisStore.setStartBlock(z); // set start position of miniFATs as
                // may have changed
                ...
                smallblocks = ByteTools
                        .append(thisStore.getBytes(), smallblocks);
        ...
        // block(s):
        Block[] smallBlocksToBig = BlockFactory
        ...
        Block[] sbIDX = getIDXBlocks(sbidx); // convert each int to 4-bytes in
    ...
    public Storage[] getAllDirectories() {
        ArrayList v = directories.getAllDirectories();
    ...
    public synchronized int[] init(ByteBuffer bbuf) {
        int pos = 0;
        ArrayList FATSectors = new ArrayList(); // one or more
        // sectors which
        // hold the FAT
        // (File
        // Allocation
        ...
        if (!header.init(bbuf))
            throw new InvalidFileException(
                    this.getFileName() + " is not a valid OLE File.");

        if (DEBUG) {
            Logger.logInfo("Header: ");
            Logger.logInfo("numbFATSectors: " + header.getNumFATSectors());
            Logger.logInfo("numMiniFATSectors: "
                    + header.getNumMiniFATSectors());
            Logger.logInfo("numbExtraDIFATSectors: "
                    + header.getNumExtraDIFATSectors());
    ...
    private ArrayList getFATSectors() {
        ArrayList FATSectors = new ArrayList();
        int[] DIFAT = header.getDIFAT(); // the Index for the FAT(which is the
        ...
        for (int x = 0; x < numExtraDIFATSectors; x++) {
            ...
            int numExtraSectorElements = Math
                    .min((header.getNumFATSectors() - blockidx), IDXBLOCKSIZE);
            for (int i = 0; i < numExtraSectorElements; i++) {
                if (i < (IDXBLOCKSIZE - 1)) { // -1 because must always have
                    // room for end of sector mark
                    int bbloc = extraSectorBytes.getInt() + 1;
                    if (bbloc <= bsz) {
                        BIGBLOCK bbd = (BIGBLOCK) bigBlocks.get(bbloc); // fails
                        // at
                        // 41k
                        // recs
                        // jpm
