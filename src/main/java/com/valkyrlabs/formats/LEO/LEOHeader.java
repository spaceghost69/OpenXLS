...
public class LEOHeader implements Serializable {
    ...
    private static final int DIFATPOSITION = 76;    // DIFAT start position WITHIN HEADER (DIFAT=where to find the FAT i.e the index to the FAT)
    ...
    private static void displayHeader(ByteBuffer data) {
        for (int i = 0; i < data.capacity(); i++) {
            com.valkyrlabs.toolkit.Logger.log("[" + i + "] " + data.get(i));
        }
    }
    ...
    static final LEOHeader getPrototype(int[] FAT) {
        ...
        BIGBLOCK retblock =
                (BIGBLOCK) BlockFactory.getPrototypeBlock(Block.BIG);
