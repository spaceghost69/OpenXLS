 ...
 * with a UTF-16 terminating null character. Thus storage and stream names are
 * limited to 32 UTF-16 code points, including the terminating null character.
 ...
 * mapping, described in Red-Black Tree. The following characters are illegal
 * and MUST NOT be part of the name: '/', '\', ':', '!'.
 * Directory Entry Name Length (2 bytes): This field MUST match the length of
 * the Directory Entry Name Unicode string in bytes. The length MUST be a
 * multiple of 2, and include the terminating null character in the count. This
 * length MUST NOT exceed 64, the maximum size of the Directory Entry Name
 * field.
 * Object Type (1 byte): This field MUST be 0x00, 0x01, 0x02, or 0x05, depending
 * on the actual type of object. All other values are not valid.
 * 0= Unknown or unallocated
 * 1= Storage Object
 * 2- Stream Object
 * 5= Root Storage Object
 * Color Flag (1 byte): This field MUST be 0x00 (red) or 0x01 (black). All other
 * values are not valid.
 * Left Sibling ID (4 bytes): This field contains the Stream ID of the left
 * sibling. If there is no left sibling, the field MUST be set to NOSTREAM
 * (0xFFFFFFFF).
 * Right Sibling ID (4 bytes): This field contains the Stream ID of the right
 * sibling. If there is no right sibling, the field MUST be set to NOSTREAM
 * (0xFFFFFFFF).
 * Child ID (4 bytes): This field contains the Stream ID of a child object. If
 * there is no child object, then the field MUST be set to NOSTREAM
 * (0xFFFFFFFF).
 * CLSID (16 bytes): This field contains an object classGUID, if this entry is a
 * storage or root storage. If there is no object class GUID set on this object,
 * then the field MUST be set to all zeroes. In a stream object, this field MUST
 * be set to all zeroes. If not NULL, the object class GUID can be used as a
 * parameter to launch applications.
 * State Bits (4 bytes): This field contains the user-defined flags if this
 * entry is a storage object or root storage object. If there are no state bits
 * set on the object, then this field MUST be set to all zeroes.
 * Creation Time (8 bytes): This field contains the creation time for a storage
 * object. The Windows FILETIME structure is used to represent this field in
 * UTC. If there is no creation time set on the object, this field MUST be all
 * zeroes. For a root storage object, this field MUST be all zeroes, and the
 * creation time is retrieved or set on the compound file itself.
 * Modified Time (8 bytes): This field contains the modification time for a
 * storage object. The Windows FILETIME structure is used to represent this
 * field in UTC. If there is no modified time set on the object, this field MUST
 * be all zeroes. For a root storage object, this field MUST be all zeroes, and
 * the modified time is retrieved or set on the compound file itself.
 * Starting Sector Location (4 bytes): This field contains the first sector
 * location if this is a stream object. For a root storage object, this field
 * MUST contain the first sector of the mini st ream, if the mini stream exists.
 * Stream Size (8 bytes): This 64-bit integer field contains the size of the
 * user-defined data, if this is a stream object. For a root storage object,
 * this field contains the size of the mini stream.
 * <p>
 * ??
...
public class Storage extends BlockByteReader {
    ...
    String name = "";
    int nameSize = -1;
    AbstractList idxs = new ArrayList();
    ...
    protected Storage(ByteBuffer buff) {
        ...
        if (LEOFile.DEBUG)
            Logger.logInfo("Storage: " + name + " storageType: " + storageType + " directoryColor:" + directoryColor +
                    " prevSID:" + prevStorageID + " nextSID:" + nextStorageID + " childSID:" + childStorageID + " sz:"
                    + sz);
    }
    ...
    public void setBytesWithOverage(byte[] newbytes) {
        ...
        if (newbytes.length < StorageTable.BIGSTORAGE_SIZE) { // usual case
     ...
     * (obtained by walking the miniFAT sector index to reference blocks in the
     * miniStream
     */
    public void initFromMiniStream(List miniStream, int[] miniFAT) throws LEOIndexingException {
            ...
            switch (idx) {
                ...
                default:
                    if (idx >= miniStream.size()) {
                        ...
                        thisBlock = (Block) miniStream.get(idx); // miniFAT is 0-based (no header sector at position 0
                                                                 // as in regular FAT)
        ...
        if (LEOFile.DEBUG) {
            if ((int) Math.ceil(this.getActualFileSize() / 64.0) != myblocks.size())
                Logger.logErr("Number of miniStream Sectors does not equal storage size.  Expected: "
                        + (int) Math.ceil(this.getActualFileSize() / 64.0) + ". Is: " + myblocks.size());
        }
     ...
     * (obtained by walking down the FAT sector index chain to obtain blocks from
     * the dta block store)
     * <p>
            ...
            switch (nextIdx) {
                ...
                default: // normal block
                    ...
                    if (nextIdx != i + 1) {
                        ...
                        if (LEOFile.DEBUG)
                            Logger.logInfo(
                                    "INFO: Storage init: jumper skipping: "
                                            + i);
                        Block skipbb = (Block) dta.get(i + 1);

                        this.addBlock(skipbb); //
                    } else if (!thisbb.getIsSpecialBlock()) { // just skip as probably a bbdix in the midst of the secid
                                                              // chain
                        this.addBlock(thisbb); //
