...
public class StorageTable implements Serializable {
    ...
    ArrayList directoryVector = new ArrayList();    // all directories in the LEO file
    ...
    public final static void writeitout(List blocks, String name) {
        try {
            FileOutputStream fos =
                    new FileOutputStream(new File(System.getProperty("user.dir") + "\\storages\\" + name));
    ...
    public void initDirectories(List blockvect, int[] FAT) {
        Iterator<Storage> rcs = directoryVector.iterator();
        List miniStream = null;
        int totrecsize = 0;

        while (rcs.hasNext()) {
            Storage rec = (Storage) rcs.next();
    ...
    public List initMiniStream(List blockvect, int[] FAT) {
        ...
        if (miniFATSectors.size() > 0) {
            try {
                ...
                for (int i = 0; i < miniStreamBlocks.length; i++) {    // should equal sbbsize
...
*/
        ...
        if (insertIdx == -1)
            ...
            for (int i = 0; i < directoryVector.size(); i++) {    // adjust prev, next, child ids if necessary
                Storage s = (Storage) directoryVector.get(i);
        ...
        if (LEOFile.DEBUG)
            Logger.logInfo(
                    "Initializing Storage: " + name + " Retrieving Data." +
                            ...
                            + blocktype
                            + " startidx: "
                            + rec.getStartBlock() + (rec.getBlockType() == Block.SMALL ? " MiniFAT" : ""));
...
//									"directoryStorage.dat");
    ...
    byte[] rebuildRootStore() {
         ...
         */
        while ((directoryVector.size() % 4) != 0) { // add "null" storages to ensure multiples of 4 (128*4=512==minimum size)
            this.createStorage("", 0, -1);
        }
        Iterator e = directoryVector.iterator();
        byte[] bytebuff = new byte[directoryVector.size() * DIRECTORY_SIZE];
        int pos = 0;
        while (e.hasNext()) {
            Storage s = (Storage) e.next();
            ByteBuffer buff =
                    this.getDirectoryHeaderBytes(s);
    ...
    /**
     * get the ArrayList of all Directories within the LEO file
     */
    ArrayList getAllDirectories() {
    ...
    public void DEBUG() {
        com.valkyrlabs.toolkit.Logger.log("DIRECTORY CONTENTS:");
        for (int i = 0; i < directoryVector.size(); i++) {
            Storage s = (Storage) directoryVector.get(i);
            String n = s.getName();
            Logger.logInfo("Storage: " + n + " storageType: " + s.getStorageType() + " directoryColor:" + s.getDirectoryColor() +
                    " prevSID:" + s.getPrevStorageID() + " nextSID:" + s.getNextStorageID() + " childSID:" + s.getChildStorageID() + " sz:" + s.getActualFileSize());
            // special storages
            if (n.equals("Root Entry")) {
                ...
                if (s.myblocks != null) {
                    int zz = 0;
                    if ((s.myblocks.get(zz) instanceof com.valkyrlabs.formats.LEO.BIGBLOCK))
                        com.valkyrlabs.toolkit.Logger.log("BLOCK 1:\t" + zz + "-" + Arrays.toString(((com.valkyrlabs.formats.LEO.BIGBLOCK) s.myblocks.get(zz)).getBytes()));
                    else
                        com.valkyrlabs.toolkit.Logger.log("BLOCK 1:\t" + zz + "-" + Arrays.toString(((com.valkyrlabs.formats.LEO.SMALLBLOCK) s.myblocks.get(zz)).getBytes()));
                }
                ...
                BiffRec rec = new com.valkyrlabs.formats.XLS.XLSRecord();        // 4 bytes are header ...
                ...
                if (slen >= 0) {
                    ...
                    com.valkyrlabs.toolkit.Logger.log("\tOLE Object:" + ss);
                }
                // AnsiClipboardFormat (variable)
//				com.valkyrlabs.toolkit.Logger.log("\t" + Arrays.toString(rec.getData()));				
            } else if (n.startsWith("000")) {        // pivot cache
                if (s.myblocks != null) {
                    for (int zz = 0; zz < s.myblocks.size(); zz++)
                        if ((s.myblocks.get(zz) instanceof com.valkyrlabs.formats.LEO.BIGBLOCK))
                            com.valkyrlabs.toolkit.Logger.log("\t" + zz + "-" + Arrays.toString(((com.valkyrlabs.formats.LEO.BIGBLOCK) s.myblocks.get(zz)).getBytes()));
                        else
                            com.valkyrlabs.toolkit.Logger.log("\t" + zz + "-" + Arrays.toString(((com.valkyrlabs.formats.LEO.SMALLBLOCK) s.myblocks.get(zz)).getBytes()));
                }
                BlockByteReader bytes = s.getBlockReader();
                int len = bytes.getLength();
                for (int z = 0; z <= len - 4; ) {
                    ...
                    rec.init();
                    com.valkyrlabs.toolkit.Logger.log("\t\t" + rec.toString());
                    z += reclen + 4;
                }
            } else {
                if (s.myblocks != null) {
