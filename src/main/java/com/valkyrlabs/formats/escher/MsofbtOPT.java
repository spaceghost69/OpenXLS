...
public class MsofbtOPT extends EscherRecord {
    ...
    private void parseData() {
        ...
        for (int i = 0; i < inst; i++) {
            ...
            if (propertyId == MSODrawingConstants.msooptpib)    // blip to display
                ...
                if (lineprops == null) lineprops = new int[3];
            ...
            props.put(Integer.valueOf(propertyId), new Object[]{Boolean.valueOf(fComplex != 0), Integer.valueOf(dtx), Boolean.valueOf(fBid != 0)});
            pos += 6;
        }
        ...
        while (ii.hasNext()) {
            ...
            if (((Boolean) o[0]).booleanValue()) {
                ...
                if (len >= 2) {
                    ...
                    System.arraycopy(recordData, pos, complexdata, 0, complexdata.length);    // get property data after main property table
                    ...
                    } else if (propId.intValue() == MSODrawingConstants.msooptFillBlipName) {    // = the comment, file name, or the full URL that is used as a fill
                        try {
    ...
    public void setProperty(int propId, boolean isBid, boolean isComplex, int dtx, byte[] complexBytes) {
        ...
        if (isComplex) // complexBytes shouldn't be null
            ...
            props.put(Integer.valueOf(propId), new Object[]{Boolean.valueOf(isComplex), Integer.valueOf(dtx), Boolean.valueOf(isBid)});
