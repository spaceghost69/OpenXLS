...
public class ColorChoice implements OOXMLElement {
    ...
    public Theme theme;
    private final SchemeClr s;
    private final SrgbClr srgb;
    private final SysClr sys;
    private final ScrgbClr scrgb;
    private final PrstClr p;

    public ColorChoice(SchemeClr s, SrgbClr srgb, SysClr sys, ScrgbClr scrgb,
                       PrstClr p) {
        this.s = s;
        this.srgb = srgb;
        this.sys = sys;
...
class SysClr implements OOXMLElement {
    ...
    private final String val; // This simple type specifies a system color
    ...
    private final String lastClr; // Specifies the color value that was last
        ...
        for (int i = 0; i < OOXMLConstants.systemColors.length; i++) {
            if (OOXMLConstants.systemColors[i][0].equals(val)) {
                return FormatHandle.HexStringToColorInt(
                        OOXMLConstants.systemColors[i][1], (short) 0);
            }
        }
        return -1;
    }
...
class ScrgbClr implements OOXMLElement {
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                if (eventType == XmlPullParser.START_TAG) {
                    ...
                    if (tnm.equals("scrgbClr")) {
                        for (int i = 0; i < xpp.getAttributeCount(); i++)
                            attrs.put(xpp.getAttributeName(i),
                                    xpp.getAttributeValue(i)); // r, g, b
                    } else {
                        clrTransform = ColorTransform.parseOOXML(xpp, lastTag);
                        break;
                    }
    ...
    public int getColor() {
        ...
        java.awt.Color c = new java.awt.Color((int) rval, (int) gval,
                (int) bval);
...
class ColorTransform {
    ...
    public ColorTransform(int[] lum, int[] hue, int[] sat, int[] alpha,
                          int tint, int shade) {
        this.lum = lum;
        this.hue = hue;
        this.sat = sat;
    ...
    public static ColorTransform parseOOXML(XmlPullParser xpp, Stack<String> lastTag) {
        ...
        try {
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                if (eventType == XmlPullParser.START_TAG) {
                    String tnm = xpp.getName();
                    if (tnm.equals("lum")) { // This element specifies the input
                    ...
                    } else if (tnm.equals("lumMod")) { // This element specifies
                    ...
                    } else if (tnm.equals("lumOff")) { // This element specifies
                        // the input color with
                        // its luminance
                    ...
                    } else if (tnm.equals("hue")) { // This element specifies
                    ...
                    } else if (tnm.equals("sat")) { // This element specifies
                        // the input color with the
                    ...
                    } else if (tnm.equals("shade")) { // This element specifies
                        // a darker version of
                        // its input color
