...
public class Fill implements OOXMLElement {
     ...
     * @param bg            int color index
     */
    public Fill(int pattern, int fg, String fgColorCustom, int bg,
                String bgColorCustom, Theme t) {

        this.patternFill = new PatternFill(PatternFill.translateIndexedFillPattern(pattern),
    ...
    public static String getOOXML(Xf xf) {
        ...
        try {
            ooxml.append("<patternFill patternType=\""
                    + OOXMLConstants.patternFill[xf.getFillPattern()] + "\">");
...
class PatternFill implements OOXMLElement {
    ...
    public PatternFill(String patternType, int fg, int bg) {
        ...
        if (fg > -1 && fg != 64) {
            ...
            attrs.put(
                    "rgb",
                    "FF" + FormatHandle.colorToHexString(FormatHandle.COLORTABLE[fg]).substring(1));
    ...
    public PatternFill(String patternType, int fg, String fgCustom, int bg,
                       String bgCustom) {
        this.patternType = patternType;
    ...
    public static PatternFill parseOOXML(XmlPullParser xpp, boolean isDxf, WorkBookHandle bk) {
        ...
        try {
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                if (eventType == XmlPullParser.START_TAG) {
                    String tnm = xpp.getName();
                    if (tnm.equals("patternFill")) { // get attributes
    ...
    public String getOOXML(boolean isDxf) {
        ...
        if (!isDxf) {
            ...
            if (patternType.equals("solid")) { // dxf needs "none" or "soild" to
    ...
    public String toString() {
        return ((patternType != null) ? patternType : "<none>") + " fg:"
                + ((fgColor != null) ? fgColor.toString() : "<none>") + " bg:"
                + ((bgColor != null) ? bgColor.toString() : "<none>");
    }
    ...
    public static GradientFill parseOOXML(XmlPullParser xpp, WorkBookHandle bk) {
        ...
        try {
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                if (eventType == XmlPullParser.START_TAG) {
                    String tnm = xpp.getName();
                    if (tnm.equals("gradientFill")) { // get attributes
    ...
    public String getOOXML() {
        ...
        if (stops != null) {
            for (int j = 0; j < stops.size(); j++)
                ooxml.append(stops.get(j).getOOXML());
        }
        ooxml.append("</gradientFill>");
        return ooxml.toString();
    }
    ...
    protected static FgColor parseOOXML(XmlPullParser xpp) {
        ...
        try {
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                if (eventType == XmlPullParser.START_TAG) {
                    String tnm = xpp.getName();
                    if (tnm.equals("fgColor")) { // get attributes
                        for (int i = 0; i < xpp.getAttributeCount(); i++)
    ...
    public static BgColor parseOOXML(XmlPullParser xpp) {
        ...
        try {
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                if (eventType == XmlPullParser.START_TAG) {
                    String tnm = xpp.getName();
                    if (tnm.equals("bgColor")) { // get attributes
                        for (int i = 0; i < xpp.getAttributeCount(); i++)
                            attrs.put(xpp.getAttributeName(i),
                                    xpp.getAttributeValue(i));
                    }
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                if (eventType == XmlPullParser.START_TAG) {
                    ...
                    if (tnm.equals("stop")) {
                        ...
                        c = (Color) Color.parseOOXML(xpp, (short) -1, bk)
                                .cloneElement();
                    }
                } else if (eventType == XmlPullParser.END_TAG) {
                    String endTag = xpp.getName();
