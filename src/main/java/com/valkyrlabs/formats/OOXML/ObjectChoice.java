...
public class ObjectChoice implements OOXMLElement {
        ...
        try {
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                if (eventType == XmlPullParser.START_TAG) {
                    ...
                    if (tnm.equals("cxnSp")) {    // connection shape
                        lastTag.push(tnm);
                        c = (CxnSp) CxnSp.parseOOXML(xpp, lastTag, bk);
                        break;
                    } else if (tnm.equals("graphicFrame")) {    // graphic data usually chart
                        lastTag.push(tnm);
                        g = (GraphicFrame) GraphicFrame.parseOOXML(xpp, lastTag);
                        break;
                    } else if (tnm.equals("grpSp")) {    // group shape - combines one or more of sp/pic/graphicFrame/cxnSp
                        lastTag.push(tnm);
                        grp = GrpSp.parseOOXML(xpp, lastTag, bk);
                        break;
                    } else if (tnm.equals("sp")) {        // shape
                        lastTag.push(tnm);
                        s = (Sp) Sp.parseOOXML(xpp, lastTag, bk);
                        break;
                    } else if (tnm.equals("pic")) {        // picture/image
                        lastTag.push(tnm);
    ...
    public String getOOXML() {
        ...
        if (cxnSp != null) ooxml.append(cxnSp.getOOXML());
        if (graphicFrame != null) ooxml.append(graphicFrame.getOOXML());
        if (grpSp != null) ooxml.append(grpSp.getOOXML());
        if (pic != null) ooxml.append(pic.getOOXML());
    ...
    public void setEmbed(String embed) {
        if (sp != null) sp.setEmbed(embed);
        else if (pic != null) pic.setEmbed(embed);
        else if (grpSp != null) grpSp.setEmbed(embed);
    }
    ...
    public void setLink(String link) {
        if (sp != null) sp.setLink(link);
        else if (pic != null) pic.setLink(link);
        else if (grpSp != null) grpSp.setLink(link);
    }
    ...
    public void setChartRId(String rId) {
        if (graphicFrame != null) graphicFrame.setChartRId(rId);
    }
     ...
     */
    public int getId() {
        if (sp != null) return sp.getId();    // embedded blip/pict
        else if (pic != null)
