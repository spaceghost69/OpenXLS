...
public class TwoCellAnchor implements OOXMLElement {
        ...
        try {
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                if (eventType == XmlPullParser.START_TAG) {
                    ...
                    if (tnm.equals("twoCellAnchor")) {        // get attributes
     ...
     */
    public static int[] convertBoundsFromBIFF8(com.valkyrlabs.formats.XLS.Boundsheet sheet, short[] bbounds) {
        // note on bounds:
...
//		bounds[5]= (int)(EMU*cw*(bbounds[5]/1024.0));
     ...
     */
    public static short[] convertBoundsToBIFF8(com.valkyrlabs.formats.XLS.Boundsheet sheet, int[] bounds) {
...
//    	bbounds[5]= (short)((bounds[5]*1024)/(TwoCellAnchor.EMU*cw));
    ...
    public String getOOXML() {
        ...
        ooxml.append(">");
        if (from != null) ooxml.append(from.getOOXML());
        if (to != null) ooxml.append(to.getOOXML());
    ...
    public void setBounds(int[] bounds) {
        ...
        if (from == null) from = new From(b);
        else from.setBounds(b);
        System.arraycopy(bounds, 4, b, 0, 4);
        if (to == null) to = new To(b);
        else to.setBounds(b);
    }
...
class From implements OOXMLElement {
    ...
    public int[] getBounds() {
        if (bounds == null) bounds = new int[4];
        return bounds;
    }
...
class To implements OOXMLElement {
    ...
    // Access Methods
    public int[] getBounds() {
        if (this.bounds == null) this.bounds = new int[4];
        return bounds;
    }
