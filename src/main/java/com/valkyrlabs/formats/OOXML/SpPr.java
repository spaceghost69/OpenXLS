...
public class SpPr implements OOXMLElement {
    ...
    public SpPr(String ns) {    // no-param constructor, set up common defaults
        x = new Xfrm();
        x.setNS("a");
        this.ns = ns;
                ...
                if (eventType == XmlPullParser.START_TAG) {
                    ...
                    if (tnm.equals("spPr")) {
                        ...
                        l = (Ln) Ln.parseOOXML(xpp, lastTag, bk);
                            ...
                            tnm.equals("prstGeom") ||
                                    tnm.equals("custGeom")) {        // GEOMETRY GROUP
                        lastTag.push(tnm);
    ...
    public String getOOXML() {
        ...
        if (bwMode != null) ooxml.append(" bwMode=\"" + bwMode + "\">");
        else ooxml.append(">");
        if (x != null) ooxml.append(x.getOOXML());    // must pass namespace to xfrm
        if (geom != null) ooxml.append(geom.getOOXML());    // geometry choice
        if (fill != null)
            ooxml.append(fill.getOOXML());    // fill choice
        if (l != null) ooxml.append(l.getOOXML());        // ln element
        if (effect != null) ooxml.append(effect.getOOXML());    // effect properties choice
        // scene3d, sp3d
    ...
    public void setEmbed(String embed) {
        if (fill != null) fill.setEmbed(embed);
    }
    ...
    public void setLink(String link) {
        if (fill != null) fill.setLink(link);
    }
