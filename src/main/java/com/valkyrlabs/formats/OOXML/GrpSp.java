    ...
    public String getOOXML() {
        ...
        if (choice != null) {
            for (int i = 0; i < choice.size(); i++) {
                ooxml.append(choice.get(i).getOOXML());
            }
        }
        ooxml.append("</xdr:grpSp>");
...
class GrpSpPr implements OOXMLElement {
    ...
    public static GrpSpPr parseOOXML(XmlPullParser xpp, Stack<String> lastTag, WorkBookHandle bk) {
        ...
        try {
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                if (eventType == XmlPullParser.START_TAG) {
                    String tnm = xpp.getName();
                    if (tnm.equals("grpSpPr")) {        // get attributes
    ...
    public String getOOXML() {
        ...
        ooxml.append(">");
        if (xf != null) ooxml.append(xf.getOOXML());
        if (fill != null) ooxml.append(fill.getOOXML());
        if (effect != null) ooxml.append(effect.getOOXML());
        ooxml.append("</xdr:grpSpPr>");
        return ooxml.toString();
    }
    ...
    public static GrpSpLocks parseOOXML(XmlPullParser xpp, Stack<String> lastTag) {
        ...
        try {
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                if (eventType == XmlPullParser.START_TAG) {
                    String tnm = xpp.getName();
                    if (tnm.equals("grpSpLocks")) {        // get attributes
