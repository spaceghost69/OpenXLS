...
public class Sp implements OOXMLElement {
    ...
    private final NvSpPr nvsp;
    private final SpPr sppr;
    private final Style sty;
    private final TxBody txb;
        ...
        try {
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                if (eventType == XmlPullParser.START_TAG) {
                    ...
                    if (tnm.equals("sp")) {        // get attributes
                        for (int i = 0; i < xpp.getAttributeCount(); i++) {
                            attrs.put(xpp.getAttributeName(i), xpp.getAttributeValue(i));
                        }
                    } else if (tnm.equals("nvSpPr")) {        // non-visual shape props
                        lastTag.push(tnm);
                        nvsp = NvSpPr.parseOOXML(xpp, lastTag);
                    } else if (tnm.equals("spPr")) {        // shape properties
                        lastTag.push(tnm);
                        sppr = (SpPr) SpPr.parseOOXML(xpp, lastTag, bk);
                        //sppr.setNS("xdr");
                    } else if (tnm.equals("style")) {        // shape style
                        lastTag.push(tnm);
                        sty = (Style) Style.parseOOXML(xpp, lastTag, bk);
                    } else if (tnm.equals("txBody")) {        // shape text body
                        lastTag.push(tnm);
    ...
    public String getOOXML() {
        ...
        if (sty != null) ooxml.append(sty.getOOXML());
        if (txb != null) ooxml.append(txb.getOOXML());
        ooxml.append("</xdr:sp>");
    ...
    public void setEmbed(String embed) {
        if (sppr != null) sppr.setEmbed(embed);
    }
    ...
    public void setLink(String link) {
        if (sppr != null) sppr.setLink(link);
    }
    ...
    public static CNvSpPr parseOOXML(XmlPullParser xpp, Stack<String> lastTag) {
        ...
        try {
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                if (eventType == XmlPullParser.START_TAG) {
                    String tnm = xpp.getName();
                    if (tnm.equals("cNvSpPr")) {        // get attributes
                        for (int i = 0; i < xpp.getAttributeCount(); i++) {
    ...
    public static SpLocks parseOOXML(XmlPullParser xpp, Stack<String> lastTag) {
        ...
        try {
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                if (eventType == XmlPullParser.START_TAG) {
                    String tnm = xpp.getName();
                    if (tnm.equals("spLocks")) {        // get attributes
                        for (int i = 0; i < xpp.getAttributeCount(); i++) {
                            attrs.put(xpp.getAttributeName(i), xpp.getAttributeValue(i));
                        }
                    }
