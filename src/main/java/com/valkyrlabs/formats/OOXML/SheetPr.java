...
public class SheetPr implements OOXMLElement {
    ...
    private final OutlinePr outlinePr;
    private final PageSetupPr pageSetupPr;
        ...
        try {
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                if (eventType == XmlPullParser.START_TAG) {
                    ...
                    if (tnm.equals("sheetPr")) {        // get attributes
                        for (int i = 0; i < xpp.getAttributeCount(); i++) {
                            attrs.put(xpp.getAttributeName(i), xpp.getAttributeValue(i));
                        }
                    } else if (tnm.equals("pageSetUpPr")) { // Page setup properties of the worksheet
    ...
    public String getOOXML() {
        ...
        ooxml.append(">");
        // ordered sequence:
        if (tab != null) ooxml.append(tab.getOOXML());
        if (outlinePr != null) ooxml.append(outlinePr.getOOXML());
        if (pageSetupPr != null) ooxml.append(pageSetupPr.getOOXML());
        ooxml.append("</sheetPr>");
    ...
    public static TabColor parseOOXML(XmlPullParser xpp) {
        ...
        try {
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                if (eventType == XmlPullParser.START_TAG) {
                    String tnm = xpp.getName();
                    if (tnm.equals("tabColor")) {        // get attributes
                        for (int i = 0; i < xpp.getAttributeCount(); i++) {
                            attrs.put(xpp.getAttributeName(i), xpp.getAttributeValue(i));
                        }
                    }
    ...
    public static OutlinePr parseOOXML(XmlPullParser xpp) {
        ...
        try {
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                if (eventType == XmlPullParser.START_TAG) {
                    String tnm = xpp.getName();
                    if (tnm.equals("outlinePr")) {        // get attributes
                        for (int i = 0; i < xpp.getAttributeCount(); i++) {
                            attrs.put(xpp.getAttributeName(i), xpp.getAttributeValue(i));
                        }
                    }
    ...
    public static PageSetupPr parseOOXML(XmlPullParser xpp) {
        ...
        try {
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                if (eventType == XmlPullParser.START_TAG) {
                    String tnm = xpp.getName();
                    if (tnm.equals("pageSetUpPr")) {        // get attributes
                        for (int i = 0; i < xpp.getAttributeCount(); i++) {
