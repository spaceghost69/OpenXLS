...
public class Graphic implements OOXMLElement {
    ...
    public String getOOXML() {
        ...
        if (graphicData != null) ooxml.append(graphicData.getOOXML());
        ooxml.append("</a:graphic>");
    ...
    public void setChartRId(String rid) {
        if (graphicData != null) graphicData.setChartRId(rid);
    }
}
...
class GraphicData implements OOXMLElement {
    ...
    public static GraphicData parseOOXML(XmlPullParser xpp, Stack lastTag) {
        ...
        try {
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                if (eventType == XmlPullParser.START_TAG) {
                    String tnm = xpp.getName();
                    if (tnm.equals("graphicData")) {        // get attributes
                        if (xpp.getAttributeCount() > 0)
                            uri = xpp.getAttributeValue(0);
                    } else if (tnm.equals("chart")) {        // one of many possible children
                        if (xpp.getAttributeCount() > 0)
    ...
    public String getOOXML() {
        ...
        if (rid != null) { // we'll assume it's a chart for nwo
            ooxml.append("><c:chart xmlns:c=\"http://schemas.openxmlformats.org/drawingml/2006/chart\" xmlns:r=\"http://schemas.openxmlformats.org/officeDocument/2006/relationships\"  r:id=\"" + rid + "\"/></a:graphicData>");
        } else
            ooxml.append("/>");
        return ooxml.toString();
    }
