...
-5400000=90 degrees counter-clockwise
        ...
        this.para = new P(fx.getFontName(), (int) fx.getFontHeightInPoints() * 100, fx.getBold(), fx.getItalic(), usty, strike,
        ...
        try {        // need: endParaRPr?
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                if (eventType == XmlPullParser.START_TAG) {
                    ...
                    if (tnm.equals("bodyPr")) {        // default text properties
                        lastTag.push(tnm);
                        b = (BodyPr) BodyPr.parseOOXML(xpp, lastTag);
                    } else if (tnm.equals("p")) {    // part of p element
                        lastTag.push(tnm);
            ...
            Logger.logErr("txPr.parseOOXML: " + e.toString());
        }
    ...
    public String getOOXML() {
        ...
        if (bPr != null) tooxml.append(bPr.getOOXML());
        tooxml.append("<a:lstStyle/>"); // TODO: HANDLE
        if (para != null) tooxml.append(para.getOOXML());
        tooxml.append("</c:txPr>");
