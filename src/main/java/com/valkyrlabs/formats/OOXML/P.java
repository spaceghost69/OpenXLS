        ...
        try {
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                if (eventType == XmlPullParser.START_TAG) {
                    ...
                    if (tnm.equals("pPr")) {        // paragraph-level text props
                        lastTag.push(tnm);
                        ppr = PPr.parseOOXML(xpp, lastTag, bk);
                    } else if (tnm.equals("r") ||
                            tnm.equals("br") ||
                            tnm.equals("fld")) {    // text run
                        lastTag.push(tnm);
...
class PPr implements OOXMLElement {
    ...
    public static PPr parseOOXML(XmlPullParser xpp, Stack<String> lastTag, WorkBookHandle bk) {
        ...
        try {
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                if (eventType == XmlPullParser.START_TAG) {
                    String tnm = xpp.getName();
                    if (tnm.equals("pPr")) {        // t element of text run -- the title string we are interested in
                        for (int i = 0; i < xpp.getAttributeCount(); i++) {    // align, defTabSz, fontAlgn, hangingPunct, indent, lvl, rtl, marL, marR ...
                            attrs.put(xpp.getAttributeName(i), xpp.getAttributeValue(i));
                        }
                    } else if (tnm.equals("defRPr")) {        // default text properties
                        lastTag.push(tnm);
    ...
    public String getOOXML() {
        ...
        ooxml.append(">");
        if (dp != null) ooxml.append(dp.getOOXML());
        ooxml.append("</a:pPr>");
        return ooxml.toString();
    }
...
class EndParaRPr implements OOXMLElement {
    ...
    private final String cs;    // really children but only have 1 attribute and no children
    ...
    public static EndParaRPr parseOOXML(XmlPullParser xpp, Stack<String> lastTag, WorkBookHandle bk) {
        ...
        try {
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                if (eventType == XmlPullParser.START_TAG) {
                    String tnm = xpp.getName();
                    if (tnm.equals("endParaRPr")) {        // get attributes
