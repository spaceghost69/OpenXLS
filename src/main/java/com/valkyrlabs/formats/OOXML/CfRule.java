...
public class CfRule implements OOXMLElement {
        ...
        try {
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                if (eventType == XmlPullParser.START_TAG) {
                    ...
                    if (tnm.equals("cfRule")) {     // get attributes: priority, type, operator, dxfId
    ...
    public String getOOXML(int priority) {
        ...
        if (formulas != null) {
            for (int j = 0; j < formulas.size(); j++) {
                ooxml.append("<formula>" + OOXMLAdapter.stripNonAsciiRetainQuote(formulas.get(j)) + "</formula>");
            }
        }
    ...
    public void setDxfId(int dxfId) {
        if (this.attrs == null) this.attrs = new HashMap<String, String>();
