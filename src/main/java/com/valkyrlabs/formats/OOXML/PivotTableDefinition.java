...
import com.valkyrlabs.OpenXLS.PivotTableHandle;
import com.valkyrlabs.OpenXLS.WorkBookHandle;
import com.valkyrlabs.formats.XLS.Boundsheet;
import com.valkyrlabs.formats.XLS.SxStreamID;
import com.valkyrlabs.formats.XLS.Sxvd;
...
//						ptview.setCDimPg(count);
                    } else if (tnm.equals("pageField")) { // count: # of pageField elements
                        parsePageFieldOOXML(xpp, ptview);
                    } else if (tnm.equals("dataFields")) {    // Represents the collection of items in the data region of the PivotTable.
//						short count = Integer.valueOf(xpp.getAttributeValue(0)).shortValue();
//						ptview.setCDimData(count);
                    } else if (tnm.equals("dataField")) { // count: # of dataField elements
                        parseDataFieldOOXML(xpp, ptview);
                    } else if (tnm.equals("colFields") || tnm.equals("rowFields")) {    // the collection of fields on the column or row axis
                        parseFieldOOXML(xpp, ptview);
                    } else if (tnm.equals("rowItems") || tnm.equals("colItems")) {        // the collection of column items or row items
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                if (eventType == XmlPullParser.START_TAG) {
                    ...
                    if (tnm.equals("pivotField")) { // Represents a single field in the PivotTable. This complex type contains information about the field, including the collection of items in the field.
                        curAxis = null;
                        for (int i = 0; i < xpp.getAttributeCount(); i++) {        // TODO: HANDLE ALL ATTRIBUTES *****
                            String nm = xpp.getAttributeName(i);
                            String v = xpp.getAttributeValue(i);
                            if (nm.equalsIgnoreCase("axis")) // Specifies the region of the PivotTable that this field is displayed
                                ...
                                ;
                            else if (nm.equalsIgnoreCase("dataField")) {    // Specifies a boolean value that indicates whether this field appears in the data region of the PivotTable.
                    ...
                    } else if (tnm.equals("items")) {    // Represents the collection of items in a PivotTable field. The items in the collection are ordered by index. Items
                        // represent the unique entries from the field in the source data.
                        parsePivotItemOOXML(xpp, ptview, curAxis);
                    } else if (tnm.equals("pivotArea")) { // parent= autoSortScope, which has no attributes or other children
    ...
    private static void parsePivotItemOOXML(XmlPullParser xpp, Sxview ptview, Sxvd axis) throws XmlPullParserException {
        try {
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                if (eventType == XmlPullParser.START_TAG) {
                    String tnm = xpp.getName();
                    if (tnm.equals("item")) { // Represents a single item in PivotTable field.
                        for (int i = 0; i < xpp.getAttributeCount(); i++) {
                            String nm = xpp.getAttributeName(i);
                            String v = xpp.getAttributeValue(i);
    ...
    private static void parseFormatsOOXML(XmlPullParser xpp, Sxview ptview) throws XmlPullParserException {
        try {
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                if (eventType == XmlPullParser.START_TAG) {
                    String tnm = xpp.getName();
                    if (tnm.equals("format")) {    // parent= formats
                        for (int i = 0; i < xpp.getAttributeCount(); i++) {
                            String nm = xpp.getAttributeName(i);
                            //String v = xpp.getAttributeValue(i);
                            if (nm.equals("dxfId")) { // Specifies the identifier of the format the application is currently using for the PivotTable. Formatting information is written to the styles part. See the Styles section (§3.8) for more information on formats.
                    ...
                    } else if (tnm.equals("chartFormat")) {    // parent= chartFormats
                        for (int i = 0; i < xpp.getAttributeCount(); i++) {
                            String nm = xpp.getAttributeName(i);
                            //String v = xpp.getAttributeValue(i);
                            if (nm.equals("chart")) { // Specifies the index of the chart part to which the formatting applies.
                            } else if (nm.equals("format")) {    // Specifies the index of the pivot format that is currently in use. This index corresponds to a dxf element in the Styles part.
                            } else if (nm.equals("series")) {    // Specifies a boolean value that indicates whether format applies to a series. (default=false)
                            }
                        }
        ...
        try {
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                if (eventType == XmlPullParser.START_TAG) {
                    ...
                    } else if (tnm.equals("references")) { // Represents the set of selected fields and the selected items within those fields
                        // count
    ...
    private static void parseLineItemOOXML(XmlPullParser xpp, Sxview ptview) throws XmlPullParserException {
        try {
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                if (eventType == XmlPullParser.START_TAG) {
                    String tnm = xpp.getName();
                    if (tnm.equals("i")) { // a colItem or rowItem
                        indexes = new short[nIndexes];
                        nLines = nIndexes;
                        index = 0;
                        type = repeat = 0;
                        for (int i = 0; i < xpp.getAttributeCount(); i++) {    // i, r, t
                            String nm = xpp.getAttributeName(i);
                            String v = xpp.getAttributeValue(i);
                            if (nm.equals("i")) {    // Specifies a zero-based index indicating the referenced data item it in a data field with multiple data items.
                            } else if (nm.equals("r")) {// Specifies the number of items to repeat from the previous row item. Note: The first item has no @r explicitly written. Since a default of "0" is specified in the schema, for any item
                                // whose @r is missing, a default value of "0" is implied.
                                repeat = Integer.valueOf(v).intValue();
                                index += repeat;
                                nLines -= repeat;
                            } else if (nm.equals("t")) {// Specifies the type of the item. Value of 'default' indicates a grand total as the last row item value
