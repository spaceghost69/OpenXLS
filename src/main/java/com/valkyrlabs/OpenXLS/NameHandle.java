...
public class NameHandle {
	 ...
	 *
	 * note: this will only be functional for the workbook that the name is
	 * contained in,
	...
	public void setScope(Handle scope) {
		...
		try {
			...
			// this really shouldnt happen unless you are passing a scope in from a
			// different workbook
	 ...
	 * @param location rangeDef Range of the cells for this named range, in excel
	 *                 syntax including sheet name, ie "Sheet1!A1:D1"
	...
	public String getXML() {
		...
		// it's possible that name expression can have quotes and other non-compliant
		// characters
		nmv = com.valkyrlabs.toolkit.StringTool.convertXMLChars(nmv);

		String nmx = getName();
		nmx = com.valkyrlabs.toolkit.StringTool.convertXMLChars(nmx);
	...
	public void deleteRow(int idxcol) throws Exception {
		...
		if (rngs.length > 1) {
			...
			throw new WorkBookException("NamedRange.updateRow Object array failed: zero CellRanges",
					WorkBookException.RUNTIME_ERROR);
		}
	...
	public void addRow(Object[] objarr) throws Exception {
		CellRange[] rngs = getCellRanges();
		if (rngs.length > 1) {
			throw new WorkBookException("NamedRange.add Object array failed: too many CellRanges.",
					WorkBookException.RUNTIME_ERROR);
		} else if (rngs.length == 0) {
			throw new WorkBookException("NamedRange.add Object array failed: zero CellRanges",
					WorkBookException.RUNTIME_ERROR);
		}
	...
	public void setLocation(String strloc) {
		...
		// if there is no sheetname, and no sheet, then this is an unusable name and
		// cannot be added
	...
	/**
	 * get the referenced named cell range as string in standard excel syntax
	 * including sheetname,
	 * 
	 * for instance "Sheet1!A1:Z255"
	 * 
	...
	public CellHandle[] getCells()
		...
		try {
			ArrayList cellhandles = new ArrayList();
	...
	public CellRange[] getCellRanges()
		...
		// 20100217 KSC: try a better way (that can handle 3D refs and complex cell
		// ranges)
		...
		// handle commas within quoted sheet names
		// (TestExtenXLSEngine.TestQuotedSheetsWithCommansInNRs)
		...
		// String[] nranges= StringTool.splitString(loc, ","); // TODO: can be another
		// delimeter?
	...
	public WorkSheetHandle[] getReferencedSheets()
		...
		if (bs == null)
			throw new SheetNotFoundException(
					"worksheet for Named Range: " + this.toString() + ":" + this.myName.getExpressionString());
		if (bs[0] == null)
			throw new SheetNotFoundException(
					"Worksheet for Named Range: " + this.toString() + ":" + this.myName.getExpressionString());
