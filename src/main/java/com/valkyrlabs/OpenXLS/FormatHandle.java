...
import com.valkyrlabs.formats.XLS.FormatConstantsImpl;
...
public class FormatHandle implements Handle, FormatConstants {
	...
	private ArrayList<BiffRec> mycells = new ArrayList<>(); // all the Cells sharing this format e.g. CellRange
	private Xf myxf;
	private int xfe;
	...
	private com.valkyrlabs.formats.XLS.WorkBook wkbook;
	private com.valkyrlabs.OpenXLS.WorkBook wbh;
	 ...
	 */
	public FormatHandle(com.valkyrlabs.OpenXLS.WorkBook book, Xf xfr) {
		myxf = xfr;
		xfe = myxf.getIdx();
		wbh = book;
		if (book != null) {
	 ...
	 */
	public FormatHandle(com.valkyrlabs.OpenXLS.WorkBook book) {
	 ...
	 */
	public FormatHandle(com.valkyrlabs.formats.XLS.WorkBook book, int xfnum) {
		wkbook = book;
	 ...
	 */
	protected FormatHandle(com.valkyrlabs.OpenXLS.WorkBook book, int xfnum,
			boolean dedupe) {
		this(book, xfnum);
	 ...
	 */
	protected FormatHandle(Condfmt cx, com.valkyrlabs.OpenXLS.WorkBook book,
	 ...
	 *             workbook
	 */
	protected void updateFromCF(Cf cf, com.valkyrlabs.OpenXLS.WorkBook book) {
		// border colors
	...
	public FormatHandle(com.valkyrlabs.OpenXLS.WorkBook book, String fontname,
	 ...
	 * @param i
	 */
	public FormatHandle(com.valkyrlabs.OpenXLS.WorkBook workBook, int i) {
	...
	void addCell(BiffRec c) {
		if (myxf != null) {
			...
			Logger.logWarn(
					"FormatHandle.addCell() - You MUST call setFont() to initialize the FormatHandle's font before adding Cells.");
		}
		mycells.add(c);
	}
	...
	public static int getColorInt(Color col) {
		...
		for (int i = 0; i < FormatHandle.COLORTABLE.length; i++) {
			double curDif = Math.pow(R - FormatHandle.COLORTABLE[i].getRed(), 2)
	...
	public void adjustPrecision(boolean increase) {
		...
		try {
			...
			// for each, find decimal place and increment/decrement; if not found, find last
			// digit placeholder
				...
				if (z != -1) { // found decimal place
					...
					for (; z < pats[i].length(); z++) {
						...
						if ((c == '0' || c == '#' || c == '?')) // numeric placeholders
							foundit = true;
						else if (foundit && !(c == '0' || c == '#' || c == '?')) // numeric placeholders. if hit last
																					// one, either inc or dec
							break;
					}
					if (increase)
						newPat += new StringBuffer(pats[i]).insert(z, "0").toString(); // pats[i].substring(0, z) + "0"
																						// + pats[i].substring(z+1);
					else {
						if (pats[i].charAt(z - 2) != '.') {
							newPat += new StringBuffer(pats[i]).deleteCharAt(z - 1).toString(); // .pats[i].substring(0,
																								// z-1) +
																								// pats[i].substring(z+1);
						} else {
					...
					if (foundit) // if had ANY numeric placeholders
						newPat += new StringBuffer(pats[i]).insert(z + 1, ".0").toString(); // pats[i].substring(0, z) +
																							// ".0" +
																							// pats[i].substring(z+1);
					else
			...
			// com.valkyrlabs.toolkit.Logger.log("Old Style" + pat + ". New Style: " +
			// newPat + ". Increase?" + (increase?"yes":"no")); // KSC: TESETING: TAKE OUT
			// WHEN DONE
	 ...
	 * @param bk
	 */
	public void setWorkBook(com.valkyrlabs.formats.XLS.WorkBook bk) {
		wkbook = bk;
	}
	...
	public JSONObject getJSONObject(int XFNum) {
		...
		try {
			...
			if (getFillPattern() >= 0) { // KSC: added >= 0 as some conditional formats have patternFillStyle==0 even
											// though they have a pattern block and
	 ...
	 *                             if true, font family will be changed to
	 *                             ArialUnicodeMS
	...
	public String getXML(int XFNum, boolean convertToUnicodeFont) {
		...
		if (getRightBorderLineStyle() != 0) {
			...
			sb.append(" Color=\"" + colorToHexString(getBorderRightColor())
					+ "\"");
			sb.append(" Weight=\""
					+ BORDER_SIZES_HTML[getRightBorderLineStyle()] + "\"");
			sb.append("/>");
		}
		if (getBottomBorderLineStyle() != 0) {
			sb.append("<Border");
			...
			sb.append(" Color=\"" + colorToHexString(getBorderBottomColor())
					+ "\"");
			sb.append(" Weight=\""
					+ BORDER_SIZES_HTML[getBottomBorderLineStyle()] + "\"");
			sb.append("/>");
		}
		if (getLeftBorderLineStyle() != 0) {
			sb.append("<Border");
			...
			sb.append(" Color=\"" + colorToHexString(getBorderLeftColor())
					+ "\"");
			sb.append(" Weight=\""
					+ BORDER_SIZES_HTML[getLeftBorderLineStyle()] + "\"");
			sb.append("/>");
		}
		if (getTopBorderLineStyle() != 0) {
			sb.append("<Border");
		...
		if (myxf.getIndent() > 0)
			sb.append(" Indent=\"" + (myxf.getIndent() * 10) + "px\""); // indent // # * 3 // spaces // - do // an
		 ...
		 */
		sb.append("<Interior");
		if (getFillPattern() > 0) { // 20070201 KSC: Background colors only valid if there is a fill pattern
			...
			// ****************************************
			// PDf processing shouldn't output white background due to z-order and
			// overwriting image/chart objects
			// ... possibly other uses need the white bg set ...???
			// ****************************************
			if (!(myxf.getFillPattern() == PATTERN_FILLED
					&& this.getWorkBook().getColorTable()[fg].equals(Color.WHITE))) {
				sb.append(" Color=\"" + colorToHexString(this.getWorkBook().getColorTable()[fg]) + "\"" + " Fg=\"" + fg
						+ "\"");
			}
			sb.append(
					" PatternColor=\"" + colorToHexString(this.getWorkBook().getColorTable()[myxf.getBackgroundColor()])
							+ "\"" + " Bg=\"" + myxf.getBackgroundColor() + "\"");
		...
		try {
			...
			Logger.logWarn("FormatHandle.getXML problem with protection setting: "
					+ e.toString());
		} finally {
			sb.append(" />");
		}
	 ...
	 * 
	 * @param s
	 *                  HTML color string (#XXXXXX) format or (#FFXXXXXX -
	 *                  OOXML-style
	 *                  format)
	 * @param colorType
	 ...
	 * 
	 * 
	 * @param s web-style hex color string, including #, or OOXML-style color
	 *          string, FFXXXXXX
	 * @return Color
	 * @see Color
	 */
	...
	private Font cloneFont(Font src) {
		Font f = new Font();
		f.setOpcode(com.valkyrlabs.formats.XLS.XLSConstants.FONT);
	...
	private Xf duplicateXf(Xf xf) {
		...
		// not used anymore canModify = true; // if duplicated, it's new and unlinked
		// (thus far)
		return xf;
	}
		...
		if (!myxf.toString().equals(xf.toString())) {
			if (myxf.getUseCount() <= 1 && xfe > 15) { // used only by one cell, OK to modify
				if (writeImmediate || wkbook.getFormatCache().get(xf.toString()) == null) {
					...
					if (xf.fill != null)
						myxf.fill = (com.valkyrlabs.formats.OOXML.Fill) xf.fill.cloneElement();
					myxf.init();
	...
	private Xf cloneXf(Xf xf) {
		...
		clone.setData(data);
		if (xf.fill != null)
			clone.fill = (com.valkyrlabs.formats.OOXML.Fill) xf.fill.cloneElement();
		clone.init();
		return clone;
	}
	 ...
	 * @return
	 */
	public static Xf cloneXf(Xf xf, com.valkyrlabs.formats.XLS.WorkBook wkbook) {
		Xf clone = new Xf(xf.getFont().getIdx(), wkbook);
		byte[] data = xf.getBytesAt(0, xf.getLength() - 4);
		clone.setData(data);
		clone.init();
	...
	public static Xf cloneXf(Xf xf, Font f,
			com.valkyrlabs.formats.XLS.WorkBook wkbook) {
	...
	public void close() {
		mycells.clear();
		mycells = new ArrayList(); // all the Cells sharing this format
		myxf = null;
		mycol = null;
		myrow = null;
		wkbook = null;
