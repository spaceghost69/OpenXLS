...
Serializable // Money objects are serializable
{
	...
	*/
		...
		if (// If the Scale Factor is not
		 (scale < 0) // 0, 1, or
			...
			throw new InvalidScaleFactorException(
				"Invalid scale factor: " + scale + " (must be 0, 1, or 2)");
	...
	public Money divide(double div) // The divisor value
	{
		...
		// necessary.
		result.value=
			value.divide(new BigDecimal(div), BigDecimal.ROUND_HALF_UP);
		return result; // Return the result.
	} // Method Money.divide()
	/**
	 *
	...
	public Money divide(long div) // The divisor value
	{
		...
		result.value=
			value.divide(
				new BigDecimal(Long.toString(div)),
	...
	throws InvalidRoundingModeException {
		if (mode == BigDecimal.ROUND_UNNECESSARY) // If Rounding Mode is not
			...
			throw new InvalidRoundingModeException(
				"Rounding mode not valid for monetary data: " + mode);
		Money result= new Money(this); // Create a new Money object for
		// the result.
