...
public class StringTool implements Serializable {
	...
	public static String convertXMLChars(String rep) {
		return com.valkyrlabs.formats.XLS.OOXMLAdapter.stripNonAscii(rep)
	 ...
	 * 
	 * @param target
	 *            the Throwable whose stack trace should be returned
	 ...
	 * 
	 * @param holder
	 *            The String that you want stripped.
	 ...
	 *            The String that you want stripped.
	 * @param stripchar
	 *            The char you want stripped from the String.
	 * @return Your stripped string.
	 */
	 ...
	 *            Text to process.
	 * @param beginDelim
	 *            Delimeter for beginning of retrieved section.
	 * @param endDelim
	...
	public static String getTextBetweenNestedDelims(String originalText, String beginDelim, String endDelim) {
		...
		if ((originalText.indexOf(beginDelim) < 0)
				|| (originalText.lastIndexOf(endDelim) < 0)) {
			return "";
		} else {
	...
	public static String getTextBetweenDelims(String originalText, String beginDelim, String endDelim) {
		...
		// Check to see that both delimiters exist in the string
		if ((originalText.indexOf(beginDelim) < 0)
				|| (originalText.indexOf(endDelim) < 0)) {
			return "";
		} else {
			int begidx = originalText.indexOf(beginDelim) + beginDelim.length();
	 ...
	 *            Text to process.
	 * @param replaceBegin
	 *            Beggining pattern of replaced section.
	 * @param replacementText
	 *            Text to replace with.
	 * @param replaceEnd
	 *            End pattern of replaced section.
	 * @return Processed text.
	 */
	...
	public static String StripChars(String theFilter, String theString) {
		...
		for (int i = 0; i < theString.length(); i++) {
			...
			if (theFilter.indexOf(curChar) < 0) { // if it's not in the filter,
													// send it thru
	...
	public static String UseOnlyChars(String theFilter, String theString) {
		...
		for (int i = 0; i < theString.length(); i++) {
			curChar = theString.charAt(i);
			if (theFilter.indexOf(curChar) > -1) { // if it's in the filter,
													// send it thru
				strOut.append(curChar);
			}
		}
	...
	public static double getApproximateHeight(java.awt.Font f, String s, double w) {
		...
		// com.valkyrlabs.toolkit.Logger.log("Font: " +
		// f.toString());
		// com.valkyrlabs.toolkit.Logger.log("l-i:" +
	...
	public static String qualifyPatternString(String pattern) {
		...
		/*
		 * if (s.length > 1) {
		 * com.valkyrlabs.toolkit.Logger.log(s[0]);
		 * java.util.regex.Pattern p =
		 * java.util.regex.Pattern.compile("\\[(.*?)\\]");
		 * java.util.regex.Matcher m = p.matcher(pattern);
		 * 
		 * while(m.find()) {
		 * com.valkyrlabs.toolkit.Logger.log(m.group(1));
		 * }
		 * }
		 */
		String[] s = pattern.split("\\[");
