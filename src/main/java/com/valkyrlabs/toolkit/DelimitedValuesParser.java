...
public class DelimitedValuesParser {
	...
	public enum Token { VALUE, NEWLINE, EOF }
	...
	throws IOException {
		...
		while (true) {
			...
			if (read == -1) {
				if (value.length() == 0) return current = Token.EOF;
				else return current = Token.VALUE;
			}
			
			if (read == delimiter) return current = Token.VALUE;
