...
public class InFile extends DataInputStream
{
    ...
    */
	...
	{
	       try  {while (this.available() != 0){sb.append(this.readLine());}}
	       catch(FileNotFoundException e){ Logger.logInfo("FNF Exception in InFile: " + e);}
	       catch(IOException e){Logger.logInfo("IO Exception in InFile: " + e);}
	       return sb.toString();
	}
	...
	public static byte[] getBytesFromFile(File file) throws IOException {
		...
		if (offset < ret.length) {throw new IOException("Read file failed -- all bytes not retreived. " + file.getName());}
		fis.close();
		return ret;
