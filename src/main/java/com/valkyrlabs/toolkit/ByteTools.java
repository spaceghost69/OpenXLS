...
public static String getByteString(byte[] bt, boolean pad){
    ...
    public static byte[] append (byte[] src, byte[] dest){
    	...
    	if (src == null) return (byte[]) dest.clone();
    	if (dest == null) return (byte[]) src.clone();
   ...
   */
    public static byte[] append(byte[] src, byte[] dest, int pos){
        int srclen = src.length;
        if(dest == null)dest = new byte[srclen];
        int destlen = dest.length;
        if(destlen < srclen){
    ...
    */
    ...
    public static byte[] doubleToLEByteArray(double d){
        ...
        for(i=bite.length-1;i>-1;i--) { //High order byte will be in b[0] 
            long irr = (t & 0xff);
    ...
    */
    public static double eightBytetoLEDouble(byte[] bite){
        ...
        try {
            ...
            d = dbl.doubleValue();
        } catch (java.io.IOException e){ Logger.logInfo("io exception in byte to Double conversion" + e);}
        return d;
    }
  ...
  //  private boolean DEBUG = false;
    ...
    */
    public static void writeToFile(byte[] b, String fname){
        try{
            ...
            fos.flush();
            fos.close();
        }catch(IOException e){Logger.logInfo("Error writing bytes to file in ByteTools: " + e);}
    ...
    */
    ...
    public static short updateGrBit(short grbit, boolean set, int bitNum) {
    	switch (bitNum) {
    	case 0:
    		if (set)  grbit|= (0x1);  else  grbit&= (0xFFFE);
    		break;
    	case 1:
    		if (set)  grbit|= (0x2);  else  grbit&= (0xFFFD);
    		break;
    	case 2:
    		if (set)  grbit|= (0x4);  else  grbit&= (0xFFFB);
    		break;
    	case 3:
    		if (set)  grbit|= (0x8);  else  grbit&= (0xFFF7);
    		break;
    	case 4:
    		if (set)  grbit|= (0x10);  else  grbit&= (0xFFEF);
    		break;
    	case 5:
    		if (set)  grbit|= (0x20);  else  grbit&= (0xFFDF);
    		break;
    	case 6:
    		if (set)  grbit|= (0x40);  else  grbit&= (0xFFBF);
    		break;
    	case 7:
    		if (set)  grbit|= (0x80);  else  grbit&= (0xFF7F);
    		break;
    	case 8:
    		if (set)  grbit|= (0x100);  else  grbit&= (0xFEFF);
    		break;
    	case 9:
    		if (set)  grbit|= (0x200);  else  grbit&= (0xFDFF);
    		break;
    	case 10:
    		if (set)  grbit|= (0x400);  else  grbit&= (0xFBFF);
    		break;
    	case 11:
    		if (set)  grbit|= (0x800);  else  grbit&= (0xF7FF);
    		break;
    	case 12:
    		if (set)  grbit|= (0x1000);  else  grbit&= (0xEFFF);
    		break;
    	case 13:
    		if (set)  grbit|= (0x2000);  else  grbit&= (0xDFFF);
    		break;
    	case 14:
    		if (set)  grbit|= (0x4000);  else  grbit&= (0xBFFF);
    		break;
    	case 15:
    		if (set)  grbit|= (0x8000);  else  grbit&= (0x7FFF);
    		break;
    	}
    	return grbit;
    }
