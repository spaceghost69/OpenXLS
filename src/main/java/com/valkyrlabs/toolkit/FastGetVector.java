...
public class FastGetVector extends ArrayList<Object>
{
	...
	*/
	public void resetHints(boolean ignore_records){
	   // ExcelTools.benchmark("Re-indexing ArrayList" + reindex_change_size++);
		if(!ignore_records){
			for(int t = 0;t<this.size();t++){
				try{
					((ArrayListHints)this.get(t)).setRecordIndexHint(t);
	...
	public void add(int idx, ArrayListHints obj){  
		this.change_offset++;// 
		obj.setRecordIndexHint(idx);
		super.add( idx, obj);
	}
/*
	public void addAll(ArrayList cv){        
		for(int i=0;i<cv.size();i++){
			Object b = cv.get(i);
			if(b instanceof ArrayListHints){
				this.add((ArrayListHints)b);
			}else{
				this.add((Object)b);
			}
		}
...
*/
	public boolean remove(Object obj){
		this.change_offset--;
		if(super.remove(obj))return true;
		return false;
	}
