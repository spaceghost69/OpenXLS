 ...
 * performance than both an <code>ArrayList</code> and a <code>LinkedList</code>
 ...
 * </pre>
 * 
 * <code>ArrayList</code> is a good general purpose list implementation. It is
 * faster than <code>TreeList</code> for most operations except inserting and
 * removing in the middle of the list. <code>ArrayList</code> also uses less
 ...
 */
class XLSOptimizedTreeList extends AbstractList<Object> {
	 ...
	 */
	public XLSOptimizedTreeList(Collection<?> coll) {
		super();
		addAll(coll);
	}
	 ...
	 * {@link java.util.List}. The nodes don't know the index of the object they are
	...
	static class AVLNode {
		 ...
		 * @param obj
		 *            the value for the ndoe
		 * @param rightFollower
		 *            the node with the value following this one
		 * @param leftFollower
		 ...
		 * 
		 * @param index
		 *            is the index of the position relative to the position of the
		 *            parent node.
		 * @param obj
		 ...
		 * 
		 * @param index
		 *            is the index of the element to be removed relative to the position
	 ...
	 */
	static class TreeListIterator implements ListIterator<Object> {
		/** The parent list */
		protected final XLSOptimizedTreeList parent;
		/**
		 * The node that will be returned by {@link #next()}. If this is equal to
		 * {@link AbstractLinkedList#header} then there are no more values to return.
		 */
		...
		/**
		 * The last node that was returned by {@link #next()} or {@link #previous()}.
		 * Set to <code>null</code> if {@link #next()} or {@link #previous()} haven't
		 * been called, or if the node has been removed with {@link #remove()} or a new
		 * node added with {@link #add(Object)}. Should be accessed through
		 * {@link #getLastNodeReturned()} to enforce this behaviour.
		 */
		 ...
		 * have this count, then a {@link java.util.ConcurrentModificationException} may
		 ...
		 */
		protected TreeListIterator(XLSOptimizedTreeList parent, int fromIndex) throws IndexOutOfBoundsException {
			super();
			this.parent = parent;
		 ...
		 *             If the list's modification count isn't the value that was
		 *             expected.
		 */
