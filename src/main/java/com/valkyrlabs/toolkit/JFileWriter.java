    ...
    */
    public static final synchronized void appendToFile(String pth, String text){
        try{
            ...
            outputFile.close();
	    }catch(Exception e){Logger.logInfo("JFileWriter.appendToFile() IO Error : " + e.toString());}
    }
	...
	public boolean writeIt()
	{
		try
		{
	     ...
	     int c;
	     if(outputFile.length() > 0){return false;}
	     while ((c = SR.read()) != -1)out.write(c);
	     out.flush();
	     out.close();
	    }
		catch(IOException e){Logger.logInfo("JFileWriter IO Error : " + e.toString());}
		return true;
	}
    ...
    public void writeLine(String file, String line)
    {
        ...
        try{
            ...
            out.close();
        } catch(FileNotFoundException e){ printErr(e.toString());}
    ...
    public void writeLogToFile(String fname, javax.swing.JTextArea jta)
    {
        ...
        catch(IOException e){printErr(e.toString());}
    }

    public String readLog(String logFname)
    {
        String addTxt = "";
        try {InFile n    = new InFile( logFname );
            while (n.available() != 0){addTxt += n.readLine();}}
        catch(FileNotFoundException e){ printErr(e.toString());}
        catch(IOException e){printErr(e.toString());}
        return addTxt += "\r\n";
    }
