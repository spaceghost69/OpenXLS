 ...
 * should be "wrapped" using the <tt>Collections.synchronizedList</tt> method.
...
class SpecialArrayList extends AbstractList<Object>
	 ...
	 * interface, relying on the backing list's <tt>size()</tt>, <tt>get(int)</tt>,
	 * and <tt>remove(int)</tt> methods.
	 * <p>
	 *
	 * Note that the iterator returned by this method will throw an
	 * <tt>UnsupportedOperationException</tt> in response to its <tt>remove</tt>
	 * method unless the list's <tt>remove(int)</tt> method is overridden.
	 * <p>
	 *
	 ...
	 * order they are returned by the collection's iterator. The <tt>ArrayList</tt>
	 ...
	 * (reduces their index). This call shortens the list by
