 ...
 * 
    ...
    This Class Demonstrates the basic functionality of of OpenXLS Chart Handling.
	...
	*/
	public void testCharts(){
		...
		for(int i = 0;i<testfiles.length;i++){
			if(false){
				java.io.File logfile = new java.io.File("com.valkyrlabs.OpenXLS" + i +".log");        
				try{
					BufferedOutputStream sysout = new BufferedOutputStream( new FileOutputStream(logfile));
					System.setOut(new PrintStream(sysout));
				}catch (IOException e){com.valkyrlabs.OpenXLS.util.Logger.log("IOE: " + e);}
			}
			finpath = testfiles[i];
			sheetname = testfiles[++i];
			com.valkyrlabs.OpenXLS.util.Logger.log("testReadWrite" + String.valueOf(i) + " =====================> " + finpath + ":" + sheetname);
			doit(finpath, sheetname);
			testNewChart();
			createBubbleChart();
			com.valkyrlabs.OpenXLS.util.Logger.log("testReadWrite DONE");
		}
	}
	...
	public void doit(String finp, String sheetnm){
		...
		try{
			...
			for (int i=0; i<charts.length; i++) {
				com.valkyrlabs.OpenXLS.util.Logger.log("Found Chart: " + charts[i]);
			}
			...
			try{
				...
				com.valkyrlabs.OpenXLS.util.Logger.log(e);
			}
			sheet.add(new Float(103.256),"F23");
			sheet.add(Integer.valueOf(125),"G23");
			
			if(ct.changeSeriesRange("Sheet1!C23:E23", "Sheet1!C23:G23"));
			com.valkyrlabs.OpenXLS.util.Logger.log("Successfully Changed Series Range!");
			
			sheet.add("D","F24");
			sheet.add("E","G24");
			if(ct.changeCategoryRange("Sheet1!C24:E24", "Sheet1!C24:G24"));
			com.valkyrlabs.OpenXLS.util.Logger.log("Successfully Changed Categories Range!");
			
			if(ct.changeTextValue("Category X", "Widget Class"));
			com.valkyrlabs.OpenXLS.util.Logger.log("Successfully Changed Categories Label!");
			
			if(ct.changeTextValue("Value Y", "Sales Totals"));
			com.valkyrlabs.OpenXLS.util.Logger.log("Successfully Changed Values Label!");
			
			ct.setTitle("New Chart Title!");
			com.valkyrlabs.OpenXLS.util.Logger.log("Chart Name: "+ ct);
